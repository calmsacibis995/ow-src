#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)ev_once.c 20.27 93/07/08";
#endif
#endif
...
{
    char            buf[BUFSIZE];
    register void  *in_buf, *buf_last_plus_one;
    int             count = 0, read;
	...
	pos = new_pos;
	new_pos = es_read(esh, getBufSize(), buf, &read);
	...
	if (read > 0) {
		...
		read = last_plus_one - pos;
            ...
            if (!multibyte) {
	        buf_last_plus_one = (char *)buf + read;
	        for (in_buf = buf; 
                     (char *)in_buf < (char *)buf_last_plus_one; 
                      in_buf = (char *)in_buf + 1)
		    if (*(char *)in_buf == '\n')
		        count++;
            } else {
 	        buf_last_plus_one = (wchar_t *)buf + read;
	        for (in_buf = (wchar_t *)buf; 
                      (wchar_t *)in_buf < (wchar_t *)buf_last_plus_one; 
                      in_buf = (wchar_t *)in_buf + 1)
		    if (*(wchar_t *)in_buf == (wchar_t)'\n')
		        count++;
	    }
	}
    }
    ...
    int             skip_white_space;
{
    ...
    wchar_t         wc_newline_str[2] = {(wchar_t)'\n', NULL};
    char            newline_str[2] = { '\n', NULL};
	...
	lt_index = 0;
        ...
        if (!multibyte)
	     ...
	     ev_find_in_esh(view->view_chain->esh, wc_newline_str, 1,
		       *first, (unsigned) phys_line, 0,
		       first, &last_plus_one);
    ...
    int             skip_white_space;
{
    ...
    *               cache = &chain_private->cache_pos_for_file_line;
	...
	if (count) {
	        ...
	        ev_find_in_esh(chain->esh, wc_buf, 1, start_search, count, 0,
			   &first, &last_plus_one);
	} else {
	    /*
...
ev_find_in_esh(esh, pattern, pattern_length, position, count, flags,
    ...
    void           *pattern;	/* pattern to search for  */
    ...
    Es_index       *last_plus_one;	/* end of found pattern   */
{
     ...
     */
    char            buf[BUFSIZE + 16];
    register void  *in_buf, *buf_last_plus_one, *matched_to, *done_match;
    register Es_index new_pos, pos, start_pattern = position;
    Es_index        length;
    int             read, test;
    int             useful_bufsize = getBufSize();
    ...
    *first = ES_CANNOT_SET;
    ...
    matched_to = pattern;
    if (!multibyte)
        done_match = (char *)pattern + pattern_length;
    else
        done_match = (wchar_t *)pattern + pattern_length;
...
Init_Backward:
	FOREVER {
		...
		start_pattern + (long)(pattern_length - 1), getBufSize() - 1)) {
	      case 0:
		pos = esbuf.first;
		read = esbuf.last_plus_one - pos;
                if (!multibyte) {
		    in_buf = (char *)buf + (start_pattern - pos);
		    FOREVER {
		        ASSERT((char *)in_buf <= (char *)buf + read);
                        test = (*(char *)matched_to != *(char *)in_buf);
                        matched_to = (char *)matched_to + 1;
                        in_buf = (char *)in_buf + 1; 
		        if (test) {
			    start_pattern--;
		    ...
		    in_buf = (wchar_t *)buf + (start_pattern - pos);
		    FOREVER {
		        ASSERT((wchar_t *)in_buf <= (wchar_t *)buf + read);
                        test = (*(wchar_t *)matched_to != *(wchar_t *)in_buf);
                        matched_to = (wchar_t *)matched_to + 1;
                        in_buf = (wchar_t *)in_buf + 1; 
		        if (test) {
			    start_pattern--;
		        } else if (matched_to == done_match) {
	...
	FOREVER {
		...
		goto Init_Forward;
		    ...
		    FOREVER {
		        ...
		        if ((char *)in_buf == (char *)buf_last_plus_one)
			    break;	/* Next buffer */
		    }
		    ...
		    FOREVER {
		        ...
		        if ((wchar_t *)in_buf == (wchar_t *)buf_last_plus_one)
			    break;	/* Next buffer */
		    }
