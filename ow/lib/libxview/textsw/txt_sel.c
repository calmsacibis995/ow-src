#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)txt_sel.c 20.58 93/12/10";
#endif
#endif
    ...
    Es_index        pos;
{
    ...
    upper_context = (int) ev_get(view->e_view, EV_CHAIN_UPPER_CONTEXT);
    if (multibyte)
        pos = textsw_mbpos_from_wcpos(FOLIO_FOR_VIEW(view), pos);
    textsw_normalize_internal(view, pos, pos,
			      upper_context, 0, TXTSW_NI_DEFAULT);
}
    ...
    Es_index        pos;
{
    if (!multibyte) {
        ...
        textsw_not_visible_normalize(abstract,
			textsw_mbpos_from_wcpos(FOLIO_FOR_VIEW(view), pos));   
    } else
    ...
    register unsigned flags;
{
	...
	if (lines_above > 0) {
	        ...
	        ev_find_in_esh(view->folio->views->esh, newline_str_wc, 1,
			   line_start, (unsigned) lines_above + 1,
			   EV_FIND_BACKWARD, &start, &line_start);
    ...
    */
    if (flags & EV_SEL_PD_PRIMARY)
        if (multibyte)
	    textsw_set_selection_wcs(VIEW_REP_TO_ABS(view), first, 
                last_plus_one,(EV_SEL_BASE_TYPE(flags) | EV_SEL_PD_PRIMARY));
        else
        ...
        if (multibyte)
	    textsw_set_selection_wcs(VIEW_REP_TO_ABS(view), first,
                    last_plus_one,EV_SEL_BASE_TYPE(flags));
        else
	   textsw_set_selection(VIEW_REP_TO_ABS(view), first, last_plus_one,
...
static void 
textswSetSelection(abstract, first, last_plus_one, type)
    Textsw          abstract;
    Es_index        first, last_plus_one;
    unsigned        type;
{
	...
	if( is_select_event ) {
		...
		if( dir != (Textsw_Caret_Direction)0 || do_dir ) {
			...
			if (action == ACTION_SELECT_ALL) {
			    /*
			     * Avoid to clear the selection when all data has
			     * been selected.
			     */
			    if (first == 0 &&
...

{
    ...
    textsw_init_selection_object(folio, &selection, buf, bufsize, 0);
    ...
    int             buf_max_len;
{
	...
	result = 0;
	    ...
	    *((char *)buf + selection.buf_len) = '\0';
  	    ...
  	    *((wchar_t *)buf + selection.buf_len) = (wchar_t)'\0';  
