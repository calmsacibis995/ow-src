#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)ev_edit.c 20.28 93/07/26";
#endif
#endif
...
Pkg_private int getEVBufferSize();
...
/*
 * If byte_flag is 1, return delta as number of the deleted bytes.
 * If byte_flag is 0, return delta as number of the deleted charcters.
 */
Pkg_private int
ev_delete_span(folio, first, last_plus_one, delta, byte_flag)
    Textsw_folio    folio;
    ...
    int		    byte_flag;
{
    ...
    if (multibyte) {
        if (byte_flag) {
	    byte_delta = (Es_index) -textsw_get_bytes_span(folio, first,
						       last_plus_one);
	    /*
	     * The following statement is needed because textsw_get_bytes_span()
	     * changes the es_position.
	     */
