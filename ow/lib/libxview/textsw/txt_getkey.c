#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)txt_getkey.c 20.39 93/11/05";
#endif
#endif
...
Pkg_private void     textsw_do_get();
    ...
    int             local_operands;
{
	...
	if (sec_first < sec_last_plus_one) {
		...
		ev_delete_span(folio,
			       (sec_first < ro_bdry) ? ro_bdry : sec_first,
			       sec_last_plus_one, &delta, 0);
    ...
    unsigned        flags;
{
    ...
    if (multibyte) 
        return 
         ((Textsw_mark)textsw_add_mark_internal(folio, position, flags));
    else
        return ((Textsw_mark) textsw_add_mark_internal(folio,
			 textsw_mbpos_from_wcpos(folio, position), flags));
    ...
    Textsw_mark     mark;
{
    ...
    if (multibyte)
        return ((Textsw_index) textsw_find_mark_internal(folio,
						     *dummy_for_compiler));
    else
        return ((Textsw_index) textsw_wcpos_from_mbpos(folio,
