#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)ev_update.c 20.47 93/07/08";
#endif
#endif
...
ev_display_line(view, width_before, lt_index, first, last_plus_one)
    ...
    Es_index        first, last_plus_one;
{
    static void    *buf;	/* Avoids constant malloc/free */
    void           *bp;
    Ev_chain        chain = view->view_chain;
    ...
    if (buf == NULL) {
        if (!multibyte) 
	    buf = xv_malloc(BUFSIZE_SMALL + 4);
        else
	    buf = xv_malloc((BUFSIZE_SMALL + 4)*sizeof(wchar_t));
    }
    ...
    if (esbuf.sizeof_buf >= BUFSIZE_SMALL)
        if (!multibyte)
            ...
            bp = malloc((esbuf.sizeof_buf + 1) * sizeof(wchar_t));
    else
        bp = buf;
    ...
    esbuf.buf = bp;
    if (!multibyte)
        ((char *)esbuf.buf)[esbuf.sizeof_buf] = '\0';
    else
        ((wchar_t *)esbuf.buf)[esbuf.sizeof_buf] = (wchar_t)'\0';
	...
	do {
	    ...
	    if (esbuf.last_plus_one < last_plus_one) {
                if (!multibyte)
		    esbuf.buf = (char *)esbuf.buf + esbuf.sizeof_buf;
                else
		    esbuf.buf = (wchar_t *)esbuf.buf + esbuf.sizeof_buf;
