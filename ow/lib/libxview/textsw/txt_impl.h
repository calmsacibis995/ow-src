...
#define TXTSW_STRING_LENGTH(ptr_to_string_t)				\
        (multibyte) ?							\
  ((wchar_t *)ptr_to_string_t->free-(wchar_t *)ptr_to_string_t->base) : \
	((char *)ptr_to_string_t->free-(char *)ptr_to_string_t->base)
...
typedef struct textsw_object {
	...
	wchar_t 		  to_insert[TXTSW_UI_BUFLEN];
	void		 *to_insert_next_free;
	...
	_xv_psmbs_t	  filename_mbs; /* for TEXTSW_FILE with xv_get() */
#endif /* OW_I18N */
} Text_object;
...
#define	TXTSW_NOT_SHOWN_AT_INIT	0x80000000  /* CTE# 10141; BID# 1173416 */

#define TXTSW_MISC_UNUSED	0x0000000c
