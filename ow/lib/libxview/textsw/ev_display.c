#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)ev_display.c 20.64 95/09/19";
#endif
#endif
    ...
    int             x, y;
{
    ...
    new_fg = xv_fg(info);
    new_bg = xv_bg(info);
    if ((new_fg != gcvalues.foreground) || (new_bg != gcvalues.background)) {
...
getEVBufferSize()
{
    if (!multibyte)
        ...
        return(EV_BUFSIZE/sizeof(wchar_t));
}
    ...
    int             scroll_by_display_lines;
{
	...
	esbuf.esh = chain->esh;
	esbuf.buf = buf;
	esbuf.sizeof_buf = getEVBufferSize();
...
/* Non-zero return says we're done */
{
	...
	ph->pos_for_line = ph->result.last_plus_one;
        if (!multibyte)
	    ph->esbuf.buf = (char *)(ph->esbuf.buf) +
                        ph->result.last_plus_one - ph->esbuf.first;
        else
	    ph->esbuf.buf = (wchar_t *)(ph->esbuf.buf) +
                        ph->result.last_plus_one - ph->esbuf.first;
	ph->esbuf.sizeof_buf -=
	    ph->result.last_plus_one - ph->esbuf.first;
	...
	if ((ph->whole_buf.last_plus_one < ES_INFINITY) &&
	    ...
	    ph->esbuf.sizeof_buf = getEVBufferSize();
	}
 ...
 */
{
	 ...
	 */
	ASSERT(pos - line_start < getEVBufferSize());
...
Glyph_restart:
	...
	if ((stop_plus_one < ES_INFINITY) &&
	    ...
	    (stop_plus_one - last_plus_one < getEVBufferSize())) {
	...
	while (esbuf.first < esbuf.last_plus_one) {
		...
		break;
            ...
            if (!multibyte)
	        esbuf.buf = (char *)esbuf.buf + 
                          result.last_plus_one - esbuf.first;
            else
	        esbuf.buf = (wchar_t *)esbuf.buf + 
                             result.last_plus_one - esbuf.first;
    ...
    while ((expand_result.break_reason == EI_PR_HIT_RIGHT
	   ...
	   && esbuf.first < esbuf.last_plus_one) {
	    ...
	    expand_result = ei_expand(
				  ...
				  (char *)out_buf + expand_result.last_plus_one,
				  out_buf_len - expand_result.last_plus_one,
				  view->rect.r_left);
        else
	    expand_result = ei_expand(
				  chain->eih, &esbuf, &rect, result.pos.x,
		          (wchar_t *)out_buf + expand_result.last_plus_one, 
				  out_buf_len - expand_result.last_plus_one,
				  view->rect.r_left);
...
#ifdef OW_I18N
Pkg_private	void
ev_set_preedit_region(chain, first, last_plus_one, type)
    Ev_chain        	    chain;
    register Es_index 	    first, last_plus_one;
    unsigned        	    type;
{
