#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)txt_e_menu.c 20.52 97/05/21";
#endif
#endif
...
static	int      extras_menufile_unique();
    ...
    Menu_item       mi;
{
	...
	/* Gee, still?... We will try the old fashioned way */
        (void) snprintf(tmp, MAXPATHLEN, "%s/lib/.%s",
		       filename, EXTRASMENU);
	if ((result = check_filename_locale(NULL, tmp, 1)) != NULL)
		goto found;
#endif
    ...
    int		copy;
{
    ...
    if ((tmp_fd = open(filename, O_RDONLY)) != -1)
	goto found;
...
found:
    if (tmp_fd>0) {
	/* be sure to close any of the opens done above. */
	close(tmp_fd);
    }
    if (copy)
...
static int
extras_menufile_unique(fn)
    char        *fn;
{
    int         i;
 
    for (i = 0; i < Textsw_nextfile; i++)
        if (strcmp( fn, Extras_stat_array[i].name ) == 0)
            return( FALSE );
    return( TRUE );
}
    ...
    Menu            menu;
{
    ...
    if (unique_menufile = extras_menufile_unique( full_file ))
    {
