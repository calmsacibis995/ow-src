#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)convpos.c 1.8 93/07/26";
#endif
#endif
...
/*
 * Calculate number of characters until mlen bytes of multibyte characters is
 * read in wstr. When mlen points a portion of a multibyte character, *adjust
 * will be set with how many bytes should be adjusted to point the front of
 * the multibyte character.
 * Assumption: wbuf is valid as wide character string.
 */
static int
...
int		*adjust;
{
    ...
    /* Using iscodeset?() is faster than using macro wcsetno() */
    for ( ; *mlen > 0 && *wstr ; wstr++) {
	if (iscodeset0(*wstr))
	    *mlen -= 1;
	else if (iscodeset1(*wstr)) {
	    if ((*mlen -= euc_width._eucw1) < 0) {
		*adjust = *mlen + euc_width._eucw1;
		break;
	    }
	}
	else if (iscodeset2(*wstr)) {
	    if ((*mlen -= euc_width._eucw2) < 0) {
		*adjust = *mlen + euc_width._eucw2;
		break;
	    }
	}
	else if (iscodeset3(*wstr))
	    if ((*mlen -= euc_width._eucw3) < 0) {
		*adjust = *mlen + euc_width._eucw3;
		break;
	    }
    }
