#ifndef lint
#ifdef sccs
static char sccsid[] = "@(#)txt_attr.c 20.144 96/12/17";
#endif
#endif
...
Pkg_private textswReplaceBytes();
Pkg_private textswReplaceWCS();
...
/*
**  Fix for 1229317 -
**  Catch the TEXTSW_READ_ONLY case so that it doesn't get
**  consumed from the view attribute list.
*/
 ...
 */
{
	...
	switch (*attrs) {
          case XV_SHOW:
            if (view && view->drop_site) {
                /*
                 *  CTE# 10141; BID# 1173416
                 *  The XV_SHOW TRUE/FALSE handling is added here so that
                 *  the drop target region can be removed when the textsw
                 *  is not being displayed, and added again when it is being
                 *  displayed.
                 *
                 *  Always remove the previous drop target, and add the
                 *  target when XV_SHOW has argument TRUE.
                 */
                xv_set(view->drop_site, DROP_SITE_DELETE_REGION, NULL, NULL);
                if((int)(attrs[1]) == TRUE)
                  xv_set(view->drop_site, DROP_SITE_REGION, &view->rect, NULL);
 
                textsw->state &= ~TXTSW_NOT_SHOWN_AT_INIT;
            } else {
                /*
                 *  During the init process for the textsw widget, the drop
                 *  site for the textsw is created.  This turns into a
                 *  problem when the XV_SHOW/FALSE is on the xv_create() as
                 *  an option to textsw, since the drop site would exist even
                 *  though the textsw is not shown (also BUG# 1173416).
                 *
                 *  The XV_SHOWs actually are taken care of in the window code
                 *  which handles its own drop sites, but never interrogates
                 *  the lower level textsw code.  So the textsw init code
                 *  happily does all of the setup assuming that the widget
                 *  will be XV_SHOWn.
                 *
                 *  An additional complication is that the textsw drop sites
                 *  never consider the status of the textsw as to whether it
                 *  is being shown or not.  Since the drop site for the textsw
                 *  is maintained by the dnd code, the textsw does not have
                 *  the right information to correctly manage when drop sites
                 *  should be registered/deregistered.
                 *
                 *  Hence the following hack.
                 *
                 *  Set this state flag such that if the XV_SHOW/FALSE comes
                 *  in, when view->drop_site does not yet exist, then the
                 *  decision to delete the drop site can be made.
                 */
                if((int)(attrs[1]) == FALSE)
                    textsw->state |= TXTSW_NOT_SHOWN_AT_INIT;
            }
	  ...
	  case XV_END_CREATE:
		...
		if (textsw->need_im) {
		     ...
		     */
		    if (!textsw->ic) { /* first time of IC creation */
			Xv_Drawable_info	*info;

			textsw->ic = (XIC)xv_get(textsw_public, WIN_IC);
			XGetICValues(textsw->ic, XNInputStyle,
				     &textsw->xim_style, NULL);
			DRAWABLE_INFO_MACRO(view_public, info);

			/* make sure XNFocusWindow is set */
			window_set_ic_focus_win(view_public,
						textsw->ic, xv_xid(info));
		    }
		    if (textsw->ic)
		...
		textsw_resize(view);
                ...
                /*
                 *  The init code needs some kind of flag to tell when
                 *  XV_SHOW/FALSE is on the xv_create() call for the textsw.
                 *  When needed, remove the drop site.
                 *
                 *  Note: the textsw_resize() assigns the drop site to the
                 *  textsw, that is why this DROP_SITE_DELETE_REGION is here.
                 *  The intent being to not affect mainline code for a special
                 *  case, I put the logic here and not in textsw_resize()
                 *  which is called in other places as well.
                 */
                if(textsw->state & TXTSW_NOT_SHOWN_AT_INIT)
	  ...
	  case TEXTSW_FILE:
	    ...
	    break;
          case TEXTSW_FILE_WCS:
            if ((wchar_t *)(attrs[1]) == (wchar_t *)NULL)
                file = NULL;
            else {
                (void)wcstombs(name_wc, (wchar_t *) (attrs[1]), MAXPATHLEN);
                file = name_wc;
            }
            break;
	  ...
	  case TEXTSW_FILE_CONTENTS:
		...
		if (scratch_esh) {
		    ...
		    mem_esh = es_mem_create(es_get_length(scratch_esh) + 1,
					    NULL_STRING);
		    if (mem_esh) {
			if (es_copy(scratch_esh, mem_esh, FALSE) 
                                        != ES_SUCCESS) {
			    es_destroy(mem_esh);
			    mem_esh = ES_NULL;
			}
			...
			scratch_esh = es_mem_create(textsw->es_mem_maximum,
						    NULL_STRING);
			if (scratch_esh) {
			    ps_esh = textsw_create_ps
                                (textsw, mem_esh, scratch_esh, &es_status);
			} else {
			    es_destroy(mem_esh);
			}
		    }
		...
		if (str_length > 0) {
			...
			seq = (Ev_impl_line_seq) ev_next->line_table.seq;
                        /* Set damage bit in line table to force redisplay  */
                        for (lt_index = 0;
		             lt_index<ev_next->line_table.last_plus_one;
 			     lt_index++) {
 			    seq[lt_index].damaged = 0;
			}
			/* reset top of view to 0 */
			ev_set(ev_next,EV_DISPLAY_START,0,NULL);
		...
		*status_ptr = TEXTSW_STATUS_OTHER_ERROR;
	    ...
	    break;	        
          ...
          case TEXTSW_FILE_CONTENTS_WCS:
            textsw_flush_caches(view, TFC_PD_SEL);
            ps_esh = ES_NULL;
            str_length = (attrs[1]?wslen((wchar_t *)attrs[1]) : 0);
            if (str_length > 0)
                 (void) wcstombs(name_wc, (wchar_t *)(attrs[1]), MAXPATHLEN);
            textsw_implicit_commit(textsw);
	  ...
	  case TEXTSW_FIRST_WC:
	        ...
	        *status_ptr = set_first(view, error_msg, file, reset_mode,
			textsw_mbpos_from_wcpos(textsw, (Es_index) (attrs[1])),
			-1, all_views);
	       goto MakeSure;
            }
	  case TEXTSW_FIRST_LINE:
...
#ifdef OW_I18N
		    /* 
		     * May need to adjust XNLineSpace if the preedit region
		     * is being drawn by the input method server.  For instance,
		     * if the input method style is overTheSpot update the IC.
		     */
		    if (textsw->ic && (textsw->xim_style & (XIMPreeditArea |
				     XIMPreeditPosition | XIMPreeditNothing))) {
			va_nested_list = XVaCreateNestedList(NULL, XNLineSpace,
			 	(int)ei_get(textsw->views->eih, EI_LINE_SPACE),
			 	NULL);
 ...
 */
{
    ...
    switch (*attrs) {
 	    ...
 	    /* Save the read-only state and set to FALSE (CTE 6507). */
 	    read_only_start = TXTSW_IS_READ_ONLY(textsw);
 	    xv_set (textsw_public, TEXTSW_READ_ONLY, FALSE, NULL);

	    /* Save the browsing state and set to FALSE (CTE 6811). */
	    browsing_start = (int) xv_get (textsw_public, TEXTSW_BROWSING);
	    xv_set (textsw_public, TEXTSW_BROWSING, FALSE, NULL);

            temp = (textsw->state & TXTSW_NO_AGAIN_RECORDING);
            if (!(textsw->state & TXTSW_INITIALIZED))
                textsw->state |= TXTSW_NO_AGAIN_RECORDING;
            (void) textswReplaceBytes(VIEW_REP_TO_ABS(view), 0,
                   TEXTSW_INFINITY, (char *)attrs[1], strlen((char *)attrs[1]));
            if (!(textsw->state & TXTSW_INITIALIZED))
                SET_BOOL_FLAG(textsw->state, temp,
                              TXTSW_NO_AGAIN_RECORDING);

 	    /* Restore the read-only state (CTE 6507). */
 	    xv_set (textsw_public,
 		TEXTSW_READ_ONLY, read_only_start,
 		NULL);

	    /* Restore the browsing state (CTE 6811). */
	    xv_set (textsw_public,
		TEXTSW_BROWSING, browsing_start,
		NULL);
            break;
	    ...
	    xv_set (textsw_public, TEXTSW_BROWSING, FALSE, NULL);
            ...
            if (!(textsw->state & TXTSW_INITIALIZED))
                textsw->state |= TXTSW_NO_AGAIN_RECORDING;
            textsw_implicit_commit(textsw);
            (void) textswReplaceWCS(VIEW_REP_TO_ABS(view), 0,
               TEXTSW_INFINITY,(wchar_t *)attrs[1], wslen((wchar_t *)attrs[1]));
            if (!(textsw->state & TXTSW_INITIALIZED))
                SET_BOOL_FLAG(textsw->state, temp,
                              TXTSW_NO_AGAIN_RECORDING);
		...
		NULL);
 		...
 		/* Save read-only state and set to FALSE (CTE 6507). */
 		read_only_start = TXTSW_IS_READ_ONLY(textsw);
 		xv_set (textsw_public, TEXTSW_READ_ONLY, FALSE, NULL);
    ...
    va_list         args;
{
	...
	textsw_flush_caches(view, TFC_STD);
	    ...
	    return ((Xv_opaque) textsw_wcpos_from_mbpos(folio,
					EV_GET_INSERT(folio->views)));
      case TEXTSW_LOWER_CONTEXT:
	...
	return ((Xv_opaque)
      ...
      case TEXTSW_CONTENTS_WCS_NO_COMMIT: { /* This is used by only Termsw. */
	    ...
	    Es_index        pos = va_arg(args, Es_index);
	    /* temporaries for TEXTSW_CONTENTS */
	    wchar_t           *buf = va_arg(args, wchar_t *);
	...
	return ((Xv_opaque) folio->es_create);
      ...
      case TEXTSW_FILE_WCS:{
            ...
            if (textsw_file_name(folio, &name))
               ...
               return((Xv_opaque)_xv_mbstowcsdup(name)); 
       }
#endif
      ...
      case TEXTSW_SUBMENU_FILE:{
		...
		return 
             ((Xv_opaque) folio->sub_menu_table[(int) TXTSW_FILE_SUB_MENU]);
	}
      case TEXTSW_SUBMENU_EDIT:{
		...
		return 
             ((Xv_opaque) folio->sub_menu_table[(int) TXTSW_EDIT_SUB_MENU]);
	}
      case TEXTSW_SUBMENU_VIEW:{
		...
		return 
               ((Xv_opaque) folio->sub_menu_table[(int) TXTSW_VIEW_SUB_MENU]);
	}
      case TEXTSW_SUBMENU_FIND:{
		...
		return 
             ((Xv_opaque) folio->sub_menu_table[(int) TXTSW_FIND_SUB_MENU]);
	}
      ...
      case TEXTSW_CHECKPOINT_FREQUENCY:
	return ((Xv_opaque) folio->checkpoint_frequency);
      case TEXTSW_HISTORY_LIMIT:
	return ((Xv_opaque)
	...
	return ((Xv_opaque) ev_get((Ev_handle) (view->e_view),
      ...
      case TEXTSW_LINE_BREAK_ACTION:
	switch ( (Ev_right_break) ev_get ( (Ev_handle) (view->e_view),
		EV_RIGHT_BREAK) ) {
		case EV_WRAP_AT_WORD :
			return (TEXTSW_WRAP_AT_WORD);
		case EV_WRAP_AT_CHAR :
			return (TEXTSW_WRAP_AT_CHAR);
		case EV_CLIP :
			return (TEXTSW_CLIP);
		default :  /* NOTE : This _won't_ happen */
			return ( (Xv_opaque) ev_get ( (Ev_handle)
				(view->e_view), EV_RIGHT_BREAK) );
	}
...
static     Es_index
textswGetContents(textsw, position, buffer, buffer_length)
    register Textsw_folio textsw;
    Es_index        position;
    void           *buffer;
...

{
    ...
    if AN_ERROR (read != buffer_length)
        if (multibyte)
	    XV_BZERO((wchar_t *)buffer+read,
                        ((buffer_length-read)*sizeof(wchar_t)));
        else
	    XV_BZERO((char *)buffer+read, buffer_length-read);
    return (next_read_at);
}
...
{
    if (multibyte)
        return(textswGetContents(textsw, position, buffer, buffer_length));
    else {
        Es_index    next_read_at, wc_position;
        int         read;
        char	   *buf_wcs = xv_malloc(buffer_length + 1);

        wc_position = textsw_mbpos_from_wcpos(textsw, position);
	   ...
	   wc_position >= es_get_length(textsw->views->esh)) {
	    next_read_at = (Es_index) es_get_length(textsw->views->esh);
	    read = 0;
        } else {
	    es_set_position(textsw->views->esh, wc_position);
	    (void) es_read(textsw->views->esh, buffer_length, buf_wcs, &read);
	    buf_wcs[read] = NULL;
	    read = mbstowcs(buffer, buf_wcs, buffer_length);
	    next_read_at = (position < 0 ? 0 : position) + read;
        }
        free((char *)buf_wcs);
        if AN_ERROR
	    (read != buffer_length) {
	    XV_BZERO((wchar_t *)buffer + read, buffer_length - read);
	    }
        return (next_read_at);
...

{
    if (!multibyte)
        ...
        Es_index    next_read_at, wc_position;
        int         read;
        wchar_t	   *buf_wcs = xv_malloc((buffer_length + 1) * sizeof(wchar_t));
