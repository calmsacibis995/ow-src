#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)ps_impl.c 20.41 93/08/31";
#endif
#endif
    ...
    register void  *bufp;
{
	...
	if ((private->scratch_position / private->scratch_max_len) !=
	    ...
	    (void) private->scratch_ops->set_position(esh, 0);
			...
			(char *)bufp + count_read, resultp);
            else
	        (void) private->scratch_ops->read(esh, len - count_read,
			(wchar_t *)bufp + count_read, resultp);
	    *resultp += count_read;
	} else {
	...
	*count_used = count;
	...
	if (remainder > 0) {
		...
		if (count > remainder) {
		    count -= remainder;
                    if (!multibyte)
		        buf = (char *)buf + remainder;
                    else
		        buf = (wchar_t *)buf + remainder;
		} else {
		    count = 0;
		}
	    }
	...
	if ((private->scratch_position / private->scratch_max_len) !=
	    ...
	    (void) private->scratch_ops->set_position(esh, 0);
	        ...
	        (void) private->scratch_ops->replace(
						 ...
						 count - remainder,
						 count - remainder,
			      (wchar_t *)buf + remainder, &count_replaced);
             else
	        (void) private->scratch_ops->replace(
						 esh,
						 count - remainder,
						 count - remainder,
			      (char *)buf + remainder, &count_replaced);
	} else {
    ...
    register void  *bufp;
{
    ...
    if (multibyte) {
        wchar_t temp;
        temp = ((wchar_t *)bufp)[*resultp];
        ((wchar_t *)bufp)[*resultp] = '\0';
        (void) fprintf(stdout, "%ws\n^^^\n", (wchar_t *)bufp);
        ((wchar_t *)bufp)[*resultp] = temp;
    } else {
        char temp;
        temp = ((char *)bufp)[*resultp];
        ((char *)bufp)[*resultp] = '\0';
        (void) fprintf(stdout, "%s\n^^^\n", (char *)bufp);
        ((char *)bufp)[*resultp] = temp;
    }
#endif
    return (next);
}
    ...
    register void  *bufp;
{
    ...
    *resultp = 0;
    ...
    while (VALID_PIECE_INDEX(private, current) && len) {
	 ...
	 */
        if (!multibyte) {
	    bufp = (char *)bufp + read_count;
        } else {
	    bufp = (wchar_t *)bufp + read_count;
        }
	...
	if (read_count < to_read) {
		...
		if (*resultp == 0 &&
			...
			read_count = len;
                    ...
                    if (multibyte) {
		        XV_BCOPY(wrap_msg_wcs + current_pos, bufp, 
    ...
    void           *buf;
{
    ...
    (void) fprintf(stdout,
	     "ps_replace [%d..%d) by %d chars (used %d); next pos %d ...\n",
	      pos, last_plus_one, count, *count_used, next);
    if (buf) {
        if (multibyte) {
            wchar_t temp = ((wchar_t *)buf)[count];
	    ((wchar_t *)buf)[count] = '\0';
	    (void) fprintf(stdout, "%ws\n^^^\n", (wchar_t *)buf);
	    ((wchar_t *)buf)[count] = temp;
        } else {
            char temp = ((char *)buf)[count];
	    ((char *)buf)[count] = '\0';
	    (void) fprintf(stdout, "%s\n^^^\n", (char *)buf);
	    ((char *)buf)[count] = temp;
        }
    ...
    void           *buf;
{
    ...
    *count_used = 0;
	...
	int             at_end_of_stream = PS_IS_AT_END(private, delta,
                                                        current);
	if (private->position == private->last_write_plus_one) {
		...
		if (delta != 0) {
		    ...
		    PS_SET_SCRATCH_SANDP(pieces[current],
                     private->rec_insert + (sizeof(long_temp)/sizeof(wchar_t)));
                else
		    PS_SET_SCRATCH_SANDP(pieces[current],
	 ...
	 */
            if (multibyte) {
		     ...
		     private->rec_insert + (sizeof(long_temp)/sizeof(wchar_t)),
		     sizeof(long_temp)/sizeof(wchar_t), (void *) &long_temp,
				 &replace_used);
            } else {
	         es_temp = es_replace(scratch,
		     private->rec_insert + sizeof(long_temp),
		     sizeof(long_temp), (void *) &long_temp,
				 &replace_used);
            }
	} else {
	    if (delta != 0) {
		/* try to reduce the finger table, to prevent growth,
		   but only if scratch has wrapped, more than 2 * max
		   data has gone thru, and current is a bit away from
		   the beginning. */
                if(SCRATCH_HAS_WRAPPED(private)
		    ...
		    int do_shift = TRUE;
			    ...
			    /* if first_valid is earlier than last_plus_one/4
			       then don't bother shifting. */
			    do_shift = FALSE;
			}
		    }
		    if (do_shift) {
			for(old_start++;
			    ...
			    old_start++);
			if (old_start > 2) {
			    ft_shift_out(&private->pieces, 2,
					 old_start);
			    INVALIDATE_CURRENT(private);
			    current = current - (old_start - 2);
			}
		    }
                }
		/* Split the current piece at the insert point */
	...
	if (count != 0) {
	    pieces[current].length = count;
            if (multibyte)
	        ASSERT(es_temp == private->rec_insert + 
                                  (sizeof(long_temp)/sizeof(wchar_t)));
            else
    ...
    int             dp_count;
{
    ...
    if (multibyte)
	   ...
	   sizeof(r_header)/sizeof(wchar_t), (void *) &r_header,
			&replace_used);
    else
        result = es_replace(esh, ES_INFINITY,
	   sizeof(r_header), (void *) &r_header,
			&replace_used);
    return (result);
}
    ...
    Es_index       *next;
{
     ...
     */
	    ...
	    replace_result =
		   ...
		   sizeof(d_header)/sizeof(wchar_t),(void *) &d_header, &dummy);
        else
            replace_result =
	        es_replace(esh, ES_INFINITY,
		   sizeof(d_header),(void *) &d_header, &dummy);
    ...
    int             first_deleted, last_plus_one_deleted;
{
	...
	*deleted_pieces_length =
    ...
    if (multibyte)
        *contents_start =
	    es_replace(esh, ES_INFINITY, sizeof(count)/sizeof(wchar_t),
		   (void *) &count, &replace_used);
    else
        *contents_start =
 ...
 */
{
	 ...
	 */
        if (multibyte) {
	    (void) es_read(scratch, sizeof(r_header)/sizeof(wchar_t),
		       (void *) &r_header, &read);
	    ASSUME(read == sizeof(r_header)/sizeof(wchar_t));
        } else {
	 ...
	 */
        if (multibyte) {
	    (void) es_replace(scratch, 
                 private->rec_start + sizeof(r_header)/sizeof(wchar_t),
	         sizeof(r_header)/sizeof(wchar_t), (void *) &r_header, &read);
	    ASSUME(read == sizeof(r_header)/sizeof(wchar_t));
        } else {
	    (void) es_replace(scratch, 
                 private->rec_start + sizeof(r_header),
	         sizeof(r_header), (void *) &r_header, &read);
	    ASSUME(read == sizeof(r_header));
        }
	...
	if ((r_header.start == r_header.stop_plus_one) &&
	    ...
	    for (i = r_header.dp_count; i > 0; i--) {
	 	 ...
	 	 */
                if (multibyte)
		    (void) es_read(scratch, sizeof(d_header)/sizeof(wchar_t),
			       (void *) &d_header, &read);
                else
	 ...
	 */
        if (multibyte)
	    (void) es_read(scratch, sizeof(insert_len)/sizeof(wchar_t),
		       (void *) &insert_len, &read);
        else
