#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)txt_input.c 20.120 94/05/25";
#endif
#endif
...
#define	TXT_INSERT_LEN(x) ((TXTSW_UI_BUFLEN - \
  ((wchar_t *)((x)->to_insert_next_free) - (wchar_t *)((x)->to_insert))) - 1)
    ...
    register int    flags;
{
    ...
    if (textsw->func_state & TXTSW_FUNC_FILTER)
        ...
        if (!multibyte)
	    count = (char *)textsw->to_insert_next_free - 
                       (char *)textsw->to_insert;
        else
	    count = (wchar_t *)textsw->to_insert_next_free - 
                       (wchar_t *)textsw->to_insert;
    }
    ...
    Notify_arg      arg;
{
    ...
    char	    byte;
    void	    partial_commit();
    Textsw parent = TEXTSW_PUBLIC (view->folio);
    ...
    if (multibyte) {  
	    ...
	    int    num_of_char = mbstowcs(textsw->to_insert_next_free,
			 	      event_string(ie), TXT_INSERT_LEN(textsw));
	    textsw->to_insert_next_free = 
                 (wchar_t *)textsw->to_insert_next_free + num_of_char;

	    if (!textsw->preedit_start) /* preedit text has been erased */
	        textsw_flush_caches(view, TFC_STD);
	    else /* preedit still exists, must be partial commit. */
	        partial_commit(textsw, num_of_char);
    ...
    /* NOTE: This is just a workaround for performance */
	...
	if (down_event) {
	    ...
	    /* taken care of -- new mouseless function  */
	...
	if (textsw->func_state & TXTSW_FUNC_FILTER) {
            if (!multibyte) { 
	        if ((char *)textsw->to_insert_next_free <
		    (char *)textsw->to_insert + 
                      sizeof((char *)(textsw->to_insert))) {
		    *(char *)(textsw->to_insert_next_free) = (char)action;
		    textsw->to_insert_next_free = 
                             (char *)textsw->to_insert_next_free + 1;
	        }
            } else {
	        if ((wchar_t *)textsw->to_insert_next_free <
		    (wchar_t *)textsw->to_insert + 
                      sizeof((wchar_t *)(textsw->to_insert))) {
		    *(wchar_t *)(textsw->to_insert_next_free) = (wchar_t)action;
		    textsw->to_insert_next_free =
                            (wchar_t *)textsw->to_insert_next_free + 1;
                }
            }
	} else {
    Process_ASCII:
		...
		if (down_event) {
			...
			goto Read_Only;
		        /*
		         * This is a workaround for bug 1090046.
		         * textsw_do_newline()called in textsw_preedit_draw()
		         * after erasing a preedit text.
		         */
		...
		if (textsw->state & TXTSW_CAPS_LOCK_ON) {
		    ...
		    /* BUG ALERT: above may need to set event_id */
                ...
                if (multibyte) {
		    /*
		     * Asian Textsw dosen't handle NULL character.
		     * If the spec is changed to handle NULL character
		    ...
		    if (isascii(byte)) {
		        ...
		        *(wchar_t *)(textsw->to_insert_next_free) = byte;

		    } else
#endif
		    if (mbtowc(textsw->to_insert_next_free, &byte, 1)
			== -1) goto Done;
		    textsw->to_insert_next_free =                
                         (wchar_t *)textsw->to_insert_next_free + 1;
		    if (textsw->preedit_start) /* preedit text still exists. */
		        partial_commit(textsw, 1);
		    if ((wchar_t *)textsw->to_insert_next_free ==
		        (wchar_t *)textsw->to_insert +
		        sizeof((wchar_t *)textsw->to_insert)) {
		        textsw_flush_caches(view, TFC_STD);
		    ...
		    *(char *)(textsw->to_insert_next_free) =
    ...
    register Event *ie;
{
    ...
    switch (result->type) {
      ...
      case TXTSW_KEY_FILTER:{
	    ...
	    textsw->func_state &= ~TXTSW_FUNC_FILTER;
            if (!multibyte)
	        textsw->to_insert_next_free = (char *)textsw->to_insert;
            else
	        textsw->to_insert_next_free = (wchar_t *)textsw->to_insert;
	    if (again_state == 0)
		textsw->func_state &= ~TXTSW_FUNC_AGAIN;
	    result = 0;
	}
    ...
    int action;
{
    ...
    Es_index        first = EV_GET_INSERT(folio->views), last_plus_one,
                    previous;
    wchar_t         newline_str_wc[2];
    char            newline_str[2];
    
    newline_str[0] = action; /* 1030878 */
    newline_str[1] = NULL;
    newline_str_wc[0] = action; /* 1030878 */
    newline_str_wc[1] = NULL;
    ...
    if (!multibyte)
        ...
        delta = textsw_do_input(view,newline_str_wc,1,TXTSW_UPDATE_SCROLLBAR);
	...
	previous = first;
	    ...
	    textsw_find_pattern(folio, &previous, &last_plus_one,
			    newline_str_wc, 1, EV_FIND_BACKWARD);
	...
	if (previous != ES_CANNOT_SET) {
	        ...
	        esbuf.esh = folio->views->esh;
	        esbuf.buf = buf;
	        esbuf.sizeof_buf = sizeof(buf)/sizeof(wchar_t);
	        if (es_make_buf_include_index(&esbuf, previous, 0) == 0) {
		    if (AN_ERROR(buf[0] != '\n')) {
		    } else {
		        for (c = buf + 1; 
                             c < buf + sizeof(buf)/sizeof(wchar_t); 
                             c++) {
			    switch (*c) {
			      case '\t':
			      case ' ':
    ...
    Textsw_view_handle view;
{
    ...
    if (!multibyte)
        ...
        textsw_init_selection_object(folio, &selection, NULL_STRING, 0, FALSE);
    ...
    int            *result;
{
    ...
    register int    down_event = event_is_down(ie);
    Textsw	    abstract = TEXTSW_PUBLIC (folio);
    ...
    register Event *ie;
{
    ...
    switch (action) {
      case TXTSW_POINT:{
		...
		if (current_first != ES_CANNOT_SET)
		    folio->column_offset = pos - current_first;
...
/* This macro is produced in order to reduce OW_I18N ifdef. */
#ifdef OW_I18N
#define TEXTSW_MOVE_CARET(v, act) \
{ \
	textsw_implicit_commit(FOLIO_FOR_VIEW(v)); \
	textsw_move_caret(v, act); \
}
#else
#define TEXTSW_MOVE_CARET(v, act)	textsw_move_caret(v, act)
#endif
 ...
 *		     3. preedit text exists.
 *
 *   In onTheSpot style, implicit commit has to be executed when
 *   folio->preedit_start is non NULL. Because preedit_start with 0 means
 *   the Assumption above is filled.
 *
 *   In overTheSpot style, textsw can not know if the preedit text exist.
 *   Therefore, implicit commit will always be executed when conversion mode
 *   is ON. This is a performance problem!!
 */
Pkg_private	void
textsw_implicit_commit(folio)
    register Textsw_folio	folio;
{
    ...
    if (folio->xim_style & XIMPreeditCallbacks) { /* onTheSpot */
	if (!folio->preedit_start)	/* no preedit text */
	    return;
    }
    else { /* overTheSpot */
	if (!folio->ic ||
	    (xv_get(TEXTSW_PUBLIC(folio), WIN_IC_CONVERSION) == FALSE))
	    return;
    }
     ...
     *  after textsw_implicit_commit(). The real preedit_erasing sent by IM
     *  will process nothing because the preedit text was already erased.
     *  This is a workaround for such IM server. IM server should send the
     *  preedit_erasing before sending the committed text, when IC is reseted. 
     *
     *  In case of overTheSpot style, this workaround will not be executed as
     *  folio->preedit_start is always 0.
     */
    if (folio->preedit_start) { /* preedit text isn't erased yet. */
	...
	int	num_of_char = mbstowcs((wchar_t *)folio->to_insert_next_free,
				       committed_string,
				       TXT_INSERT_LEN(folio));
	folio->to_insert_next_free = 
               (wchar_t *)folio->to_insert_next_free + num_of_char;
...
/* Insert text at the front of a preedit text */
static void
partial_commit(textsw, insert_wlen)
register Textsw_folio textsw;
int	insert_wlen;
{
	Es_index	old_insert_pos = EV_GET_INSERT(textsw->views);
	Es_index	mark_pos, tmp;

	/* Set insertion position to the front of a preedit text */
	mark_pos = textsw_find_mark_internal(textsw, textsw->preedit_start);
	EV_SET_INSERT(textsw->views, mark_pos, tmp);

	/* For inserting a committed text as a normal text */
	...
	/* Update preedit_start to the front(left) of the preedit text */
	...
	/* Reset insertion pos to the back(right) of the preedit text */
	EV_SET_INSERT(textsw->views, old_insert_pos + insert_wlen, tmp);
}
#endif /* OW_I18N */
