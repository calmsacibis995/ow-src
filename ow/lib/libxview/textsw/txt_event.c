#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)txt_event.c 20.66 93/11/05";
#endif
#endif
    ...
    Notify_event_type type;	/* Currently unused */
{
	...
	if (folio->ic && folio->focus_view != view_public) {
	    ...
	    /* This is workaround for 1127889. */
	    if (xv_get(view_public, WIN_IC_ACTIVE))
		XSetICFocus(folio->ic);
	}
#endif
    ...
    register Event *event;
{
	...
	textsw_implicit_commit(folio);
	    ...
	    textsw_possibly_normalize(VIEW_REP_TO_ABS(view), new_position );
	}
    }
...
#ifdef OW_I18N
    /* 
     * Update XNSpotLocation if the preedit region is being drawn
     * by the IM server.  For instance, if the input method style
     * is overTheSpot.
     */
    if (textsw->ic && (textsw->xim_style & XIMPreeditPosition) 
                && textsw->focus_view) {
        XPoint		loc;
        int			x, y;
    ...
    Event *event;
{
    ...
    if ((xevent_next.type == KeyRelease) &&
      ...
      (c == (((char *)folio->to_insert_next_free)-(char *)1))) {
