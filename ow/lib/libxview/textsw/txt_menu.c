#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)txt_menu.c 20.95 93/12/16";
#endif
#endif
    ...
    Textsw_folio    folio;
{
    ...
    /* Initialize the menu_items memory to zero.*/
    memset ( (void *) menu_items, 0,
	(size_t) (TEXTSW_MENU_LAST_CMD * sizeof (Menu_item) ) );
    ...
    /* Set the MENU_RELEASE attribute for all the menu items, so they'll be
	freed. */
    for (index = 0; index < TEXTSW_MENU_LAST_CMD; index++)
	if (menu_items [index] )
		xv_set (menu_items [index], MENU_RELEASE, NULL);
    xv_set (undo_cmds_item, MENU_RELEASE, NULL);
    xv_set (break_mode_item, MENU_RELEASE, NULL);
    xv_set (find_sel_cmds_item, MENU_RELEASE, NULL);
    xv_set (select_field_cmds_item, MENU_RELEASE, NULL);
    ...
    Event          *ie;
{
    ...
    /* freeze caret so don't invalidate menu's bitmap under data.
       only do this if this is really a textsw.  This could be a cmdtool. */
    if (!(folio->state & TXTSW_NO_RESET_TO_SCRATCH)) {
    ...
    Menu_item       cmd_item;
{
    ...
    static wchar_t wc_bar_lt[] = { '<', '|',  0 };
    static wchar_t wc_bar_gt[] = { '|', '>',  0 };
	...
	break;
      ...
      case TEXTSW_MENU_FIND_AND_REPLACE:{
	    ...
	    Frame           popup = (Frame) xv_get(base_frame, XV_KEY_DATA,
						   SEARCH_POPUP_KEY);
	    if (popup) {
		Panel ppanel = (Panel)xv_get(popup, FRAME_CMD_PANEL);
		int i = 0;
		extern Panel_item search_panel_items[];
		PANEL_EACH_ITEM(ppanel,search_panel_items[i])
		    i++;
		PANEL_END_EACH
		(void) textsw_get_and_set_selection(popup, view,
