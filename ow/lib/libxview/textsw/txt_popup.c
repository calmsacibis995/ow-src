#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)txt_popup.c 1.56 93/07/26";
#endif
#endif
     ...
     Destroy_status status;
{
    ...
    if (textsw) {
	/* clear the key so that the second time thru this function, textsw
	   will be 0. */
	xv_set(popup_frame,XV_KEY_DATA, TEXTSW_CURRENT_POPUP_KEY, 0, NULL);
    ...
    int             max_str_len;
{
    ...
    char            selection_buf[MAX_STR_LENGTH * sizeof(wchar_t)];
    unsigned        options = EV_SEL_PRIMARY;

    textsw_init_selection_object(
	 folio, &selection, selection_buf, getBufSize(MAX_STR_LENGTH), FALSE);
	...
	if (selection.buf_len >= max_str_len)
	    ...
	    selected_str[selection.buf_len] = NULL;
            ...
            int i, j;
            char *selectedStr = selected_str;
            wchar_t *selectionBuf = (wchar_t *)selection.buf;
            for (i = 0; i<selection.buf_len;i++) {
                j = wctomb(selectedStr, *selectionBuf++);
                selectedStr += j;
            }
            *selectedStr = NULL;
        }
