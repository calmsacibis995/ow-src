#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)termsw.c 1.62 95/11/01";
#endif
#endif
...
Pkg_private     Xv_opaque
termsw_folio_get(termsw_folio_public, status, attr, orig_args)
    Termsw_view     termsw_folio_public;
    int            *status;
...
Pkg_private     Xv_opaque
termsw_view_get(termsw_view_public, status, attr, orig_args)
    Termsw_view     termsw_view_public;
    int            *status;
    Attr_attribute  attr;
    va_list         orig_args;
    ...
    Xv_Window       termsw_view_public;
{
	 ...
	 */
        length = (int) getTextswLength(termsw_view_public);
	termsw_folio->user_mark =
	    textswAddMark(termsw_view_public,
