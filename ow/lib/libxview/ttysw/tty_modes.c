#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)tty_modes.c 20.55 95/05/18";
#endif
#endif
    ...
    Ttysw_view_handle ttysw_view;
{
    ...
    Textsw          textsw;
    Textsw_view_handle textsw_view_handle;
    Termsw_folio    termsw;
    int             off = 0;
    int             fd;
#ifdef OW_I18N
    ...
    textsw = ttysw->current_view_public;	/* Textsw really need the
						 * public view handle */
    textsw_view_handle = VIEW_PRIVATE(textsw);
...
#   endif /* XV_USE_SVR4_PTYS */
    ...
    /* clear the dropsite */
    if (textsw_view_handle->drop_site) 
	xv_set(textsw_view_handle->drop_site, DROP_SITE_DELETE_REGION, NULL,
	       NULL);
    return (0);
}
...

{
	...
	textsw_do_resize(textsw_view);
         ...
         *  If XV_SHOW/FALSE has been applied to the textsw before, and we
         *  find ourselves here, then we have to erset the drop site.
         *
         *  The reset only need happen when textsw_do_resize() is not called,
         *  since that code path also does the dropsite reset.
         */
        textsw_reset_dropsite(textsw_view);
    }
    ...
    int             old_cooked_echo, new_cooked_echo;
{
	 ...
	 */
	if (textswFindMark(textsw, termsw->pty_mark) ==
		...
		termsw->read_only_mark =
		    textswAddMark(textsw, length, TEXTSW_MARK_READ_ONLY);
	    }
	}
    }
