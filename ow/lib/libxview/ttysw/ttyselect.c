#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)ttyselect.c 20.54 97/05/21";
#endif
#endif
    ...
    register char  *data;
{
	...
	if (row == begin->tsp_row && row == end->tsp_row) {
	     ...
	     */
            proc(begin->tsp_col, linelength[row], row, data, ttysel);
	     ...
	     * Full line hilite
	     */
            proc(0, linelength[row], row, data, ttysel);
	}
    }
}
    ...
    int             start, end, row;
{
    ...
    if (!multibyte) {
        ...
        int  cwidth;
        int  offset;
        wchar_t *str = ((wchar_t **)image)[row];
        wchar_t temp_char = (wchar_t)'\0';
	    ...
	    temp_char = str[end + 1];
	    str[end + 1] = (wchar_t)'\0';
            (void) ttysw_pclearline(start, wslen(str), row);
        } else
            (void) ttysw_pclearline(start, wslen(str) + 1, row);
        (void) ttysw_pstring((str + start), boldify, start, row, PIX_SRC);
    ...
    struct inputevent *event;
{
    ...
    if (tb->tsp_col > linelength[tb->tsp_row])
        tb->tsp_col = linelength[tb->tsp_row];
           
    *te = *tb;

    if (multibyte) {
        register wchar_t  *line = ((wchar_t **)image)[tb->tsp_row];
        int             cwidth;
        int             offset;
        ...
        case SEL_WORD:{
            ...
            for (col = te->tsp_col; col < linelength[tb->tsp_row]; col++) {
                wchr = line[col];
        ...
        case SEL_LINE:
            tb->tsp_col = 0;
            te->tsp_col = linelength[tb->tsp_row] - 1;
            break;
        case SEL_PARA:{
            register int    row;
            ...
            te->tsp_col = linelength[te->tsp_row] - 1;
            break;
          }
        } 
    } else {  
        register char  *line = ((char **)image)[tb->tsp_row];
        switch (level) {
        case SEL_CHAR:
	    break;
        case SEL_WORD:{
		...
		if (linelength[row] == 0)
		    break;
	    te->tsp_row = MAX(te->tsp_row, row - 1);
	    te->tsp_col = linelength[te->tsp_row];
	    break;
	  }
        }
    }
    ...
    register int                        start, finish;
{
    ...
    if (multibyte) {
        wchar_t  *line = ((wchar_t **)image)[row];
    ...
    if (linelength[row] == finish && finish == ttysw_right)
    ...
    register FILE  *file;
{
    ...
    for (char_index = start; char_index <= finish; char_index++) {
        if (linelength[row] == char_index) {
            ...
            if (multibyte) {
                if (*(((wchar_t **)image)[row] + char_index) != TTY_NON_WCHAR)
                    putwc(*(((wchar_t **)image)[row] + char_index), file);
            } else
	        putc(*(((char **)image)[row] + char_index), file);
        }
    }
}
    ...
    Seln_holder    *holder;
{
    ...
    seln_query(holder, ttysel_copy_in, (char *) (&context),
		   SELN_REQ_CHARSIZE, TTY_INFINITY,
		   0);
    query_result =
    	seln_query(holder, ttysel_copy_in, (char *) (&context),
    ...
    if( query_result != SELN_SUCCESS ) {
    	(void) seln_query(holder, ttysel_copy_in, (char *) (&context),
		      SELN_REQ_BYTESIZE, TTY_INFINITY,
    ...
    int             buffer_length;
{
	...
	*context->response_pointer++ = (char *) count;
	...
	if (!multibyte) {
        ...
        if (buffer_length < sizeof(wchar_t **))
               return SELN_FAILED;
        count = 0;
        if (!ttysel->sel_null)
            ttyenumerateselection(ttysel, ttycountchars, (char *) (&count));
    ...
    register int    max_length;
{
    ...
    while (curr_row < last_row) {
         ...
         */
        row_end = linelength[curr_row];

        if (multibyte) {
            wchar_t *src = ((wchar_t **)image)[curr_row] + curr_col;
            for( i = curr_col; i < row_end ; i++ ) {
                if( *src == TTY_NON_WCHAR ){
                    src++;
                    continue;
                }
                char_len = wctomb( tmp, *src++ );
                if( char_len == 0 )
                        char_len = 1;
                ...
                XV_BCOPY( tmp, dest, char_len );
                dest += char_len;
            }
            row_col_len = linelength[curr_row] - curr_col;
        } else {
            char *src = ((char **)image)[curr_row] + curr_col;
	    row_col_len = MIN(linelength[curr_row] - curr_col, max_length);
	    i = row_col_len;
	    while (i--) {
	        *dest++ = *src++;
	    }
	    if ((max_length -= row_col_len) <= 0) {
	        goto continue_reply;
	    }
	}
    ...
    if (multibyte) {     
        ...
        char *src = ((char **)image)[curr_row] + curr_col;
        row_end = MIN(end_col + 1 - curr_col, max_length);
        i = row_end;
        while (i--)
	    *dest++ = *src++;
        if ((max_length -= row_end) <= 0)
	    goto continue_reply;

    }

    if (end_col == linelength[curr_row] && end_col < ttysw_right) {
        dest[-1] = '\n';
        *dest = '\0';
        max_length--;
    }
    ...
    register int    max_length;
{
    ...
    last_row = ttysel->sel_end.tsp_row;
    end_col = ttysel->sel_end.tsp_col;
    dest = (wchar_t *) context->response_pointer;
    ...
    while (curr_row < last_row) {
        if (multibyte) {
            wchar_t *src = ((wchar_t **)image)[curr_row] + curr_col;
            row_len = MIN( tty_get_nchars(curr_col, 
                 TTY_LINE_INF_INDEX, curr_row), max_length/sizeof(wchar_t));
            row_col_len = 0;
            index = row_len;
            while (index) {
                row_col_len ++;
            ...
            if ((max_length -= row_len * sizeof(wchar_t)) <= 0) {
                goto continue_reply;
            }
            if (row_col_len + curr_col != ttysw_right) {
                *dest++ = (wchar_t)'\n';
            ...
            char *src = ((char **)image)[curr_row] + curr_col;
            row_len = MIN( tty_get_nchars(curr_col, 
                 TTY_LINE_INF_INDEX, curr_row), max_length);
            row_col_len = row_len;
            index = row_len;
            while (index--) {
                (void)mbtowc(dest++,src++,1);
            }
    ...
    if (multibyte) {
        ...
        char *src = ((char **)image)[curr_row] + curr_col;
        row_len = MIN( tty_get_nchars(curr_col, end_col, curr_row),max_length);
        index = row_len;
        row_col_len = row_len;
        while (index--) {
            (void) mbtowc(dest++,src++,1);
    ...
    register Seln_request *buffer;
{
		   ...
		   result = SELN_SUCCESS;
                ...
                case SELN_REQ_CHARSIZE:
                   ...
                   if (multibyte) 
                        current_size = MIN(context->bytes_left,
                                         (buffer->buf_size - 3 * 
                                   sizeof(Seln_attribute))/sizeof(wchar_t));
                   else
                       current_size = MIN(context->bytes_left,
                             (buffer->buf_size - 3 * sizeof(Seln_attribute)));
	...
	if (result==SELN_FAILED) {
        ...
        if (multibyte)
            current_size = MIN(context->bytes_left, 
                                  buffer->buf_size/sizeof(wchar_t));
        else
    ...
    */
    if( context->request_is_ascii ){
       ...
       (void) ttysw_input_it(ttysw, (char *) response_ptr, true_len);
        ...
        if (multibyte) {
            true_len = wslen( (wchar_t *) response_ptr );
            (void) ttysw_input_it_wcs(ttysw, (wchar_t *)response_ptr, true_len);
        } else {
	    /* transfering in COMPOUND_TEXT which is really wide_char,
	       but running in a single byte locale */
	    char *buf;
	    int buflen;
	    buflen = (wslen((wchar_t *)response_ptr) * MB_CUR_MAX) + 1;
	    buf = xv_malloc (buflen);
	    buflen = wcstombs(buf,(wchar_t *)response_ptr, buflen);
            (void) ttysw_input_it(ttysw, buf, buflen);
	    xv_free(buf);
	}
    }
