#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)tty_mapkey.c 20.43 97/05/21";
#endif
#endif
    ...
    struct ttysubwindow *ttysw;
{
	...
	return;
    (void) strncpy(rc, p, 1025);
    (void) strncat(rc, "/.ttyswrc", 1025 - strlen(rc));
	...
	if ((int)defaults_get_boolean("term.useAlternateTtyswrc",
		...
		if ((p=(char*)getenv("OPENWINHOME")) != (char *)NULL) {
		    (void)strncpy(rc, p, 1025);
		    (void) strncat(rc, "/lib/.ttyswrc", 1025 - strlen(rc));
		} else
    ...
    struct inputevent *ie;
{
	...
	for (kmt = ttysw->ttysw_kmt; kmt < ttysw->ttysw_kmtp; kmt++) {
		...
		len = strlen(kmt->kmt_to);
                ...
                if (multibyte) {
		    int	     len_wcs;
		    wchar_t  *kmt_to_wcs, ktw_buf[1024];

		    if ( len >= (1024/sizeof(wchar_t)) ) {
		        kmt_to_wcs =(wchar_t *)xv_malloc(len*sizeof(wchar_t));
		    } else
		        kmt_to_wcs = ktw_buf;
		    ...
		    if (kmt->kmt_output)
		        (void)ttysw_output_wcs(ttysw->public_self
                                               , kmt_to_wcs, len_wcs);
		    else
		    ...
		    if (kmt->kmt_output)
		        (void)ttysw_output(ttysw->public_self,kmt->kmt_to,len);
		    else
    ...
    char           *s;
{
    ...
    p = xv_malloc((unsigned) (strlen(s) + 1));
    if (p == (char *) NULL) {
	xv_error(NULL,
