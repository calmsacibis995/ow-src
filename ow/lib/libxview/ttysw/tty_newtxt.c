#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)tty_newtxt.c 1.45 93/07/08";
#endif
#endif
    ...
    int             len;
{
    ...
    if (pixfont != (Xv_opaque) xv_get(screen, XV_KEY_DATA, 
                      TTY_CURRENT_FONT_KEY) || old_gc_list != gc_list) {
        setup_font(window, pixfont);
	...
	old_depth = xv_depth(info);
        ...
        if (multibyte) 
            ...
            if (op == PIX_SRC || op == PIX_NOT(PIX_SRC))
	        routine2 = XDrawImageString;
	    else
	      routine2 = XDrawString;
    ...
    new_fg = xv_fg(info);
    new_bg = xv_bg(info);
    if (((new_fg != gc_values.foreground) || 
	 (new_bg != gc_values.background)) && 
    ...
    if (multibyte)
        (void) (*routine) (display, drawable, xv_get(pixfont, FONT_SET_ID),
                  *gc, xbasew, ybasew, (wchar_t *)string, len);
    else
        (void) (*routine2) 
                 (display, drawable, *gc, xbasew, ybasew, (char *)string, len);
}
    ...
    int             sX, sY, W, H, dX, dY;
{
    ...
    XSetState(display, gc_list[BACK_GC], xv_fg(info), xv_bg(info), GXcopy, 
               AllPlanes);
