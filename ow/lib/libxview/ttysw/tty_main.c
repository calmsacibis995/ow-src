#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)tty_main.c 20.101 97/06/25";
#endif
#endif
    ...
    register Ttysw_folio ttysw;
{
    ...
    if (!multibyte) {
		...
		|| c == tty_geteofc(ttysw)
		|| c == tty_geteolc(ttysw)
		|| c == tty_geteol2c(ttysw))
	    return (1);
    } else {
        wchar_t c = *(iwbp.WC - 1);
        if (c == (wchar_t)'\n'
		|| c == (wchar_t)tty_geteofc(ttysw)
		|| c == (wchar_t)tty_geteolc(ttysw)
		|| c == (wchar_t)tty_geteol2c(ttysw))
	    return (1);
    }
    ...
    int             pty;
{
	...
	Termsw_folio    termsw = TERMSW_FOLIO_FOR_VIEW    
                          (TERMSW_VIEW_PRIVATE_FROM_TTY_PRIVATE(ttysw));

	if (termsw->pty_eot > -1) {
	    char *eot_bp = ibuf.SB + termsw->pty_eot;

	    /* write everything up to pty_eot */
	    if (eot_bp >= irbp.SB) {		/* was: > */
		cc = write(pty, irbp.SB, eot_bp - irbp.SB);

#ifdef DEBUG
	        printf("write to pty: ");
	        ttysw_print_debug_string(irbp.SB, eot_bp - irbp.SB);
#endif /* DEBUG */
	...
	if (!ttysw_pty_output_ok(ttysw))
	    return;
	cc = write(pty, irbp.SB, iwbp.SB - irbp.SB);

#ifdef DEBUG
	printf("write to pty: ");
	ttysw_print_debug_string(irbp.SB, iwbp.SB - irbp.SB);
#endif /* DEBUG */

	if (cc > 0) {
	    irbp.SB += cc;
	    if (irbp.SB == iwbp.SB)
		irbp.SB = iwbp.SB = ibuf.SB;
	} else if (cc < 0)
	    perror(XV_MSG("TTYSW pty write failure"));
    }
}
    ...
    int         nbytes;
{
    ...
    static char mbs_left[TTYSW_SMALLBUF];
...
static int
ttysw_pty_outputWC(ttysw, pty)
    register Ttysw_folio ttysw;
    int             pty;
{
    wchar_t  nullc = (wchar_t)'\0';
    char     mb_irbp[TTYSW_BUFLEN];
    wchar_t  save_char;
    int      mb_nbytes;
    register int    cc;
    ...
    if (ttysw_getopt((caddr_t) ttysw, TTYOPT_TEXT)) {
        ...
        if (termsw->pty_eot > -1) {
            wchar_t *eot_bp = ibuf.WC + termsw->pty_eot;

            /* write everything up to pty_eot */
            if (eot_bp >= irbp.WC) {
                /* convert wchar_t to char and calls write() */
                save_char = *eot_bp;
                *eot_bp   = nullc;
                mb_nbytes = wcstombs_with_null(mb_irbp,irbp.WC,
                                                    eot_bp - irbp.WC);
                *eot_bp   = save_char;
                if( mb_nbytes == -1 ) {
                    perror(XV_MSG(
                          "TTYSW:ttysw_pty_output: invalid wide character"));
                    return;
                }
    ...
    if (iwbp.WC > irbp.WC) {
        /*
         * Bail out if we need to present a complete input line but don't 
         * have one yet.
         */
        if (!ttysw_pty_output_ok(ttysw))
            return;
        /* convert wchar_t to char and call write() */
        save_char = *(iwbp.WC);
        *(iwbp.WC) = '\0';
        mb_nbytes = wcstombs_with_null(mb_irbp, irbp.WC, iwbp.WC - irbp.WC );
        *(iwbp.WC) = save_char;
        if( mb_nbytes == -1 ) {
...
{
    if (!multibyte)
        return(ttysw_pty_outputSB(ttysw, pty));
    else
        return(ttysw_pty_outputWC(ttysw, pty));
}
    ...
    register int    cc;
{
	...
	if (cmd_start > pty_index) {
		...
		textsw_remove_mark(textsw, termsw->read_only_mark);
	        ...
	        (void) textsw_delete_wcs(textsw, pty_index, cmd_start);
	    if (termsw->append_only_log) {
		termsw->read_only_mark =
		    textswAddMark(textsw, pty_index, TEXTSW_MARK_READ_ONLY);
	    }
    ...
    int				pty;
{
    ...
    unsigned char       mb_buf_p[TTYSW_BUFLEN];
    unsigned char       *a_mb_buf_p = mb_buf_p;
    unsigned char       mb_data[TTYSW_BUFLEN];
    static unsigned char rest_of_char[TTYSW_SMALLBUF];
    static int          rest_of_nchar = 0;
    int                 mb_buf_length = 0;


    if (multibyte) {
        if ((rest_of_nchar > 0)  && (rest_of_nchar <= TTYSW_BUFLEN)) {      
...
get_some_more:
    ...
    if (multibyte) {
        databuf.maxlen = ((oebp.WC - owbp.WC) - mb_buf_length) - 1;
        databuf.buf = (char *)mb_data;
    } else {
        databuf.maxlen = oebp.SB - owbp.SB;
        databuf.buf = owbp.SB;
    }
	...
	if (databuf.len > 0) {
            if (multibyte) {
                ...
                if ( (oebp.WC - owbp.WC) - mb_buf_length <= 4 )
                    goto process_mb_buf;
    ...
    process_mb_buf:
	        ...
	        ttysw_print_debug_string(owbp.SB, databuf.len);
#endif
    ...
    int             pty;
{
    ...
    unsigned char                   mb_buf[TTYSW_SMALLBUF];
    static unsigned char            rest_of_char[TTYSW_SMALLBUF];
    static int                      rest_of_nchar = 0;
    ...
    iov[0].iov_len = 1;
 /* jjd    iov[1].iov_base = owbp; */
    iov[1].iov_len = oebp - owbp;

    if (multibyte) {
        if (!(mb_buf_p = (unsigned char *)malloc
              ((iov[1].iov_len * sizeof(wchar_t)) + sizeof(rest_of_char)))) {
            perror(XV_MSG("TTYSW:ttysw_pty_input: out of memory"));
            return;
        }
        if (rest_of_nchar != 0)
        XV_BCOPY(rest_of_char, mb_buf_p, rest_of_nchar);
        iov[1].iov_base = (char *)mb_buf_p + rest_of_nchar;
    }   
...

{
    ...
    while (owbp.WC > orbp.WC &&!(ttysw->ttysw_flags&TTYSW_FL_FROZEN)){
	if (is_not_text) {
	...
	if ( ttysw->implicit_commit == 0 ) {
        ...
        if (!multibyte)
	    cc = ttyswOutputItSB(ttysw_view, orbp.SB, owbp.SB - orbp.SB);
        else
	    cc = ttyswOutputItWC(ttysw_view, orbp.WC, owbp.WC - orbp.WC);

	/*
	 * WHY HACK? There are synchronization problem with commited
	 * string with newer preedit string.  Since shelltool and/or
	 * cmdtool will not really knows exactly when commited string
	 * would be displayed onto the screen (not like textsw or
	 * panel, all commited string going throught the pty and
	 * either tty driver or application like vi's responsiblity to
	 * send back the echo to the pty, then ttysw can display).
	 * However, if the next preedit string starts before commited
	 * string actualy displayed, user will see the some flushing
	 * on the screen.  Therefor ttysw implements some hack to
	 * reduce the chance of seeing this screen flush.
	 */
	/*
	 * HOW WORKS? Basically, ttysw assumes all the commited string
	 * will be echoed somehow, and delay the display of the next
	 * preedit string until all the commited string came throught
	 * the pty.  The variable "committed_left" keep track how many
	 * commited characters remain out standing between the ttysw
	 * and pty.  In the following statement decrease the number in
	 * the "committed_left" with just recieved number of
	 * characters from pty.  ttysw will set the value for the
	 * "committed_left" when recieves commited string (in
	 * ttysw_process_keyboard() function).  The preedit callback
	 * routine will hold on the contents of the preedit string and
	 * not to display until "committed_left" become 0.
	 */
	/*
	 * PROBLEM: This hack is not 100% gurantee to work, since some
	 * of the application or tty setting may not echo the string.
	 * Or, first character in the long commited string may
	 * generate the some escape sequences to control the screen
	 * (cursor move and such).  Those escape sequences will
	 * confuses ttysw.  However, this is the best we can think of,
	 * and so far no known customer complained.
	 */
        if (committed_left > 0) {
            committed_left -= cc;
            if (committed_left <= 0) {
		/*
		 * FIX_ME: If there are preedit string need to be
		 * display which has been suspended by this
		 * "committed_left", we should display those preedit
		 * string now.
		 */
                committed_left = 0;
            }
    ...
    register int    len;
{
    int overflow;
    if (!multibyte) {
        if (iwbp.SB + len < iebp.SB) {
            (void) XV_BCOPY((char *)addr, iwbp.SB, len);
            iwbp.SB += len;
            return(len);
        }
    } else {        
        if (iwbp.WC + len < iebp.WC) {
            (void) XV_BCOPY((wchar_t *)addr, iwbp.WC, len * sizeof(wchar_t));
            iwbp.WC += len;
            return(len);
        }
    }
    ...
    register int    len;
{
    ...
    register int	i;
    char		*save_char;
    static char		rest_of_char[TTYSW_SMALLBUF];

    if (multibyte) {
        /* convert char to CHAR: allocate to worst case (ascii) */
        if (!(wcs_buf = (wchar_t *)malloc((len + 2)*sizeof(wchar_t)))) {
            perror(XV_MSG("TTYSW:ttysw_input_it: out of memory"));
            return 0;
        }

        if (!(save_char = (char *)malloc(len + 24))) {
            perror(XV_MSG("TTYSW:ttysw_input_it: out of memory"));
	    free(wcs_buf);
            return (0);
        }
	...
	int	bytes_copied;
        ...
        if (multibyte) {
	    bytes_copied = ttysw_copy_to_input_buffer
                                    (ttysw, wcs_buf, wc_nchar);
	    free(wcs_buf);
        } else {
    ...
    register int                len;
{
	...
	return (len);
        ...
        if (multibyte)
            ...
            char *SBbuffer = (char *)malloc(len);           
            (void) wcstombs(SBbuffer, addr, len);
            bytes_copied = ttysw_copy_to_input_buffer(ttysw,SBbuffer,len);
            free(SBbuffer);
        }
	if (bytes_copied > 0) {
	    /*
    ...
    register struct inputevent *ie;
{
	...
	switch (event_action(ie)) {
	  case KBD_USE:
		...
		window_set_ic_focus_win(ttysw_view_public, 
                                          ttysw->ic, xv_xid(info));
	    }
#endif
	...
	if (TTY_IS_TERMSW(ttysw)) {
	    Termsw_view_handle termsw = TERMSW_VIEW_PRIVATE_FROM_TTY_PRIVATE
                                                  (ttysw);
	...
	/*
	 * May need to resize the preedit region for instance if the
	 * input method is overTheSpot.
	 */
        if (ttysw->ic) {
            XRectangle	    x_rect;
    	    Rect	    *xv_rect;
...

{
    ...
    if (multibyte) {
        ...
        /*
	 * This is a committed string or a string generated from a
	 * non-English keyboard.  For example, a kana string from the
	 * Nihongo keyboard.  This string should only be displayed
	 * once on the down event.  While those committed string is
	 * process thought the pty, we will delay the display of the
	 * next preedit string to prevent possible screen flushs.
	 */
            wchar_t *ws_char;
...

{
    ...
    ws.ws_row = lines;
    ws.ws_col = cols;
    ws.ws_ypixel = row_to_y(lines);
    ws.ws_xpixel = col_to_x(cols);
...
int		as_input;
{
	...
	if ( conv_on != 0 ) {
	    ...
	    commit_text = (wchar_t *)xv_get(ttysw_public,
	     ...
	     * (shelltool => cmdtool).
	     */
	    if ( ( commit_text != (wchar_t *)NULL ) && as_input ) {
