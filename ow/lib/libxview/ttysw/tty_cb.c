#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)tty_cb.c 1.28 97/01/09";
#endif
#endif

/*
 *      (c) Copyright 1990-1993 Sun Microsystems, Inc. Sun design patents
...
XPointer 	callback_data;
{
    ...
    if( !folio->im_store )
    	folio->im_store = (CHAR *)xv_malloc( ITERM_BUFSIZE * sizeof(CHAR) );
    if( !folio->im_attr )
    	folio->im_attr  = (XIMFeedback *)xv_malloc( ITERM_BUFSIZE * sizeof( XIMFeedback) );
...
XIMPreeditDrawCallbackStruct 	*callback_data;
{
    ...
    if( callback_data->text == NULL && org_len == chg_length ) {
        ...
        /*
         * since we got the cursor sitting at the end position of
         * the edit-buffer, shift is back to where is supposed to
         * be for the repaint.  ESC# 500556
         */
        ttysw_pos(folio->im_first_col, folio->im_first_row);
#ifdef  DEBUG
	...
	if( !callback_data->text->encoding_is_wchar && len != 0 ) {
	    wcs_buf = (CHAR *)xv_calloc(len + 1, sizeof(CHAR));
	...
	if ( chg_length < len ) {		/* need a buffer */
		...
		new_text = (CHAR *)xv_calloc(new_len + 1, sizeof(CHAR));
		new_attr = (XIMFeedback *)xv_calloc(new_len + 1,
						sizeof(XIMFeedback));
	    } else {
		new_text = buf2_text;
		new_attr = buf2_attr;
	...
	if ( chg_length < len ) {
		...
		xv_free(folio->im_store);
		xv_free(folio->im_attr);
