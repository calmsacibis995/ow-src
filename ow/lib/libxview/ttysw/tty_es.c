#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)tty_es.c 20.23 93/07/26";
#endif
#endif
...
#define NO_LOCAL_ECHO(_termsw, _textsw, _private, _count) \
	...
	==  (multibyte ? textsw_find_mark_wc((_textsw), (_termsw)->pty_mark) \
                     : textsw_find_mark((_textsw), (_termsw)->pty_mark)) )))
    ...
    void      *buf;
{
    ...
    Termsw_view_handle termsw_view = TERMSW_VIEW_PRIVATE_FROM_TTY_PRIVATE
                                     (ttysw);
    Ttysw_view_handle ttysw_view = TTY_VIEW_PRIVATE_FROM_ANY_VIEW
                                   (TERMSW_VIEW_PUBLIC(termsw_view));
    register Termsw_folio folio = TERMSW_FOLIO_FROM_TERMSW_VIEW_HANDLE
                                  (termsw_view);
	...
	/* copy buf into iwbp */
        if (multibyte) {
            XV_BCOPY((wchar_t *)buf, iwbp.WC, MIN(count*sizeof(wchar_t), 
                        (iebp.WC - iwbp.WC)*sizeof(wchar_t)));
            iwbp.WC += MIN(count, iebp.WC - iwbp.WC);
        } else {
	    XV_BCOPY((char *)buf, iwbp.SB, MIN(count, iebp.SB - iwbp.SB));
	    iwbp.SB += MIN(count, iebp.SB - iwbp.SB);
        }
    ...
    Attr_attribute *attr_argv;
{
    ...
    register Ttysw_view_handle ttysw_view = TTY_VIEW_PRIVATE_FROM_ANY_VIEW
                                            (TERMSW_VIEW_PUBLIC(termsw_view));
    register Termsw_folio folio = TERMSW_FOLIO_FROM_TERMSW_VIEW_HANDLE
                                  (termsw_view);
	...
	if ((Es_attribute) * attrs == ES_HANDLE_TO_INSERT) {
		...
		/* Really should loop, in case esh > iebp-iwbp */
                if (!multibyte) {
		    (void)es_read(to_insert,iebp.SB-iwbp.SB,iwbp.SB,&result);
		    iwbp.SB += result;
                } else {
		    (void)es_read(to_insert,iebp.WC-iwbp.WC,iwbp.WC,&result);
		    iwbp.WC += result;
                }
		ttysw_reset_conditions(ttysw_view);
		ATTR_CONSUME(*attrs);
