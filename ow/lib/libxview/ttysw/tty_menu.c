#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)tty_menu.c 20.68 94/09/06";
#endif
#endif
    ...
    Menu_item       cmd_item;
{
    ...
    register        Termsw_folio
                    termsw_folio = TERMSW_FOLIO_FOR_VIEW
                             (TERMSW_VIEW_PRIVATE_FROM_TEXTSW(textsw));
...
Press \"Continue\" to proceed."),
    ...
    (void) win_getmouseposition(termsw_folio->textedit, 
    ...
    Menu_item       cmd_item;
{
	...
	tmp_index = (int) textswFindMark(textsw, termsw->pty_mark);
	insert = (Textsw_index) getTextswInsertionPoint(textsw);
	if (insert != tmp_index) {
	    setTextswInsertionPoint(textsw, tmp_index);
	}
