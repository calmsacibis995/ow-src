...
/*
**	Fix for bug 1254876. Change made to SCROLLBAR_VIEW_LENGTH is
**	over-ridden in the resize procedure if the window is made smaller.
**	Suggested solution:
**	First take into account any changes that the user may have made
**	to the scrollbar parameters by calling canvas_scroll()
*/
    OPENWIN_EACH_VIEW(canvas_public, view_window)
	sb = (Scrollbar)xv_get(canvas_public, OPENWIN_VERTICAL_SCROLLBAR, view_window);
	if (sb) canvas_scroll(xv_get(view_window, CANVAS_VIEW_PAINT_WINDOW, 0), sb);
	sb = (Scrollbar)xv_get(canvas_public, OPENWIN_HORIZONTAL_SCROLLBAR, view_window);
	if (sb) canvas_scroll(xv_get(view_window, CANVAS_VIEW_PAINT_WINDOW, 0), sb);
    OPENWIN_END_EACH
/*
**	End of fix	DPT 19-Nov-96
*/
