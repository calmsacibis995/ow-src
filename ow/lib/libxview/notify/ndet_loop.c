#ifndef	lint
#ifdef sccs
static char     sccsid[] = "@(#)ndet_loop.c 20.39 97/06/25 Copyr 1985 Sun Micro";
#endif
#endif
...
pkg_private_data struct sigaction ndet_sigvec =
	{SA_RESTART | SA_SIGINFO, {ndet_signal_catcher}, {0}, {0,0}};
	static int      ndet_signal_code;
	static ucontext_t *ndet_signal_context;
#endif SVR4

static int      ndet_update_itimer();
...
ndet_update_virtual_itimer()
{
     ...
     * was set to.
     */
    n = ndet_update_itimer(&enum_itimer);
    if (n != 0) {
		/*
		**	Probably linked with XIL :-(
		**	XIL uses unbound threads which fouls up virtual itimer. 
		**	What can we do? Well, we can either ignore it or set a real timer.
		**	I have absolutely no idea which would be best.
		**	Imagetool is the main culprit here, and it doesn't seem to suffer
		**	from the lack of timer. I *think* it's safe to ignore it.
		*/
	}
...
ndet_update_real_itimer()
{
    ...
    n = gettimeofday(&enum_itimer.current_tv,NULL); /* SYSTEM CALL */
...
#else
	old_handler(sig, ndet_signal_code, scp);
#endif
    ...
    NTFY_ENUM_DATA  context;
{
	...
	if (client->nclient == ndet_auto_nclient) {
	    /* Lot of processing here, preserve the global client. */
	    client_save = ntfy_enum_client;
	    tmp = ndet_auto_sig_send(client, condition, context);
	    ntfy_enum_client = client_save;
	    return(tmp);
	}
	else {
	    if (ndis_enqueue(client, condition) != NOTIFY_OK)
