#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)file_list.c 1.30 95/08/14";
#endif
#endif
     ...
     Attr_avlist avlist;
{
    ...
    row_height = font_height + (int)(2 * (.3 * font_height));
     ...
     char *directory;
{
    ...
    (void) strcpy(file_path, private->directory);
    xv_dirname( file_path );
    (void) xv_stat( file_path, &(current->stats) );
    (void) strcpy(file_path, "..");
     ...
     char *name;
{
    ...
    if ( !new ) {
	flist_error(private, XV_MSG("out of memory opening folder"));
	return FALSE;
    }
