#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)file_chsr.c 1.64 94/05/25";
#endif
#endif
...
/* running in mouseless */
static int		fc_in_full_mouseless = -1;
...
static Panel_setting	fc_document_txt_notify();
     ...
     Attr_avlist avlist;
{
    ...
    /*
     * Need to know if running in full or basic mouseless
     */
    if ( fc_in_full_mouseless == -1 ) {
	fc_in_full_mouseless 
	    = defaults_get_enum("openWindows.keyboardCommands",
     ...
     Fc_private *private;
{
     ...
     * comments in fc_event_interposer(). Do this before creating
     * panel items so all VISIBILITY_NOTIFIES are received by
     * fc_event_interposer().
     */
     ...
     Fc_private *private;
{
	...
	/*
	 * L&F Note: If running in Basic Mouseless, then <Return>
	 * behaves as DefaultAction.  In Full Mouseless, it behaves
	 * as usual.  Install a notify proc to do the DefaultAction
	 * thing in Basic Mouseless.
	 */
	if ( !fc_in_full_mouseless ) {
	    xv_set(private->ui.document_txt,
		   PANEL_NOTIFY_LEVEL,	PANEL_SPECIFIED,
		   PANEL_NOTIFY_STRING,	"\r",
		   PANEL_NOTIFY_PROC,	fc_document_txt_notify,
		   NULL);
	}
     ...
     Event *		event;
{
    ...
    pre_value = xv_strcpy(NULL, (char *)xv_get(item, PANEL_VALUE));

    if ( private->document_default_event )
	(* private->document_default_event)( item, event );

    post_value = (char *)xv_get(item, PANEL_VALUE);

    /* wipe out "Please enter a file name" message */
    if ( pre_value && post_value
	&& !strequal(pre_value, post_value)
	&& (xv_get(FC_PUBLIC(private), FRAME_LEFT_FOOTER) != NULL)
	)
	xv_set(FC_PUBLIC(private), FRAME_LEFT_FOOTER, "", NULL);
     ...
     */
    if ( !private->save_to_dir )
	fc_item_inactive(private->ui.save_btn, no_string(post_value));

    xv_free_ref( pre_value );
} /* fc_document_txt_event() */



/*
 * Save Typein PANEL_NOTIFY_PROC.  Emulate DefaultAction.
 */
static Panel_setting
fc_document_txt_notify( item, event )
     Panel_text_item item;
     Event *event;
{
    Fc_private *private = (Fc_private *)xv_get(item, XV_KEY_DATA, FC_KEY);
    ...
    fc_save_notify(private->ui.save_btn, event);
    xv_set(private->ui.save_btn, PANEL_BUSY, FALSE, NULL);

    /* if save succeeded, must unshow the chooser */
    if ( (int) xv_get(private->ui.save_btn, PANEL_NOTIFY_STATUS) == XV_OK )
	xv_set(FC_PUBLIC(private), XV_SHOW, FALSE, NULL);

    return panel_text_notify(item, event);
} /* fc_document_txt_notify() */
     ...
     File_list_row *row;
{
	...
	if ( !(notify_client && private->filter_func) )
	    status = (File_chooser_op) FILE_LIST_IGNORE;
    }
#endif
