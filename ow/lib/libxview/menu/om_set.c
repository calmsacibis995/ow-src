#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)om_set.c 20.102 94/03/29";
#endif
#endif
...
Pkg_private void menu_internal_cleanup();
	  ...
	  case MENU_APPEND_ITEM:{
 	  ...
 	  case MENU_FRAME_ADD:{
		...
		if (m->nframes + 1 > m->max_frames)  {
		    ...
		    /*
		     * Alloc new list - MENU_FRAMES_BLOCK_SIZE chunks bigger than
		     * the old one
		     */
		    new_list = (Frame *)xv_calloc(m->max_frames + MENU_FRAMES_BLOCK_SIZE,
					sizeof(Frame));
		     ...
		     */
		    m->max_frames += MENU_FRAMES_BLOCK_SIZE;
	      ...
	      default:{
		    ...
		    (void) sprintf(dummy,
			   XV_MSG("Invalid argument for attribute MENU_LINE_AFTER_ITEM: %d"),
				   (int) attrs[1]);
		    menu_error(NULL, dummy);
		}
	    }
	    break;

	  case MENU_CLASS:
	    menu_error(NULL, XV_MSG_CONST("MENU_CLASS attribute is get-only"));
	    break;
    ...
    register Attr_attribute *attrs;
{
	...
	switch (attrs[0]) {
	  ...
	  case MENU_PULLRIGHT:
	    /* Do necessary bookkeeping for submenu reference count. */
	    if (mi->pullright && !mi->gen_pullright && mi->value)
		xv_set (mi->value, XV_DECREMENT_REF_COUNT, NULL);
	    mi->value = (Xv_opaque) attrs[1];
	    if (mi->value) {
		MENU_PRIVATE(mi->value)->parent = mi;
	    	xv_set (mi->value, XV_INCREMENT_REF_COUNT, NULL);
	    }
    ...
    void            (*destroy_proc) ();
{
	...
	int j;
	for (j = 0; j < m->nframes; ++j)  {
		xv_set(m->frame_list[j],FRAME_MENU_DELETE,
			MENU_PUBLIC(m),NULL);
        }
	...
	for (; m->nitems-- > 0;) {
	    ...
	    /* If item is a pullright menu, decrement its reference counter. */
	    if (mi->pullright && !mi->gen_pullright && mi->value)
		xv_set (mi->value,
			XV_DECREMENT_REF_COUNT,
			NULL);
    ...
    register Xv_menu_info *m;
{
    ...
    if (panel && panel->item_destroy_flag == 1) panel->item_destroy_flag = 2;
	...
	int		set;
{
	...
	if (set)  {
		 ...
		 * accelerator binding was requested. If yes, (xv_strncasecmp 
