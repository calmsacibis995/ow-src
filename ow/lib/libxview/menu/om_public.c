#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)om_public.c 20.153 94/05/18";
#endif
#endif

/*
 *	(c) Copyright 1985-1993 Sun Microsystems, Inc. Sun design patents 
...
Xv_public void destroy_menu_and_submenus (Menu menu_public);
...
static void build_menu_tree_list (Menu menu_public, Menu **list, int *size);
...
#endif
{
    ...
    if (!menu_public) {
	menu_error(0, XV_MSG_CONST("menu_show: no menu specified"));
	return;
    }
	...
	menu_error(0,
		     XV_MSG_CONST("menu_show: menu not owned by this server"));
    ...
    /* ju */
    /* need to check for any menus that are already up and bring them down
       before Grabbing the pointer because menu_done() ungrabs the pointer. */
    if (!menu_active_menu_key)
	menu_active_menu_key = xv_unique_key();
    if (xv_get(server,XV_KEY_DATA,menu_active_menu_key)) {
	menu_internal_cleanup(xv_get(server,XV_KEY_DATA,menu_active_menu_key));
    }
    /* ju */
    ...
    if (XGrabPointer(display, xv_get(win, XV_XID),
		     ...
		     CurrentTime)	/* time */
	!=GrabSuccess) {
	menu_error(0, XV_MSG_CONST("menu_show: unable to grab pointer"));
	/* no menus are active */ /* ju */
    ...
    if (XGrabKeyboard(display, xv_get(win, XV_XID),
		     ...
		     CurrentTime)	/* time */
	!=GrabSuccess) {
	menu_error(0, XV_MSG_CONST("menu_show: unable to grab keyboard"));
...
#ifdef OW_I18N
    /*
     * Mouseless operation uses input from parent window (not within
     * the menu window itself), and there are known keystroke conflict
     * with IM today (control+space), therefor we will turn off the IM
     * temporary while MENU window is up.  The variable ic_was_active
     * will remembers parent windows original mode.
     */
	...
	if (group_info == NULL) {
	    menu_error(0,
		     XV_MSG_CONST("menu_show: unable to allocate group_info"));
	    XUngrabPointer(display, CurrentTime);
    ...
    Xv_opaque      *avlist;
{
	...
	menu_error(object,
	       XV_MSG_CONST("menu_create: unable to allocate menu structure"));
	return (int) XV_ERROR;
    }
    if (!m_cache) {
   ...
   /* Above XV_BCOPY() zaps all of m, so must set individual fields after it.*/
	...
	menu_error(object, XV_MSG_CONST("Unknown menu type"));
    }
    ...
    Xv_opaque      *avlist;
{
	...
	menu_error(object,
	       XV_MSG_CONST("Menu_create_item: unable to allocate menu_item"));
	return (int) XV_ERROR;
    }
    mi->color_index = -1;
    ...
    Attr_attribute  avlist[ATTR_STANDARD_SIZE];
{
	...
	if (m == NULL) {
	    menu_error(menu_public,
         XV_MSG_CONST("menu_find: menu's gen_proc failed to generate a menu"));
	    return NULL;
	}
    } else {
	m = m_base;
	...
	if (gen_item_proc = mi->gen_proc) {
	    ...
	    if (mi == NULL) {
		menu_error(menu_public,
XV_MSG_CONST("menu_find: menu item's gen_proc failed to generate a menu item"));
		goto exit;
	    }
	}
	for (attrs = avlist; *attrs; attrs = attr_next(attrs)) {
    ...
    Event          *event;
{
    ...
    if (m->pin_window) pl = (Panel) xv_get (m->pin_window, FRAME_CMD_PANEL);
    if (pl) panel = PANEL_PRIVATE (pl);
    if (panel && panel->item_destroy_flag != 2)
    ...
    Xv_menu_info   *menu;
{
    ...
    /* Initialize the # of rows and cols for the panel in case they've
       been fixed by the application. */
    num_rows = menu->nrows_fixed;
    num_cols = menu->ncols_fixed;

    /* The following layout algorithm is the same as in compute_dimensions()
       in om_render.c. This preserves the layout of the menu after it's been
       pinned. (BID 1056714). */
    /* If the menu has a title, don't include it in the layout calculations. */
    if (menu->item_list [0]->title)
	nitems = menu->nitems - 1;
    else
	nitems = menu->nitems;

    /* Check for fixed # of columns. */
    if (menu->ncols_fixed) {
	if (nitems < num_cols)
		num_cols = nitems;
	num_rows = ( (nitems - 1) / num_cols) + 1;
    }

    /* Check for fixed # of rows. */
    else if (menu->nrows_fixed) {
	if (nitems < num_rows)
		num_rows = nitems;
    }

    /* If the layout is not fixed, make it 1 column. */
    else
	num_rows = nitems;
	...
	if (choice_item)
		...
		mi->panel_item_handle = xv_create(panel, PANEL_BUTTON,
			  ...
			  PANEL_ITEM_COLOR, (mi->color_index > -1) ?
				mi->color_index : menu->color_index,
			  PANEL_LABEL_WIDTH, label_width,
			  PANEL_MENU_ITEM, TRUE,
			  PANEL_LABEL_FONT, (mi->image.font ? mi->image.font :
				menu->default_image.font),
	...
	if (menu->item_list[0]->title)
	    i--;
	xv_set (choice_item, PANEL_VALUE, i,
		PANEL_VALUE_FONT, (mi->image.font ? mi->image.font :
		menu->default_image.font),
		NULL);
    } else if (menu->class == MENU_TOGGLE) {
	i = 0;
	...
	xv_set(choice_item, PANEL_VALUE, toggle_value,
		PANEL_VALUE_FONT, (mi->image.font ? mi->image.font :
		menu->default_image.font),
		NULL);
    }
	...
	Event			*event;
{
	...
	if (!menu_private->group_info)  {
	     ...
	     */
	        ...
	        if (group_info == NULL) {
	            menu_error(0,
			       XV_MSG_CONST("Unable to allocate group_info"));
	        }
	        xv_set(server, XV_KEY_DATA, menu_group_info_key, group_info, 0);
	        group_info->server = server;
            }
...
menu_error(obj, msg)
    ...
    char	*msg;
{
    xv_error(obj, ERROR_STRING,	XV_MSG_VAR(msg),
		  ERROR_PKG,	MENU,
		  NULL);
}

Xv_public void
destroy_menu_and_submenus (Menu menu_public)
{
	Menu *menu_list = NULL;
	int menu_list_size = 0, j;

	build_menu_tree_list (menu_public, &menu_list, &menu_list_size);

	/* Destroy each menu in the menu tree. */
	for (j = 0; j < menu_list_size; j++)
		menu_destroy (menu_list [j] );
	free (menu_list);
	return;
}

/* Recursive routine for building a submenu list from a hierarchy. */
static void
build_menu_tree_list (Menu menu_public, Menu **list, int *size)
{
	Xv_menu_info *menu = MENU_PRIVATE (menu_public);
	Xv_menu_item_info *mi;
	int j;

	for (j = menu->nitems - 1; j >= 0; j--) {
		mi = menu->item_list [j];

		/* If item is a pullright, add it to the list. */	
		if (mi->pullright && !mi->gen_pullright && mi->value) {
			int found, k;

			xv_set (mi->value,
				XV_DECREMENT_REF_COUNT, NULL);

			/* Before recursively calling this routine, make 
			   sure the menu is not already in the list. Assuming
			   lists are never big enough to warrant anything but
	 		   a brute force search. */
			for (k = 0, found = 0; (k < *size && !found); k++)
				if (mi->value == (*list) [k] ) found = 1;
			if (!found)
				build_menu_tree_list (mi->value, list, size);
			mi->value = NULL;
			mi->pullright = FALSE;
		}
	}
	if (!*list) {
		if ( !(*list = (Menu *) malloc (sizeof (Menu) ) ) ) return;
		*size = 1;
	}
	else {
		Menu *temp_ptr;

		if ( !(temp_ptr = (Menu *) realloc (*list,
			(*size + 1) * sizeof (Menu) ) ) ) return;
		(*size)++;
		*list = temp_ptr;
	}
	(*list) [*size - 1] = menu_public;
	return;
}
