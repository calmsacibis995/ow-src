#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)om_render.c 20.179 93/11/14";
#endif
#endif
    ...
    Xv_menu_item_info *parent;
{
	...
	if (gen_menu == NULL) {
	    menu_error((Xv_opaque) menu,
       XV_MSG_CONST("menu_render: menu's gen_proc failed to generate a menu"));
	...
	if (!m->window) {
	    menu_error(0,
	         XV_MSG_CONST("menu_create: unable to allocate menu window"));
	...
	if (!m->shadow_window) {
            ...
            if (!m->shadow_window) {
                menu_error(0,
                XV_MSG_CONST("menu_create: unable to allocate shadow window"));
                cleanup(m, CLEANUP_ABORT);
                return;
            }
    ...
    check_cmc = m->group_info->depth <= 1 || (parent && parent->parent
	&& parent->parent->stay_up);
	...
	*row = m->nrows - 1;
	...
	if (*column > 0 && m->item_list[0]->title) {
	    ...
	    newitem -= *column;
    	    ...
    	    /* Fix for BID 1079729. Need to filter out entries on the first
	       row for the menu title case. */
    	    if (*row < 1) newitem = 0;
	}
	...
	/* Fix for BID 1079729. */
	if ( (m->item_list [0]->title) && (*row < 1)  && (*column > 0) )
		newitem = 0;
    }
    ...
    Event          *event;
{
	...
	if (event_is_iso(event)) {
	     ...
	     */
#ifdef OW_I18N
	    wchar_t	target_char[MB_LEN_MAX];  /*
						   * Need enough space
						   * for mbtowc(, ,
						   * MB_CUR_MAX)
						   */
		...
		if (mbtowc(target_char, event_string(event), MB_CUR_MAX) < 0)
		    target_char[0] = 0;
	    } else {
		char	mb;

		/*
		 * Have to take care the 8 bit character case.
		 */
		mb = event_action(event);
		if (mbtowc(target_char, &mb, 1) < 0)
		    target_char[0] = 0;
	    }
#endif
	    newitem = m->curitem; /* could be 0 */
    ...
    int		   call_gen_proc;
{
	...
	mi->parent = menu;
        ...
        /* ACC_XVIEW */
	/*
	 * Skip if title item, but std_image passed is not label
	 */
    ...
    if (menu->ncols_fixed || menu->nrows_fixed)
    {
               ...
               (rect->r_width > max_width))
        {
            if (menu->ncols_fixed)
            {
                ...
                {
                    menu_error(0, XV_MSG_CONST("Menu too large for screen"));
                    return FALSE;
                }
            }
            else
    ...
    int             stay_up;	/* TRUE or FALSE */
{
	...
	if (!gen_menu) {
	    menu_error((Xv_opaque) mi,
XV_MSG_CONST("Pullright Generate Procedure failed to generate a pullright menu"));
    ...
    Xv_Window       window;
{
    ...
    XCharStruct     	    text_overall_return = {0};
#endif /* OW_I18N */
    int			    width;
    int			    x;
    Drawable		    xid;
    ...
    Xv_Window       window;
{
	...
	if (!shadow_gc) {
	    menu_error(NULL,
        XV_MSG_CONST("menu_create: unable to create shadow Graphics Context"));
	    return;
	}
