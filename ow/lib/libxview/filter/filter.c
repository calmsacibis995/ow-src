#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)filter.c 20.28 97/05/21";
#endif
#endif
    ...
    char           *filename;	/* use this to label errors */
{
	...
	if (stream_get_sequence(in, scratch, digits) != NULL)
	    keynum = atoi(scratch);
	else if ((result = stream_get_token(in, scratch, breakProc)) &&
		 !strequal(result, ")")) {
	    (void) stream_get_sequence(in, scratch, digits);
	    keynum = atoi(scratch);
	    if (!(result = stream_get_token(in, scratch, breakProc)) ||
			!strequal(result, ")"))
		goto error;
	} else
	    goto error;
