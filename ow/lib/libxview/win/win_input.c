#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)win_input.c 20.227 95/08/11";
#endif
#endif
...
extern char    *xv_app_name_fullpath;
...
/* Defines keypad cursor movement keys. CRT 4168 (1109875). */
#define IsXViewCursorKey(ksym) (IsCursorKey (ksym) || ksym == XK_F27 || \
	ksym == XK_F29 || ksym == XK_F33 || ksym == XK_F35)
...
/*
 * the X protocol specifies that extension events will
 * fall within the range of [64..127].
 */
#define is_extension_event(xevent) \
	( ((xevent)->type > 63) && ((xevent)->type < 128) )
...

{
    ...
    XV_SL_TYPED_FOR_ALL(SERVER_PRIVATE(xv_default_server), server, Server_info *) 
...
ContProcess:
    ...
    switch (event_type) {
      ...
      case KeyRelease:{
	   ...
	   /* Modify the key mapping to use F1 for Help if there's no Help
	      key on the keypad and store the Help key being used. */
           if (help_ksym == NoSymbol) {
		if (!XKeysymToKeycode(display, XK_Help)) {
	    		if ( (XK_Help & KEYBOARD_KYSM_MASK) == KEYBOARD_KYSM)
				key_sem_map [ (XK_F1 & 0xff) ] = ACTION_HELP
					- XVIEW_SEMANTIC;
			else
				ascii_sem_map [ (XK_F1 & 0xff) ] = ACTION_HELP
					- XVIEW_SEMANTIC;
			help_ksym = XK_F1;
		}
		else
	...
	/* If NumLock is on, we do our own keysym lookup since the server
	   doesn't alter the keysym with the NumLock modifier (this will
	   allegedly be fixed in R6). This lets us properly interpret keypad
	   keys. If, however, the Shift key is also down, we interpret the
	   key as a regular cursor movement key if it's one of those keys. */
	if (ek->state & num_lock_modmask) {
	 	int   index, kpadkey = False, csrkey = False;
		int	keysyms_per_keycode;
		int 	min_keycode, max_keycode;
		KeySym *tmp_keysym_list, kpksym;

		XDisplayKeycodes( display, &min_keycode, &max_keycode );
		tmp_keysym_list = XGetKeyboardMapping( display, min_keycode,
		    max_keycode - min_keycode + 1, &keysyms_per_keycode );
		XFree(tmp_keysym_list);

		/* If it's a cursor movement key and Shift is on, don't
		   send as a keypad key.  CRT 4168 (1109875). */
		for (index = 0; index < keysyms_per_keycode; index++) {
		   	if ((ksym = XLookupKeysym(ek, index)) != NoSymbol) {
				if (IsKeypadKey(ksym)) {
					kpksym = ksym;
					kpadkey = True;
				}
				if (IsXViewCursorKey (ksym) && (ek->state
					& ShiftMask) ) csrkey = True;
			}
		}
		...
		if (kpadkey && !csrkey) {
	    		...
	    		(void)win_translate_KP_keysym(kpksym, buffer);
	    		buf_length = strlen(buffer);
	    		DoLookup = False;
			ksym = kpksym;	/* Reset ksym to correct KeySym */
		}
		else if (csrkey) {
	...
	/* Do a check for the case of the shift key pressed with one of the
	   keypad keys and override the server's interpretation of this being
	   a # keypad key so we can interpret it as a cursor movement key
	   for mouseless selection. */
	else if (ek->state & ShiftMask) {
		ksym = XKeycodeToKeysym(display, ek->keycode, 0);
		if ( (ksym == XK_Left) || (ksym == XK_Right) || (ksym == XK_Up)
			|| (ksym == XK_Down) || (ksym == XK_Home) || 
			(ksym == XK_R7) || (ksym == XK_End) || (ksym == XK_R13)
			|| (ksym == XK_R9) || (ksym == XK_R15) ) {
			buf_length = 0;
			DoLookup = FALSE;
		}
	}
	if (DoLookup) {
...
#endif
		 (ksym != help_ksym)) && !defeat_event_security) {
		goto Default;
	    }
	    ...
	    if (server->ctrl_alt_meta_key && (ek->state & ControlMask)
		&& (ek->state & alt_modmask) ) {
		ek->state &= ~ControlMask;
		ek->state &= ~alt_modmask;
		ek->state |= meta_modmask;
	    ...
	    /* Shift handling - use the shift modifier to un-NUMLOCK
	     * when NUMLOCK is on, execpt when trying to catch the
	     * Shift-Ctrl-Return accelerator.
	     * This is an inconsistency in our keymapping 
             * strategy that should be corrected by developing a
             * consistent OpenWindows keymapping strategy.
	     */
	    if (event_shift_is_down(event) && keyboard_key &&
		    (!(ek->state & num_lock_modmask) || ksym == XK_Return)){
		/* 
		...
		*pwindow = 0;
		...
		Xv_object       dummy_window = window, prev_win = window;

		while ( (window = xv_get(prev_win, WIN_PARENT) )
			&& (!( (WIN_PRIVATE (prev_win) )->top_level) ) ) {
...
Default:
	   ...
	   * the programmer.
	   *
	   * Since the drawable isn't necessarily valid for an extension event,
	   * make sure the xv_server is correct.  SERVER_DISPLAY_CONTEXT is the
	   * same for all display's, so the use of xv_default_server shouldn't
	   * fail in the multi-server case.
	   */
	  if ( is_extension_event(xevent) ) {
	    if ( XFindContext(display, (Window) display,
                         (XContext)xv_get(xv_default_server,
					  SERVER_DISPLAY_CONTEXT),
                         (caddr_t *)&server_public) == 0
		) {
		void (*extensionProc)()
		    = (void (*)()) xv_get(server_public, SERVER_EXTENSION_PROC);
    ...
    int		    appl_cmdline_argc;
{
    ...
    argv[argc++] = xv_app_name_fullpath;
	...
	do {
	    xwin = parent;
	    temp = XQueryTree(xv_display(info), xwin, &root, &parent,
			      &children, &nchildren);
	} while( parent != root );

	if (temp) {
	    XGetWindowAttributes(xv_display(info), xwin, &xwin_attr);
	    if (nchildren)
	      XFree((char *)children);
	}
    } else
