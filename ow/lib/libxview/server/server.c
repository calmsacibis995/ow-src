#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)server.c 20.164 97/05/21";
#endif
#endif
    ...
    Server_info	*server;
{
    ...
    strncpy(modifier_string, returned_string, 128);
    ...
    Attr_avlist     avlist;
{
	...
	/* Get the resources from the users .Xdefaults file */
        if (home = getenv("HOME"))
	    (void) strncpy(filename, home, MAXPATHLEN);
        else
	    filename[0] = NULL;
        (void) strncat(filename, "/.Xdefaults", MAXPATHLEN - strlen(filename));
    ...
    Server_info  *server;
{
	...
	if (i == OLLC_BASICLOCALE) {
	     ...
	     */
	    server_warning(XV_MSG("Could not obtain the Basic Locale "
    ...
    char	*character_set;
{
   ...
   /*
    * Handling xview apps brought up under non EUC encoding for
    * multibyte locales in CDE - code is not EUC specific. 
    */

    if (defaults_get_boolean("OpenWindows.NonEucLocaleWarning",
			     "OpenWindows.NonEucLocaleWarning",FALSE)) {
      char		*type;
      XrmValue		 xrm_value;
      if (XrmGetResource(server->db,"OpenWindows.NonEucLocaleWarningMessage",
			 "OpenWindows.NonEucLocaleWarningMessage",
			 &type,&xrm_value))
	server_warning(xrm_value.addr); 
      else server_warning("Please use a supported encoding");
    }
	 ...
	 */
	(void) sprintf(msg,
		       XV_MSG("Error when setting all locale categories"
			      " to \"%s\" (set via %s)"),
	...
	if ((oi != &server->ollc[OLLC_BASICLOCALE] && ! is_8859_1_locale
	  ...
	  || (is_c_locale && strcmp(oi->locale, "C") != 0)) {
	     ...
	     */
	    (void) sprintf(msg, XV_MSG("Can not use \"%s\" as locale category"
	...
	if (Ollc_const[i].posix >= 0
	 ...
	 && setlocale (Ollc_const[i].posix, oi->locale) == NULL) {
	     ...
	     */
	    (void) sprintf(msg,
			   XV_MSG("Error when setting locale category (%s) to "
				  "\"%s\" (set via %s"),
			   server_get_locale_name_str(i),
			   oi->locale,
			   server_get_locale_from_str(oi->from));
	...
	oi = &server->ollc[OLLC_BASICLOCALE];
	(void) sprintf(msg, 
		       XV_MSG("Supplied locale \"%s\" (set via %s) is not "
...
#ifdef OW_I18N
    if (!XSupportsLocale()) {
	(void) sprintf(msg, XV_MSG("Xlib does not support locale \"%s\" ("
				   "which is for non internationalized "
