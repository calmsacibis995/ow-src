#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)font.c 20.119 97/05/21";
#endif
#endif
...
struct font_return_attrs {
    ...
    char	delim_used;
#ifdef OW_I18N
    int		type;		/* Type of font. ie: FONT_TYPE_TEXT */
    char	*locale;	/* locale the font created in */
    char	**names;	/* name list of the font set */
    short	free_names;	/* Flag for free names field */
    char	*specifier;	/* Specifier for creating the font or fontset */
#endif /*OW_I18N*/ 
    char	*encoding;
    char	*registry;
...
#ifdef OW_I18N
static const Font_locale_info		C_locale = {
	"C",
	NULL,
	10, 
	12,
...
static Font_locale_info		*fs_locales = (Font_locale_info *)&C_locale;
#endif
...
#ifdef OW_I18N
	char	*current_defaults_locale = (char *)defaults_get_locale();
	
	/* 
	 * This is because defaults_get_locale() returns a pointer to
	 * the string which could be changed by defaults_set_locale().
	 */
	if (current_defaults_locale)
...
/*
 *  This function will be recursively called for 15 times or
 *  until the fontset definition is found in the fontset database 
 *  OpenWindows.fs. 
 */
static char *
...
int                     count;
{
    ...
    if (count > 15)
	return NULL;
	/* If the list has FS_DEF, the fontset definition is found */	    
    if (strncmp(list, FS_DEF, FS_DEF_LEN) == 0) {
	if ((key = strchr(list, ',')) != NULL) {
		key = skip_space(key + 1);
		return key;
	}
	/* If the list has FS_ALIAS, keep looking */	    
    ...
    Attr_avlist     avlist;
{
    ...
    if (my_attrs.type == FONT_TYPE_TEXT)  {
        /* load the font set */
        font_set = (XFontSet)xv_load_font_set(display,  my_attrs.locale,
		    my_attrs.names);
...
#ifdef OW_I18N
/*
 *  Extract the field specified by attr_pos from names.
    ...
    Destroy_status  status;
{
	...
	if (font->setwidthname)
	    xv_free(font->setwidthname);
	if (font->addstylename)
	    xv_free(font->addstylename);
...
#ifdef OW_I18N
/*
 *  font_name_is_equivalent()
 *  Compare the finfo->name with the info in my_attrs.
 *  This will do wild card matching also.
...
/*
 * font_find_font()
 * Search in font_list for an opened font that matches the values
 * specified in my_attrs.  If there is none, then open a new font.
 */
    ...
    struct font_return_attrs *attrs;
{
	...
	xv_free(attrs->foundry);
	attrs->free_foundry = 0;
    }
    if (attrs->free_family) {
	xv_free(attrs->family);
	attrs->free_family = 0;
    }
    if (attrs->free_style) {
	xv_free(attrs->style);
	...
	xv_free(attrs->setwidthname);
	attrs->free_setwidthname = 0;
    }
    if (attrs->free_addstylename) {
	xv_free(attrs->addstylename);
...
font_attrs_name:
    ...
    if (!font_attrs->names)  {
        xv_error(NULL,
            ERROR_STRING, XV_MSG("Cannot construct fontset name from given information."),
            ERROR_PKG, FONT,
            0);
        return (XV_ERROR);
    }
