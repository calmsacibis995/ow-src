#ifndef lint
#ifdef SCCS
static char     sccsid[] = "@(#)sel_util.c 1.29 93/11/05";
#endif
#endif
...
/*
 * This routine is exactly like seln_give_up_selection() except it gives up
 * the selection using the passed in time instead of the time the current
 * owner got ownership of the selection.  This is because that time might be
 * 0 which just happens to be CurrentTime It is not a good idea to set ownership
 * to None using CurrentTime.
 */

static void
seln_give_up_selection_using_event_time(server, rank, time)
    Xv_Server       server;
    Seln_rank       rank;
    Time            time;
{
	...
	{
	    XSetSelectionOwner(display, selection, None, time);
