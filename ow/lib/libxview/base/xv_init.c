...
Xv_private_data char *xv_app_name_fullpath;
Xv_private_data char **xv_app_name_argv;
Xv_private_data int    xv_app_name_argc;
char 			*getenv();
...
#endif
{
    ...
    (void) sprintf(xv_domain, "%s_%04d", XV_TEXT_DOMAIN,
	XV_BASE_VERSION_NUMBER);
	...
	switch ((Xv_attr) attrs[0]) {
	  case XV_INIT_ARGS:
	    ...
	    if (argv[0]) {
	      ...
	      xv_app_name_fullpath = strdup(argv[0]);
#ifdef OW_I18N
    ...
    /* save argv so that we can set it on WM_COMMAND for dt.session. */
    xv_app_name_argv = (char**)xv_malloc(sizeof(char**) * argc);
    xv_app_name_argc = argc;
    for(i=0; i < argc; i++)
	xv_app_name_argv[i] = strdup(argv[i]);
	...
	xv_app_name_fullpath = strdup("xview");
    }
...
#if defined(__ppc) /* bilan */
	notify_exclude_fd = ConnectionNumber((Display *)xv_get(server, XV_DISPLAY, 0));
#else
