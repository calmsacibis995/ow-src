#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)p_choice.c 20.152 97/04/11";
#endif
#endif
    ...
    Attr_avlist     avlist;
{
    ...
    panel_make_image(ip->value_font, &dp->choices[0],
                     PIT_STRING, panel_strsave_wc(NULL_STRING), 
		     FALSE, FALSE);
#else
    panel_make_image(ip->value_font, &dp->choices[0],
    ...
    register Attr_avlist avlist;
{
	...
	if (!re_alloc_choices(ip, choices_type, choices))
	    return (Xv_opaque) XV_ERROR;
	choices_changed = TRUE;
	if (temp_choices) xv_free (temp_choices);
    }
    ...
    Event          *event;
{
    ...
    if (panel) panel->item_destroy_flag = 1;
...
image_size(image, above_baseline, max_width, dp)
    register Panel_image *image;
    ...
    register Choice_info *dp;
{
	...
	sp = IMAGE_STRING_I18N(image);
        size = XV_PF_TEXTWIDTH(STRLEN(sp), font, sp);
	chrwth = xv_get(font, FONT_COLUMN_WIDTH_I18N);
	...
	{
#ifdef OW_I18N
            if ( wslen(sp) || ( !wslen(sp)

#else
            if ( strlen(sp) || ( !strlen(sp)
#endif /* OW_I18N */
                && (dp->display_level != PANEL_CURRENT)
                && (dp->display_level != PANEL_NONE)
                && (dp->feedback != PANEL_MARKED) ) )
    ...
    register Item_info *ip;	/* the item */
{
	...
	for (i = 0; i <= dp->last; i++) {
	    ...
	    switch (image->im_type) {
	      case PIT_STRING:
		size = XV_PF_TEXTWIDTH(STRLEN(IMAGE_STRING_I18N(image)),
			    image_font(image), IMAGE_STRING_I18N(image));
	...
	for (j = 0;
	    ...
	    size = image_size(image, &above_baseline, max_stringwidth,dp);
    ...
    int             selected;
{
	...
	if (dp->feedback == PANEL_MARKED ||
	    ...
	    PANEL_EACH_PAINT_WINDOW(panel, pw)
		DRAWABLE_INFO_MACRO(pw, info);
		panel_paint_text(pw, ip->value_font_id, color_index,
				 left, top, IMAGE_STRING_I18N(image));
	    PANEL_END_EACH_PAINT_WINDOW
	}
	break;
