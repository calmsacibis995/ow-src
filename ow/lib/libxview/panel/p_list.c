#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)p_list.c 1.158 97/02/10"; 
#endif
#endif
...
#ifdef OW_I18N
    dp->font_set = ip->panel->std_font_id;
    dp->stored_length_wc = FALSE;

    /* Turn off active IM conversion status. */
    ip->flags &= ~IC_ACTIVE;
#else
	 ...
	 */
	  ...
	  case PANEL_LIST_ROW_VALUES: {
		  ...
		  if (!dp->insert_duplicate &&
	          ...
	          if (which_attr == PANEL_LIST_ROW_VALUES) {
		      /*
		       * _xv_set_wcs_attr_dup() will make another wide-char
		       * copy of the string.  since this attribute is here
		       * only for performance, work around this extra dup
		       * by manually setting the pointers.  this should
		       * probably be moved into an _xv_*_attr_dup() call...
		       */
		      node->string.pswcs.storage = pswcs.value;
		      node->string.pswcs.value = node->string.pswcs.storage;
		      node->string.pswcs.length = pswcs.length;
		      node->string.psmbs.value = NULL;
		      pswcs.value = NULL;
		      pswcs.storage = NULL;
		      pswcs.length = NULL;
		  } else
		   ...
		   */
		  if (entry->glyph)
		      set_row_glyph(dp, node, (Pixrect *) entry->glyph, &glyph_width_changed);
		  else {
		      if ( node->glyph )
		   ...
		   */
		  node->f.inactive = entry->inactive;
		  if ( node->f.inactive )
		      node->f.selected = FALSE;
		  
		  /*
		   * do PANEL_LIST_SELECT stuff
		   */
		  if (!node->f.inactive) {
...
pls:
	  ...
	  case PANEL_LIST_GLYPHS:
		...
		while (avlist[i+1]) {
		    node = next_row(dp, node, i++);
		    set_row_glyph(dp, node, (Pixrect *) avlist[i], &glyph_width_changed);
		}
		break;
	  ...
	  case PANEL_LIST_GLYPH:
		...
		node = find_or_create_nth_row(dp, which_row, TRUE);
		if (avlist[2])
		    set_row_glyph(dp, node, (Pixrect *) avlist[2], &glyph_width_changed);
		else {
		    if ( node->glyph )
			glyph_width_changed = TRUE;
		    node->glyph = NULL;
	  ...
	  case PANEL_LIST_INSERT:
		if (!dp->insert_delete_enabled) {
		    panel_error(panel_list_public,
		 XV_MSG_CONST("PANEL_LIST insertions are currently disabled"));
		    break;
		}
		which_row = (int) avlist[1];
		(void) panel_list_insert_row(dp, which_row, SHOW_ROW,
					     DO_NOT_REPAINT_LIST);
		break;
	  ...
	  case PANEL_LIST_INSERT_STRINGS_WCS:
		...
		for (obj_ptr = (Xv_opaque *) avlist[2]; *obj_ptr; obj_ptr++) {
		    ...
		    if (insert_glyphs)
			set_row_glyph(dp, node, (Pixrect *) *obj_ptr, &glyph_width_changed);
		    else {
#ifdef OW_I18N
			_xv_set_wcs_attr_dup(&node->string, pswcs.value);
#else
		...
		dp->nrows = (unsigned short) which_row;
		if (dp->list_sb)
		    xv_set(dp->list_sb,
	  ...
	  case PANEL_LIST_DELETE:
		if (!dp->insert_delete_enabled) {
		    panel_error(panel_list_public,
	          XV_MSG_CONST("PANEL_LIST deletions are currently disabled"));
		    break;
		}
		which_row = (int) avlist[1];
		node = dp->rows;
	  ...
	  case PANEL_LIST_INACTIVE:
		...
		node = find_or_create_nth_row(dp, which_row, FALSE);
		if (node) {
		    node->f.inactive = (unsigned int) avlist[2];
		    if ( node->f.inactive )
			node->f.selected = FALSE;
		}
		break;
	  ...
	  case PANEL_LIST_SORT:
	    ...
	    /* Get the # of elements in the linked list. */
	    for (count = 1, node = dp->rows; node && node->next;
		node = node->next, count++);
	    sort_array = (Row_info *) malloc (count * sizeof (Row_info) );

	    /* If there's memory to do a quick sort, do it. */
	    if (sort_array) {
		for (node = dp->rows, j = 0; j < count; j++,
			node = node->next) sort_array [j] = *node;

		if (forward)
			cmp_fcn = cmp_fwd;
		else
			cmp_fcn = cmp_bwd;
		qsort (sort_array, count, sizeof (Row_info), cmp_fcn);
		for (node = dp->rows, j = 0; j < count;
			j++, node = node->next) {
			node->client_data = sort_array [j].client_data;
			node->display_str_len =
				sort_array [j].display_str_len;
			node->font = sort_array [j].font;
			node->glyph = sort_array [j].glyph;
			node->string = sort_array [j].string;
			node->mask_glyph =
				sort_array [j].mask_glyph;
			node->click_time =
				sort_array [j].click_time;
			node->exten_data =
				sort_array [j].exten_data;
			node->f = sort_array [j].f;
		}
		free (sort_array);
	    }

	    /* Use bubble sort if memory not available for quick sort. */
	    else {
	    	do {
			sort_count = 0;
			for (node = dp->rows; node && node->next;
				...
				node->mask_glyph = next->mask_glyph;
				node->click_time = next->click_time;
				node->exten_data = next->exten_data;
				node->f = next->f;
				...
				next->mask_glyph = row_tmp.mask_glyph;
				next->click_time = row_tmp.click_time;
				next->exten_data = row_tmp.exten_data;
				next->f = row_tmp.f;
		...
		/* Having sorted the list, current_row should refer */
		/* to the first selected item. */
		/* Otherwise, on an exclusive list, the item */
		/* does not get deselected when another is selected.  */
		/* Bug ID 1266805 */
		for (node = dp->rows; node; node = node->next) {
			if (!node->f.inactive && node->f.selected) {
				dp->current_row = node;
			}
		}
    ...
    if (!dp->choose_none && dp->rows && !dp->setting_current_row) {
        ...
        for (node = dp->rows; node; node = node->next)
            if (node->f.selected && !node->f.inactive)
            	break;
        if (!node) {
	    for (node=dp->rows; node; node = node->next)
		if ( !node->f.inactive ) {
    ...
    /*
     * a glyph changed (at least in width), find the widest glyph
     * which gets used for text offset in paint_row().
     */
    if ( glyph_width_changed ) {
	dp->max_glyph_width = 0;
	for (node = dp->rows; node; node = node->next)
	    if (node->glyph) {
		if ( node->glyph->pr_width > (int)dp->max_glyph_width )
		    dp->max_glyph_width = node->glyph->pr_width;
	    }
    }


    /*
     * Expand-to-fit mode, make sure string_x is calcuated
     * before compute_dimensions() gets called.
     */
    if ( dp->width == 0 )
	dp->string_x = LIST_BOX_BORDER_WIDTH + ROW_MARGIN 
	    + PANEL_LIST_COL_GAP + dp->max_glyph_width;
    ...
    va_list         valist;
{
	...
	row = va_arg(valist, int);
	node = find_or_create_nth_row(dp, row, FALSE);
	return (node ? (Xv_opaque) node->f.inactive : (Xv_opaque) XV_ERROR);
	...
	for(ii=0; ii<count; ++ii) {
	    ...
	    entry->inactive = node->f.inactive;
    ...
    Row_info	*edit_row;
{
	...
	/*  ESC# 503482; BID# 1227357
	 *  Make sure string length of changed list item is updated, before 
	 *  painting it.
	 */
...
#endif /* OW_I18N */
    row->f.show = TRUE;
	/* This is re-introduced for two bug fixes, not just one - 1227357 */
    ...
    Event	*event;
{
	...
	/*  ESC# 503482; BID# 1227357
	 *  In the case of multiple change options, make sure accept_change() 
	 *  is called only once.  This can happen when both read_only mode is
	 *  changed and list entry is modified at the same time.
	 */
    ...
    Panel_list_info *dp;
{
    ...
    if (dp->TITLE) {
#ifdef OW_I18N
	font_set = ip->panel->bold_font_id;
#else
    ...
    Menu_item menu_item;	/* not used */
{
...
#ifdef OW_I18N
    /*
     * Characters have different widths.  Display_length is actually
     * described in how many columns, and not how many characters.
     * Text_width is in pixels. So in order to compare display_length
     * and stored_length they both have to be converted to pixels.  I
     * am assuming one byte still occupies one column, so if the user
     * specified PANEL_VALUE_STORED_LENGTH, the maximum storage in
     * pixels would be (n bytes * default column width).  This is a
     * reasonable assumption in EUC world, but would be completely
     * broken for Unicode.
     */
    {
	...
	if (dp->stored_length_wc == FALSE) {
	    stored_length = (int) xv_get(dp->text_item,
	...
	/*
	 * Need to limit the display length to be less than or equal
	 * to the stored length.  So calculate everything in pixels.
	 */
    ...
    Menu_item menu_item;	/* not used */
{
...
#ifdef OW_I18N
            /*  Found during fix of ESC# 503482; BID# 1227357
             *  Bug Alert: This handling for notify_proc is wrong.  As
             *  written, it will always set notify_proc to NULL.  It should
             *  set notify_proc to either panel_list_ip->notify_wc or 
             *  panel_list_ip->notify, but doing so will result in a 
             *  recuresion loop with the testcase provided by BID# 1227357.
             *
             *  Not calling the notify_proc here is not too bad, because
             *  the XView programming manual states that the notify roputine
             *  will only be called for select, de-select, add or delete
             *  operations on list items (this is a mode change to read_only).
             */
#ifdef NOTDEF
  	    if (wchar_notify(panel_list_ip))
			notify_proc = panel_list_ip->notify_wc;
	 ...
	 */
        for (row = dp->rows; row; row = row->next)
            if (row->f.selected && !row->f.inactive)
            	break;
        if (!row) {
	    for (row = dp->rows; row; row = row->next)
		if ( !row->f.inactive ) {
    ...
    Row_info *row;
{
    ...
    /*
     * Could attempt to flag when this field *has* to be recalc'd
     * but it does not appear to be that expensive of an operation,
     * since usually the strings will fit in the displayable area 
     * the of list (i.e. one call to XTextWidth/XwcTextEscapement).
     */
    if ( dp->width != 0 )
	set_row_display_str_length(dp, row);
    ...
    Panel_list_info *dp;
{
	...
	DRAWABLE_INFO_MACRO(pw, info);
	panel_paint_text(pw, panel->bold_font_id, 
		ip->color_index, x, y + 
    ...
    Row_info       *row;
{
    ...
    int             average_width;
    int             width;
    int             curr_delta, prev_delta;
    int             prev_display_str_len;
    ...
    /*
     * if empty string, return otherwise 'divide by 0' error will
     * occur in calculations that follow
     */  
    if (!row->display_str_len)
        return;
...
#ifdef OW_I18N
    width = XwcTextEscapement(font_set, row->STRING, row->display_str_len);
#else
    width = XTextWidth(font_struct, row->string, row->display_str_len);
#endif
 
    /*
     * if entire string width is less than max string width, return
     */
    ...
    /*
     * string width is greater than max string width;
     *
     * find the average width of each character in this
     * string, then determine the number of characters that
     * can be displayed (this may be more or less than
     * the actual number of characters that can be displayed,
     * due to variable width fonts). Loop until the delta
     * between the string length and the max_string_width
     * no longer decreases.
     */
    average_width = width / row->display_str_len;
       
    /*                                            
     * upon entry to this loop, width, max_string_width and
     * row->display_str_len must be set to max values
     */
    do
    {  
        prev_delta = abs( max_string_width - width );
        prev_display_str_len = row->display_str_len;

        /*                                           
         * given delta between string width and max_string_width,
         * roughly calculate how many characters of average width
         * would occupy this space, then add it (or subtract)
         * to the current display_str_len then call XTextWidth
         * to find the real string width
         */
        row->display_str_len += (max_string_width - width) / average_width;
#ifdef OW_I18N
        width = XwcTextEscapement(font_set, row->STRING,
                                  row->display_str_len);
#else
        width = XTextWidth(font_struct, row->string, row->display_str_len);
#endif
        curr_delta = abs( max_string_width - width );
    }



    /*
     * loop until curr_delta is zero (indicating we've found
     * just the right number of characters who's width equals
     * max_string_width or
     *
     * curr_delta >= prev_delta (indicating that previous
     * iteration was actually closer) or
     *
     * or curr_delta is somewhere within a character or two
     * (which means we're pretty damn close so we should
     * probably drop out of the loop
     */
    while ((curr_delta < prev_delta) && curr_delta &&
           (curr_delta > average_width*2));
                                            
 

 
    /*
     * if current string width doesn't equal max_string width
     * exactly, continue...
     */
    ...
    {  
        if (prev_delta < curr_delta)
            row->display_str_len = prev_display_str_len;

        /*                                               
         * if it's less than the maximum width, add a character or
two.
         */
#ifdef OW_I18N
        ...
        /*
         * if we went over the maximum, then delete a char or two
         */
#ifdef OW_I18N
        while (XwcTextEscapement(font_set, row->STRING,
row->display_str_len) >
            max_string_width) {
#else
        while (XTextWidth(font_struct, row->string, row->display_str_len) >
    ...
    Xv_Font	    font;
{
	...
	panel_error(font,
		 XV_MSG_CONST("Font height exceeds row height; font ignored"));
	row->font = NULL;
    }
...
static void
set_row_glyph(dp, row, glyph_pr, changed)
     Panel_list_info *dp;
     Row_info *row;
     Pixrect *glyph_pr;
     int *changed;
{
	...
	if ( !row->glyph || (glyph_pr->pr_width != row->glyph->pr_width) ) {
	    *changed = TRUE;
	}
	row->glyph = glyph_pr;
    } else {
	panel_error((Xv_opaque)glyph_pr,
    XV_MSG_CONST("Panel List glyph height exceeds row height; glyph ignored"));
	if ( row->glyph )
	    *changed = TRUE;
	row->glyph = NULL;
    }
    ...
    Pixrect *glyph_pr;
{
	...
	panel_error((Xv_opaque)glyph_pr,
  XV_MSG_CONST("Panel List mask glyph depth not equal 1; mask glyph ignored"));
	row->mask_glyph = NULL;
	return;
    }
	...
	panel_error((Xv_opaque)glyph_pr,
XV_MSG_CONST("Panel List mask glyph height exceeds row height; glyph ignored"));
	row->mask_glyph = NULL;
    }
}
...
int cmp_fwd (Row_info* e1, Row_info* e2)
{
#ifdef OW_I18N
	return (wscoll (e1->string.pswcs.value, e2->string.pswcs.value) );
#else
	return (strcmp (e1->string, e2->string) );
#endif
}

int cmp_bwd (Row_info* e1, Row_info* e2)
{
#ifdef OW_I18N
	return (wscoll (e2->string.pswcs.value, e1->string.pswcs.value) );
#else
	return (strcmp (e2->string, e1->string) );
#endif
}
