#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)p_set.c 20.97 93/07/01";
#endif
#endif
    ...
    register Panel_info *panel;
{
...
#ifdef OW_I18N
    panel->std_font_id = (XFontSet) 
	xv_get(panel->std_font, FONT_SET_ID);
#else
    panel->std_font_id = (Font) xv_get(panel->std_font, XV_XID);
#endif /* OW_I18N */
    ...
    if (save_bold_name = bold_name = xv_font_bold())
    {
        ...
        if (panel->bold_font == NULL)
            panel_error(NULL, XV_MSG_CONST("Unable to find bold font"));
        if (bold_name)
            xv_free( bold_name );
    }
    ...
    if (panel->bold_font == NULL) {
        panel_error(NULL,
	        XV_MSG_CONST("Unable to find bold font; using standard font"));
        panel->bold_font = panel->std_font;
    }  
#ifdef OW_I18N
    panel->bold_font_id = (XFontSet) 
	xv_get(panel->bold_font, FONT_SET_ID);
#else
    panel->bold_font_id = (Font) xv_get(panel->bold_font, XV_XID);
#endif /* OW_I18N */
    ...
    int             arg;
{
	...
	xv_set(ITEM_PUBLIC(ip),
	       ...
	       PANEL_LABEL_STRING_WCS, IMAGE_STRING_I18N(label),
	       0);
#else
	xv_set(ITEM_PUBLIC(ip),
	       PANEL_PAINT, PANEL_NONE,
	       PANEL_LABEL_FONT, panel_font,
	       PANEL_LABEL_STRING, IMAGE_STRING_I18N(label),
	       0);
#endif /* OW_I18N */
