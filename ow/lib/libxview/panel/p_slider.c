#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)p_slider.c 20.88 94/09/23 Copyr 1984 Sun Micro";
#endif
#endif
...
typedef struct {	/* data for a slider */
    ...
    Panel_item      value_textitem;	/* for displaying value as a text
					 * item */
    int             width;
    char	   *min_tick_string_mb; /* Multibyte copy of min_tick_string */
    char	   *min_value_string_mb;/* Multibyte copy of min_value_string */
    char	   *max_tick_string_mb;/* Multibyte copy of max_tick_string */
    char	   *max_value_string_mb;/* Multibyte copy of max_value_string */
} Slider_info;
    ...
    register Attr_avlist avlist;
{
	...
	switch (attr) {
	  ...
	  case PANEL_MIN_TICK_STRING:
	    ...
	    if (dp->min_tick_string_mb)
		xv_free (dp->min_tick_string_mb);
	    if (*avlist) {
		dp->min_tick_string = 
		    (wchar_t *) _xv_mbstowcsdup((char *) *avlist);
		dp->min_tick_string_mb = panel_strsave ( (char *) *avlist);
	    } else {
		dp->min_tick_string = NULL;
		dp->min_tick_string_mb = NULL;
	    }
	    size_changed = TRUE;
	    avlist++;
	    break;
	  ...
	  case PANEL_MIN_TICK_STRING_WCS:
		...
		/* Save a multibyte copy of the string for multibyte API
		    compatibility. */
		mb_size = MB_CUR_MAX * wslen ( (wchar_t *) *avlist) + 1;
		mbstr = (char *) malloc (sizeof (char) * mb_size);
		if (mbstr && (wcstombs (mbstr, (wchar_t *) *avlist, mb_size)
			!= (size_t) -1) )
			dp->min_tick_string_mb = panel_strsave (mbstr);
		else
			dp->min_tick_string_mb = NULL;
		if (mbstr) free (mbstr);
	    } else {
	  ...
	  case PANEL_MIN_VALUE_STRING:
	    ...
	    if (dp->min_value_string_mb)
		xv_free (dp->min_value_string_mb);
	    if (*avlist) {
		dp->min_value_string = 
		    (wchar_t *) _xv_mbstowcsdup((char *) *avlist);
		dp->min_value_string_mb = panel_strsave ( (char *) *avlist);
	    } else {
		dp->min_value_string = NULL;
		dp->min_value_string_mb = NULL;
	    }
	    size_changed = TRUE;
	    avlist++;
	    break;
	  ...
	  case PANEL_MIN_VALUE_STRING_WCS:
		...
		if (mbstr && (wcstombs (mbstr, (wchar_t *) *avlist, mb_size)
			!= (size_t) -1) )
			dp->min_value_string_mb = panel_strsave (mbstr);
		else
			dp->min_value_string_mb = NULL;
		if (mbstr) free (mbstr);
	    } else {
	  ...
	  case PANEL_MAX_TICK_STRING:
	    ...
	    if (dp->max_tick_string_mb)
		xv_free (dp->max_tick_string_mb);
	    if (*avlist) {
		dp->max_tick_string = 
		    (wchar_t *) _xv_mbstowcsdup((char *) *avlist);
		dp->max_tick_string_mb = panel_strsave ( (char *) *avlist);
	    } else {
		dp->max_tick_string = NULL;
		dp->max_tick_string_mb = NULL;
	    }
	    size_changed = TRUE;
	    avlist++;
	    break;
	  ...
	  case PANEL_MAX_TICK_STRING_WCS:
		...
		if (mbstr && (wcstombs (mbstr, (wchar_t *) *avlist, mb_size)
			!= (size_t) -1) )
			dp->max_tick_string_mb = panel_strsave (mbstr);	
		else
			dp->max_tick_string_mb = NULL;
		if (mbstr) free (mbstr);
	    } else {
	  ...
	  case PANEL_MAX_VALUE_STRING:
	    ...
	    if (dp->max_value_string_mb)
		xv_free (dp->max_value_string_mb);
	    if (*avlist) {
		dp->max_value_string = 
		    (wchar_t *) _xv_mbstowcsdup ((char *) *avlist);
		dp->max_value_string_mb = panel_strsave ( (char *) *avlist);
	    } else {
		dp->max_value_string = NULL;
		dp->max_value_string_mb = NULL;
	    }
	    size_changed = TRUE;
	    avlist++;
	    break;
	  ...
	  case PANEL_MAX_VALUE_STRING_WCS:
		...
		if (mbstr && (wcstombs (mbstr, (wchar_t *) *avlist, mb_size)
			!= (size_t) -1) )
			dp->max_value_string_mb = panel_strsave (mbstr);
		else
			dp->max_value_string_mb = NULL;
		if (mbstr) free (mbstr);
	    } else {
	...
	if (dp->flags & SHOWVALUE) {
	    ...
	    /*   
                BUGid 1172611 and ESC 9952: to fix panel slider for
                PANEL_VALUE_DISPLAY_LENGTH < 4.  In this case
                we use the PTEXT_IS_NUMTEXT bit in the flag because
                we know this bit is only used in here and in p_txt.c
                and we know that this field takes numberic input.
                Just to clearify, text_dp is a text item not num_text
                item eventhough we set PTEXT_IS_NUMTEXT.
            */
            text_dp = TEXT_PRIVATE(dp->value_textitem);
            text_dp->flags |= PTEXT_IS_NUMTEXT;
            xv_set(dp->value_textitem,
                        PANEL_VALUE_DISPLAY_LENGTH, dp->valuesize,
                        NULL); 
...
#ifdef OW_I18N
      case PANEL_MIN_TICK_STRING:
	return (Xv_opaque) dp->min_tick_string_mb;
...
#ifdef OW_I18N
      case PANEL_MIN_VALUE_STRING:
	return (Xv_opaque) dp->min_value_string_mb;
...
#ifdef OW_I18N
      case PANEL_MAX_TICK_STRING:
	return (Xv_opaque) dp->max_tick_string_mb;
...
#ifdef OW_I18N
      case PANEL_MAX_VALUE_STRING:
	return (Xv_opaque) dp->max_value_string_mb;
