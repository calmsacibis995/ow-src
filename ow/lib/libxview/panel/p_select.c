#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)p_select.c 20.82 94/10/18";
#endif
#endif
    ...
    Notify_arg      arg;
{
	...
	switch (event_action(event)) {
	  ...
	  case ACTION_ADJUST:
	  case LOC_DRAG:
	    /* for selecting text and moving out of text item, events should
	       still be sent to text item. If selecting text then 
	       TEXT_HIGHLIGHTED flag is set */
	    if (panel->current && 
		((panel->current->item_type == PANEL_TEXT_ITEM)||
		 (panel->current->item_type == PANEL_NUMERIC_TEXT_ITEM)) &&
		(panel->current->flags & TEXT_HIGHLIGHTED)) {
		new = panel->current;
	    } else {
		new = panel_find_item(panel,event);
	    }
	    break;
	  case ACTION_SELECT:
	    if (event_is_up(event) && panel->current &&
		((panel->current->item_type == PANEL_TEXT_ITEM)||
		 (panel->current->item_type == PANEL_NUMERIC_TEXT_ITEM)) &&
		(panel->current->flags & TEXT_HIGHLIGHTED)) {
		new = panel->current;
