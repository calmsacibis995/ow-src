/*	@(#)panel_impl.h 20.93 94/03/17 SMI	*/
...
typedef struct panel_info {
...
#ifdef OW_I18N
    /*
     * OW_I18N case will NOT be using Selection_item to reduce the
     * number of copy/conversion have to take.  OW_I18N need to
     * support the CTEXT and STRING (single byte transfer), but we
     * will not know which data type to transfer until actual
     * selection operation began.  Selectoin_item forth to have
     * pre-converted text, therefor we will not use Selectoin_item and
     * do the converion in convert_proc instead.
     */
    ...
    int			item_destroy_flag; /* indicates OK to destroy items. */
} Panel_info;
