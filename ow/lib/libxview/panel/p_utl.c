#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)p_utl.c 20.110 94/08/25";
#endif
#endif
...
panel_col_to_x(font, col)
    ...
    int             col;
{
    int		chrwth = xv_get(font, FONT_COLUMN_WIDTH_I18N);
    return (col * chrwth);
}
    ...
    int             inverted_desired;
{
    ...
    if (is_string(dest)) {
        if (IMAGE_STRING_I18N(dest))
            xv_free(IMAGE_STRING_I18N(dest));
    }
...
#ifdef OW_I18N
	    value_str = panel_strsave_wc(NULL_STRING);

	/* Keep a copy of the multibyte string for multibyte API
	   compatibility. */
	{
		char *mbstr;
		int mb_size;
		...
		if (image_string (dest) ) xv_free (image_string (dest) );
		mb_size = MB_CUR_MAX * wslen (value_str) + 1;
		mbstr = (char *) malloc (sizeof (char) * mb_size);
		if (mbstr && (wcstombs (mbstr, value_str, mb_size)
			!= (size_t) -1) )
			image_set_string (dest, panel_strsave (mbstr) );
		else
			image_set_string (dest, (char *)
				panel_strsave (NULL_STRING) );
		if (mbstr) free (mbstr);
	}
...
#endif /* OW_I18N */
	    return (size);
	IMAGE_SET_STRING_I18N(dest, str);
	...
	if (!value || PR_NOT_SERVER_IMAGE(value))
	    panel_error(value, XV_MSG_CONST("Invalid Server Image specified"));
...
#ifdef OW_I18N
	    if (image_font(image))
		font_id = (XFontSet) xv_get(image_font(image), FONT_SET_ID);
#else
	    if (image_font(image))
		font_id = (XID) xv_get(image_font(image), XV_XID);
#endif /* OW_I18N */
	    else if (image_bold(image))
		font_id = panel->bold_font_id;
	    else
		font_id = panel->std_font_id;
	    if (lines == 1) {
		/* Center single line within label rect */
		baseline_y += (rect->r_height - chrht) / 2;
#ifdef OW_I18N
		panel_paint_text(pw, font_id, color_index,
				 rect->r_left, baseline_y,
				 (wchar_t *) IMAGE_STRING_I18N(image));
#else
		panel_paint_text(pw, font_id, color_index,
				 rect->r_left, baseline_y,
				 (u_char *) IMAGE_STRING_I18N(image));
#endif /* OW_I18N */
	    } else {
...
#ifdef OW_I18N
    /* Set the IM conversion status active or inactive as appropriate. */
    if (ic_active (ip) == FALSE)
            xv_set(panel->public_self, WIN_IC_ACTIVE, FALSE, NULL);
    else
            xv_set(panel->public_self, WIN_IC_ACTIVE, TRUE, NULL);
#endif
}
    ...
    int		    y;	/* y-coordinate for focus window */
{
	...
	if (sb) {
	    ...
	    pixels_per_unit = (int) xv_get (sb, SCROLLBAR_PIXELS_PER_UNIT);
	    if (y < (view_start * pixels_per_unit) ||
		y >= (view_start + view_length) * pixels_per_unit) {
	...
	if (sb) {
	    ...
	    view_length = (int) xv_get(sb, SCROLLBAR_VIEW_LENGTH);
	    pixels_per_unit = (int) xv_get (sb, SCROLLBAR_PIXELS_PER_UNIT);
	    if (x < (view_start * pixels_per_unit) ||
		x >= (view_start + view_length) * pixels_per_unit) {
		max_view_start = (int) xv_get(sb, SCROLLBAR_OBJECT_LENGTH) -
		    view_length;
...
Pkg_private void
panel_error(object, msg)
    Xv_object	 object;
    char	*msg;
{
    xv_error(object,
	     ERROR_STRING,	XV_MSG_VAR(msg),
	     ERROR_PKG,		PANEL,
	     NULL);
}
