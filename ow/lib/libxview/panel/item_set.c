#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)item_set.c 20.104 93/05/14";
#endif
#endif
    ...
    Attr_avlist     avlist;
{
	...
	switch (attrs[0]) {
	  ...
	  case PANEL_VALUE_FONT:
...
#ifdef OW_I18N
		ip->value_font_id = (XFontSet)
			xv_get(ip->value_font, FONT_SET_ID);
#else
		ip->value_font_id = (XID) xv_get(ip->value_font, XV_XID);
#endif /* OW_I18N */
		if (ip->value_font != panel->std_font)
		...
		ip->value_font_id = panel->std_font_id;
	  ...
	  case PANEL_ITEM_MENU:
...
#ifdef OW_I18N
		{
		    ...
		    label_data = (Xv_opaque) panel_strsave_wc(IMAGE_STRING_I18N(label));
		}    
#else
		    label_data = (Xv_opaque) IMAGE_STRING_I18N(label);
#endif /* OW_I18N */
		else
 ...
 * PANEL_ITEM_WANTS_WC					     */
	...
	label_size = XV_PF_TEXTWIDTH(STRLEN(IMAGE_STRING_I18N(label)),
				     image_font(label), IMAGE_STRING_I18N(label));
	if (ip->label_width)
