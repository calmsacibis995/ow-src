#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)p_btn.c 20.115 94/07/22";
#endif
#endif
    ...
    Event          *event;
{
	...
	height = 0;
        ...
        if (pin_is_default) {
            ...
            IMAGE_STRING_I18N(&image) = NULL_STRING;
            label = (Xv_opaque) NULL_STRING;
	    olgx_state |= OLGX_LABEL_IS_WCS;
        } else if (!(IMAGE_STRING_I18N(&image) = (CHAR *) 
	...
	if (pin_is_default) {
	    ...
	    IMAGE_STRING_I18N(&image) = NULL_STRING;
	    label = (Xv_opaque) NULL_STRING;
	} else if (!(IMAGE_STRING_I18N(&image) = (char *) xv_get(default_menu_item,
		MENU_STRING))) {
	    olgx_state |= OLGX_LABEL_IS_PIXMAP;
	    image.im_type = PIT_SVRIM;
	    image_svrim(&image) = xv_get(default_menu_item, MENU_IMAGE);
    ...
    Event          *event;
{
	...
	if (panel) panel->item_destroy_flag = 0;
    ...
    Menu            menu;
{
	...
	if (gen_menu == NULL) {
	    panel_error(menu,
 XV_MSG_CONST("begin_preview: menu's gen_proc failed to generate a menu"));
	    return (NULL);
	}
	return (gen_menu);
    } else
    ...
    Event	   *event;
{
    ...
    panel = ip->panel;
    if (panel) panel->item_destroy_flag = 0;
    if ( (ip->flags & IS_MENU_ITEM) && panel)
        panel->item_destroy_flag = 1;
    (*ip->notify) (ITEM_PUBLIC(ip), event);
    if (panel && panel->item_destroy_flag == 2)
	return; 
    if (panel) panel->item_destroy_flag = 0;
