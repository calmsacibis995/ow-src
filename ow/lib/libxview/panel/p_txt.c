#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)p_txt.c 20.260 97/05/21";
#endif
#endif
...
#ifdef OW_I18N
static void     text_free_terminators();
#endif
    ...
    Attr_avlist     avlist;
{
    ...
    font_set_extents = XExtentsOfFontSet(panel->std_font_id);
    ...
    dp->undo_buffer = (CHAR *) xv_calloc(1,
			(u_int) (dp->stored_length * sizeof(CHAR) + 1));
    ...
    if (!dp->undo_buffer || !dp->value_wc || !dp->value)
#else
    dp->value = (char *) xv_calloc(1, (u_int) (dp->stored_length + 1));
	 ...
	 */
	snprintf(delim_chars, 256, delims);
    ...
    register Attr_avlist avlist;
{
	...
	switch (avlist[0]) {
	  ...
	  case PANEL_VALUE_WCS:
	    ...
	    /* Can't carry selection over to new value */
	    if ( panel->sel_holder[PANEL_SEL_PRIMARY] == ip )
		xv_set(panel->sel_owner[PANEL_SEL_PRIMARY], 
		       SEL_OWN, FALSE, 
		       NULL);

	    if (avlist[0] == PANEL_VALUE)
	        new_value = _xv_mbstowcsdup((char *) avlist[1]);
	    else
	        new_value = panel_strsave_wc((wchar_t *) avlist[1]);
	    break;
#else
	  case PANEL_VALUE:
...
#ifdef OW_I18N
            font_set_extents = 
		XExtentsOfFontSet(ip->value_font_id);
            pc_home_y = font_set_extents->max_logical_extent.y;
#else
	    x_font_info = (XFontStruct *) xv_get(ip->value_font, FONT_INFO);
	    pc_home_y = -x_font_info->ascent;
	  ...
	  case PANEL_NOTIFY_STRING:
	    text_free_terminators(dp);
	    dp->terminators_wc = _xv_mbstowcsdup((char *) avlist[1]);
	    break;

	  case PANEL_NOTIFY_STRING_WCS:
	    text_free_terminators(dp);
	    dp->terminators_wc = panel_strsave_wc((wchar_t *)avlist[1]);
	    break;
#else
	  case PANEL_NOTIFY_STRING:
	    if (dp->terminators)
	  ...
	  case PANEL_VALUE_STORED_LENGTH:
	    dp->flags &= ~STORED_LENGTH_WC;
	    goto p_v_s_l_w; /* "p_v_s_l_w" in just below case */

	  case PANEL_VALUE_STORED_LENGTH_WCS:
	    dp->flags |= STORED_LENGTH_WC;
p_v_s_l_w: /* "p_v_s_l_w" is just used from above case */
	  ...
	  case PANEL_VALUE_STORED_LENGTH:
	    ...
	    dp->undo_buffer = (CHAR *) realloc(dp->undo_buffer,
			(u_int) (dp->stored_length + 1) * sizeof(CHAR));
...
#ifdef OW_I18N
            if ((dp->display_length > 4) ||
		   (dp->flags & PTEXT_IS_NUMTEXT))
                dp->display_width = panel_col_to_x(ip->value_font, dp->display_length);
            else
            {   
                XFontSetExtents *font_set_extents;

                font_set_extents = XExtentsOfFontSet(ip->value_font_id);
                dp->display_width = dp->display_length * font_set_extents->max_logical_extent.width;
            }
#else
            dp->display_width = panel_col_to_x(ip->value_font, dp->display_length);
#endif /* OW_I18N */

	    goto set_no_button; /* "set_no_button" is in the next case */
...
set_no_button: /* "set_no_button" is used from above case blk only */
            /* If length is 4, make room for the scrolling buttons. */
            if((dp->display_length < 4)
                && (dp->display_length >= dp->stored_length))
	        no_buttons = TRUE;
            else 
                no_buttons = FALSE; 
	...
	dp->value[dp->stored_length]=NULL;
#endif /* OW_I18N */
    ...
    va_list         avlist;
{
	...
	if (which_attr == PANEL_VALUE) {
	    wcstombs (dp->value, dp->value_wc, dp->stored_length);
...
#ifdef OW_I18N
      case PANEL_MASK_CHAR: 
	wctomb(temp_mask, dp->mask);
	return (Xv_opaque) temp_mask[0];
    ...
    Destroy_status  status;
{
	...
	if ((panel->sel_holder[i] == ip)&&
	    ((int)xv_get(panel->sel_owner[i], SEL_OWN))) {
    ...
    register Event *event;
{
    ...
    /* If the Shift key is down, a SELECT is an ADJUST. */
    if ( (event_action (event) == ACTION_SELECT) &&
	(event->ie_shiftmask & SHIFTMASK) )
	event_set_action (event, ACTION_ADJUST);
	...
	if (dnd_decode_drop(panel->sel_req, event) != XV_ERROR) {
		...
		update_value(ip, event_action(event), TRUE, FALSE, &retstatus);
			...
			dp->caret_position = dp->seln_first[PANEL_SEL_PRIMARY];
#ifdef OW_I18N
			size.x = XwcTextEscapement(ip->value_font_id,
...
/* If there are uncommitted preedit text when any
 * of the following event comes in, then the preedit
 * has to be committed first.  This is called implicit
 * commit.
 */
...
/* convert the committed string from multibyte to wide
 * char once for later use.
 */
...
#ifdef OW_I18N
    if (panel_printable_char(event_action(event), ie_string_wc) ||
#else
	...
	paint_caret(ip, FALSE);
#ifdef  OW_I18N
    /* if the event is caused by committed string from the
     * input method server, this ie_string is always non 
     * NULL.
     */
    if (ie_string_wc != NULL) {
	wc_ptr = ie_string_wc;
    ...
    Panel_item	    item_public;
{
    ...
    /* Cancel any selections this item owns, except for the clipboard. */
    for (i = 0; i < NBR_PANEL_SELECTIONS; i++) {
	if ((panel->sel_holder[i] == ip)&&
	    ((int)xv_get(panel->sel_owner[i], SEL_OWN))) {
	    char *rank_name;
	    rank_name = (char *) xv_get(panel->sel_owner[i], SEL_RANK_NAME);
	    if (strcmp(rank_name,"CLIPBOARD")!=0) {
		xv_set(panel->sel_owner[i], SEL_OWN, FALSE, 0);
	    }
	}
    }
...
#ifdef OW_I18N
		/*
		 * Update the XNSpotlocation for the preedit region
		 * if we are using an overTheSpot preedit style.
		 */
    ...
    int          highlight;
{
	...
	if (dp->first_char)
	    x += dp->scroll_btn_width;
	if (dp->mask == (CHAR) '\0') /* not masked */
	{
	    PANEL_EACH_PAINT_WINDOW(panel, pw)
		panel_paint_text(pw, ip->value_font_id, 
				ip->color_index,
...
#ifdef OW_I18N
/*
 * In case of OW_I18N, it is most likely wide character is necessary,
 * therefor panel_printable_char() will require to have them by the
 * caller.
 */
panel_printable_char(code, wcs)
    int             code;	/* event action code */
    wchar_t	   *wcs;
{
    /*
     * Check wide char first, then check event code in case that is
     * looks like an 8 bit chars.
     */
    return  ((wcs != NULL && iswprint(*wcs)) ||
	     (code <= 0xff && isprint(code)));
}
    ...
    int             rank;	/* PANEL_SEL_PRIMARY or PANEL_SEL_SECONDARY */
{
    ...
    size.x = XwcTextEscapement(ip->value_font_id, dp->value_wc,
			       wslen(dp->value_wc));
#else
	...
	for (i = dp->last_char; (i >= 0) && (x < max_caret); i--)  {
            x += XwcTextEscapement(ip->value_font_id, 
				   &dp->value_wc[i], 1);
	}
#else
	XFontStruct	*x_font_info;
    ...
    register Event *event;
{
    ...
    wchar_t		*wcs;
    int			 mb;
    Panel_setting	 result;


    if (event_string(event))
	wcs = _xv_mbstowcsdup((char *) event->ie_string);
    else
	wcs = NULL;

    if (panel_erase_action(event) ||
	panel_printable_char(event_action(event), wcs) ||
	event_action(event) == ACTION_CUT ||
	event_action(event) == ACTION_PASTE ||
	event_action(event) == ACTION_UNDO) {
	result = PANEL_INSERT;
	...
	if (wcs != NULL)
	    mb = event->ie_string[0];
	else
	    mb = event_action(event);

	if (mb == xv_iso_next_element || mb == '\r' || mb == '\n')
	    result = event_shift_is_down(event) ? PANEL_PREVIOUS : PANEL_NEXT;
	else
	    result = PANEL_NONE;
    } else
	result = PANEL_NONE;
    ...
    register Item_info *ip;
{
	...
	if (is_string(&ip->label)) {
	    image_size = XV_PF_TEXTWIDTH(STRLEN(IMAGE_STRING_I18N(&ip->label)),
			  image_font(&ip->label), IMAGE_STRING_I18N(&ip->label));
    ...
    int		   *format;
{
	...
	*format = 8;
	...
	goto length;  /* "length:" is in the next if block */
	...
	sdfprintf((stderr, "Length %d\n", dp->sel_length_data));
/*
 * "length:" will be used from panel->atom.length_chars if block in
 * above.
 */
length:
	...
	*format = 32;
    ...
    } else if (*type == (Atom) xv_get(XV_SERVER_FROM_WINDOW(panel_public),
                SERVER_ATOM, "TARGETS")) {

        static Atom atom_list[9];
        int i;
 
        i=0;    
        atom_list[i++] = XA_STRING;
        atom_list[i++] = panel->atom.delete;
        atom_list[i++] = panel->atom.length;
        atom_list[i++] = panel->atom.selection_end;
        atom_list[i++] = panel->atom.seln_yield;
        atom_list[i++] = panel->atom.compound_text;
        atom_list[i++] = panel->atom.length_chars;
        atom_list[i++] = (Atom) xv_get(XV_SERVER_FROM_WINDOW(panel_public),
                SERVER_ATOM, "TARGETS");
        atom_list[i++] = (Atom) xv_get(XV_SERVER_FROM_WINDOW(panel_public),
    ...
    int		    rank;
{
	...
	if (seln_highlight == HL_INVERT) {
	    /* need to save the caret state because panel_invert doesn't
	       know about the caret */
	    paint_caret(ip,FALSE);
		...
		/* need to save the caret state because panel_clear_rect 
		   doesn't know about the caret */
		paint_caret(ip,FALSE);
    ...
    int		    rank;	/* PANEL_SEL_PRIMARY or PANEL_SEL_SECONDARY */
{
    ...
    for (new = dp->seln_first[rank]; new <= dp->seln_last[rank];
	 new++)
#ifdef OW_I18N
	dp->undo_buffer[undo_index++] = dp->value_wc[new];
#else
...
#ifdef OW_I18N
    /*
     * In the OW_I18N case, we will NOT use Selection_item, please see
     * the panel_impl.h (around define for sel_item) for more details.
     */
#endif
    ...
    int		    calc_from_caret_pos;
{
    ...
    dp->caret_offset  = XwcTextEscapement(ip->value_font_id, 
	&dp->value_wc[dp->first_char], 
	dp->caret_position - dp->first_char);
    ...
    int             *retstatus;
{
    ...
    *retstatus=0;
	...
	if (sel_length != SEL_ERROR) {
	     ...
	     * today in local xfer.  Otherwise, this code should not
	     * be necessary.
	     */
		...
		if (sel_length == 0) {
		    ...
		    if (state < 0) {
		        ...
		        goto error;/* "error:" is beginning of next if block */
		    }
...
/*
 * "error:" is used from just above if block.
 */
error:
#else
	if (sel_length == SEL_ERROR) {
	 ...
	 * sel_length = MIN(sel_length, dp->stored_length + 1 
	 * - orig_len); also changed OW_I18N code
	 */
	...
	dp->caret_position += sel_length;
        val_change = sel_length;
	...
	selection_action = TRUE;
        /* ... fall through to ACTION_GO_CHAR_BACKWARD *//*FALLTHROUGH*/
	...
	selection_action = TRUE;
	/* ... fall through to ACTION_GO_CHAR_FORWARD *//*FALLTHROUGH*/
	...
	selection_action = TRUE;
	/* ... fall through to ACTION_GO_WORD_BACKWARD *//*FALLTHROUGH*/
	 ...
	 *  to calculate word separations for other locales.
	 *  For ASCII set we're going through the
	 *  original algorithm plus making sure we're not stepping
	 *  into other code sets.  For non-ASCII sets we do not
	 *  check for delimiters, but use wckind.so to tell us
...

	{
	    ...
	    if (iswascii(sp[insert_pos-1])){
	        for (i = insert_pos - 1; i >= 0 && 
			(iswascii(sp[i])) && 
			delim_table[sp[i]]; i--);
	    }
	    else
	...
	selection_action = TRUE;
	/* ... fall through to ACTION_GO_WORD_END *//*FALLTHROUGH*/
...
#ifdef OW_I18N
	{
		...
		for (i = insert_pos; i < orig_len && 
			(wchar_kind(sp[i]) == wc_type); i++);
		for (; i < orig_len && (wchar_kind (sp [1] ) != wc_type); i++);
	    }
	}
	...
	selection_action = TRUE;
	/* ... fall through to ACTION_LINE_START *//*FALLTHROUGH*/
	...
	selection_action = TRUE;
	/* ... fall through to ACTION_LINE_END *//*FALLTHROUGH*/
 ...
 * That is when "action" argument contains wide character instead of
 * event_action.
 */
wc_printable:
	if (iswprint(action)) {
	 ...
	 */
		...
		if (orig_len < dp->stored_length) {	/* there is room */
	     ...
	     } else {            /* alarm if read-only */
		Text_info *dp = TEXT_FROM_ITEM (ip);

		if (dp->flags & PTXT_READ_ONLY) text_alarm(ip);
	    }
	}
	break;
	...
	if (new_len < orig_len) {
		...
		XFontSetExtents *font_set_extents;
		int string_width;
                pc_adv_x = XwcTextEscapement(ip->value_font_id, 
					     &sp[insert_pos], 
					     1);
		string_width = XwcTextEscapement(ip->value_font_id, 
						 &sp[insert_pos],
						 wslen(&sp[insert_pos]));
		font_set_extents = XExtentsOfFontSet(ip->value_font_id);
		PANEL_EACH_PAINT_WINDOW(panel, pw)
		    ...
		    XClearArea(xv_display(info), xv_xid(info),
			       ...
			       ip->value_rect.r_top,
			       string_width,
			       font_set_extents->max_logical_extent.height,
			       False);
		    panel_paint_text(pw, ip->value_font_id,
    ...
    int		    caret_sensitive;
{
		...
		x += XwcTextEscapement(ip->value_font_id, 
              ...
              {  
                if (!dp->first_char)
                {
                  ...
                  {
#ifdef OW_I18N
		      y += XwcTextEscapement(ip->value_font_id, 
	...
	/* Be sure we didn't back up past an inserted character. */
	if ( (val_change > 0) && (dp->last_char < dp->caret_position) )
		dp->last_char = dp->caret_position;

	/* Compute value offset */
#ifdef OW_I18N
	size.x = XwcTextEscapement(ip->value_font_id,
				   &dp->value_wc[dp->first_char],
				   dp->last_char - dp->first_char + 1 < 0 ?
				   0 : dp->last_char - dp->first_char + 1);
#else
    ...
    Item_info		*ip;
{
    ...
    /* set up */
    interm_display_len = XwcTextEscapement(ip->panel->std_font_id, 
	ip->panel->preedit->text->string.wide_char,
...
/*  NOTE:  The new R5 XIM spec does not have visible position
 *  nor visible type, so have to nuke the following code to
 *  meet the spec.  But what to do?  This is a design issue
 *  However, the code is left here under #ifdef notdef for
 *  future reference.
 */
	...
	case XIMVisiblePosMid:
		...
		i >= 0 && len >= 0; i--)
		len -=XwcTextEscapement(ip->panel->std_font_id, &str[i], 1);
	...
	case XIMVisiblePosEnd:
	    ...
	    display_length = XwcTextEscapement(ip->panel->std_font_id, str, wslen(str));
		...
		while ((j < real_display_len) && (i >= 0)) {
		    prev_pos = i;
		    j += XwcTextEscapement(ip->panel->std_font_id, &str[i], 1);
		    i--;
		}
	...
	case XIMVisiblePosStart:
	    /* ----------X++++++++++++++, including X */
	    display_length = XwcTextEscapement(ip->panel->std_font_id, 
	...
	if (primary_seln_panel && ip->panel->preedit->text->length)
	    text_seln_delete(ip, PANEL_SEL_PRIMARY);
	 /*
	...
	/*
	 * Repaint for invert or underline if there is selection.
	 * Becuase the attributes disappear with paint_value_and_interm().
	 * In this case, ip->panel->preedit->text->length is always 0.
	 */
	if (primary_seln_panel || secondary_seln_panel)
	    paint_value(ip, PV_HIGHLIGHT);
    }
}
...

{
    ...
    /* for some reason in 493 and 1093 preedit text is painted
     * 2 pixels higher than before, so the underline also need
     * to be moved up 2 pixels.  This can be removed once we
     * figured out why preedit text is painted two pixels
     * higher than the text ledge.
     */
    y = rect_bottom(&ip->value_rect) - 2;
    count = 0;
	...
	case XIMReverse:
	    ...
	    adv_x = XwcTextEscapement(ip->panel->std_font_id, 
	...
	case XIMUnderline:
	    ...
	    PANEL_EACH_PAINT_WINDOW(ip->panel, pw)
	    if (ip->color_index >= 0) {
		xv_vector(pw, interm_rect.r_left, y, 
			  interm_rect.r_left+interm_rect.r_width-1, 
			  y, ip->color_index < 0 ? PIX_SET :
			  PIX_SRC | PIX_COLOR(ip->color_index),
			  0);
	     } else {
		DRAWABLE_INFO_MACRO(pw, info);
    ...
    register XIMFeedback	*attr;
{
	...
	j += XwcTextEscapement(ip->panel->std_font_id, 
				&str[last_pos], 1);
	last_pos++;
    }
    /* Sometimes when boundary is right in the middle */
    ...
    register XIMFeedback  	*interm_attr;
{
    ...
    value_right_display_len = XwcTextEscapement(ip->panel->std_font_id,
	...
	if (str_left) {
	    PANEL_EACH_PAINT_WINDOW(panel, pw)
		panel_paint_text(pw, ip->panel->std_font_id, 
				 ip->color_index, x, 
				 y+dp->font_home, &str_left[0]);
            PANEL_END_EACH_PAINT_WINDOW
            x += XwcTextEscapement(ip->panel->std_font_id, 
	...
	if (str_right) {
        ...
        PANEL_EACH_PAINT_WINDOW(panel, pw)
                panel_paint_text(pw, ip->panel->std_font_id, 
				 ...
				 y+dp->font_home, buf);
        PANEL_END_EACH_PAINT_WINDOW
        x += XwcTextEscapement(ip->panel->std_font_id, buf, wslen(buf));
        xv_free(buf);
    }
    ...
    Item_info	*ip;
{
    ...
    int		      caret_pos; /* caret positioning */
    int               item_height;
 ...
 */
	...
	/* Erase the preedit string first */
	
 	caret_pos = dp->text_rect.r_left + dp->caret_offset;
	XwcTextExtents(ip->value_font_id,
		       panel->preedit->text->string.wide_char,
		       panel->preedit->text->length,
		       &overall_ink_extents,
		       &overall_logical_extents);
	
	if (dp->flags & UNDERLINED) {
	    if (ip->panel->status.three_d) {
		/* 3D text ledge is 2 pixels high.*/
		item_height = ip->value_rect.r_height - 2;
	    } else {
		/* (2D is 1 pixel high.) */
		item_height = ip->value_rect.r_height - 1;
	    }
	} else {
	    item_height = ip->value_rect.r_height;
	}
	PANEL_EACH_PAINT_WINDOW(panel, pw)
	    DRAWABLE_INFO_MACRO(pw, info);
	    XClearArea(xv_display(info), xv_xid(info),
		       caret_pos,
		       ip->value_rect.r_top,
		       ip->value_rect.r_width - caret_pos,
		       item_height,
		       False);
	PANEL_END_EACH_PAINT_WINDOW
	    
/* Update the panel text item with the committed string */

	while (*wc_ptr && (i < dp->stored_length)) {
	    dp->undo_buffer[i] = *wc_ptr;
...
/* update undo info */
	...
	i = (i < dp->stored_length) ? i : dp->stored_length;
	dp->undo_buffer[i] = '\0';
	dp->undo_direction = DELETE;
	dp->saved_caret_offset = dp->caret_offset;
	dp->saved_caret_position = dp->caret_position;
    ...
    panel->preedit->text->string.wide_char = NULL_STRING;
    ...
    Text_info	*dp;
{
    ...
    dp->terminators = (char *) 0;
    dp->terminators_wc = (wchar_t *) 0;
}
#endif /* OW_I18N */
