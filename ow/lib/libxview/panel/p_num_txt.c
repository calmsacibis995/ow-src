#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)p_num_txt.c 20.54 95/01/23";
#endif
#endif
    ...
    Attr_avlist avlist;
{
    ...
    dptmp = TEXT_PRIVATE(dp->text_field);
    dptmp->flags |= PTEXT_IS_NUMTEXT;
    ...
    Attr_avlist	    avlist;
{
	...
	switch (avlist[0]) {
	  ...
	  case XV_KEY_DATA:
		...
		xv_set(dp->text_field, XV_KEY_DATA, avlist[1], avlist[2], 0);
	    break;

	  case PANEL_CLIENT_DATA:
	    xv_set(dp->text_field, PANEL_CLIENT_DATA, avlist[1], 0);
	    break;
	  ...
	  case PANEL_VALUE_WCS:
	    value_change = TRUE;
	    new_value = (int) watoi((wchar_t *)avlist[1]);
	    break;
#else
	  case PANEL_MASK_CHAR:
	    value = (char) avlist[1];
...
#ifdef OW_I18N
      case PANEL_VALUE_WCS:
	return (Xv_opaque) xv_get(dp->text_field, PANEL_VALUE_WCS);
#endif /* OW_I18N */
      case PANEL_MIN_VALUE:
    ...
    Event *event;
{
	...
	return FALSE;
      ...
      case PANEL_NON_PRINTABLE:
        return (event_is_string(event)
		|| !panel_printable_char(event_action(event), NULL));
