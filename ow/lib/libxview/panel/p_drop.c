#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)p_drop.c 1.23 93/06/14";
#endif
#endif
    ...
    Attr_avlist	    avlist;
{
    ...
    for ( ; *avlist; avlist = attr_next(avlist)) {
	  ...
	  case PANEL_DROP_BUSY_GLYPH:
		...
		panel_error(avlist[1],
			    XV_MSG_CONST("Invalid Server Image specified"));
		break;
	    }
	  ...
	  case PANEL_DROP_DND_TYPE:
	    switch( (int) avlist[1] ) {
	        case PANEL_DROP_USERDEF:
                ...
                default:
                  panel_error(avlist[1],
			XV_MSG_CONST("Invalid value for PANEL_DROP_DND_TYPE"));
                  break;
            }
	    break;
