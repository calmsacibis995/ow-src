#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)p_mlinetxt.c 1.36 96/06/11";
#endif
#endif
...
typedef struct {
    ...
    int		    show;	   /* initial XV_SHOW value for textsw */
#ifdef OW_I18N
    char	   *mbs_value;
    int		    mbs_value_size;
    ...
    Attr_avlist avlist;
{
    ...
    /*
     * Pre-parse XV_SHOW.  Don't really set it until the
     * Textsw gets positioned in mltxt_resize().
     */
    dp->show = TRUE;
    for ( ; *avlist; avlist = attr_next(avlist))
	if ( avlist[0] == XV_SHOW ) {
	    ATTR_CONSUME(avlist[0]);
	    dp->show = (int) avlist[1];
	    break;
	}


    dp->textsw = xv_create(panel_public, TEXTSW,
	XV_SHOW, FALSE,		/* don't map until positioned */
    ...
    Panel_item	    item_public;
{
    ...
    /*
     * Hold off setting initial XV_SHOW value until the
     * Multiline is positioned.  This prevents the Textsw
     * from getting mapped at the default position (0,0)
     * and later getting moved by the resize op.
     */
    if ( dp->show != ALREADY_POSITIONED ) {
	if ( dp->show == TRUE )
	    xv_set(dp->textsw, XV_SHOW, TRUE, 0);
	dp->show = ALREADY_POSITIONED;
    }
    ...
    Event *event;
{
	...
	return FALSE;
	  ...
	  else return !panel_printable_char(event_action(event), NULL);
