#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)help.c 1.78 97/05/21";
#endif
#endif
    ...
    char	   *sys_command;
{
    ...
    if (display_env) {
	snprintf(buffer, 64, "DISPLAY=%s", display_env);
	putenv(buffer);
    }
    ...
    Event          *client_event;
{
...
#ifdef OW_I18N
    app_name_id = (char *) xv_get (server, XV_APP_NAME);
    SPRINTF(client_name,"%s: %s", gettext (app_name_id ? app_name_id
	: xv_app_name), XV_MSG ("Help") );
#else
	...
	help_info->help_textsw = xv_create(help_info->help_frame, TEXTSW,
				...
				/* Fix for 4005231 - don't want the textsw to become writable */
