...
#if defined(i386)
/*
 * XXXppc:  I believe this to be bogus.  I can't find anyplace in OL or XW
 * that sets the MP_I386 bit, so we always end up with LSByte/MSBit, which
 * is what you would expect and what's done on the Vax.
 * And so that's what I'm doing on ppc.
 * Jordan.Brown@west.sun.com 07/16/94 (rev 11/10/94)
 */
...
#else
#if defined(ultrix) || defined(__ppc)
        ximage->byte_order = LSBFirst;
        ximage->bitmap_bit_order = MSBFirst;
#else
...
#define DEPTH_24        5
	...
	if (!(temp_gcs[index] = XCreateGC(display, xid, 0, 0))) {
	    printf(XV_MSG("Server probably run out of memory in XCreateGC\n"));
	    abort();
	} else {
	    return (temp_gcs[index]);
	}
