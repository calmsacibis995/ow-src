#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)window_cms.c 20.56 93/04/07";
#endif
#endif
...
#ifdef OW_I18N
            /*
             * Update XNColormap for the preedit or status region
	     * when colormap updated or WIN_CMS set. This is necessary,
	     * for instance, when the preedit or status region
	     * is being drawn by the IM server (using overTheSpot or
	     * imDisplaysInClient input method styles).
             */
...
#ifdef OW_I18N
            /*
             * Update XNForeground when WIN_FOREGROUND_COLOR is set.
	     * This is necessary, for instance, when the preedit or status
	     * region is being drawn by the IM server (using overTheSpot
	     * or imDisplaysInClient input method styles).
             */
            if (win->win_use_im && win->xic) {
                XVaNestedList       list;
...
#ifdef OW_I18N
            /*
             * Update XNBackground when WIN_BACKGROUND_COLOR is set.
	     * This is necessary, for instance, when the preedit or status
	     * region is being drawn by the IM server (using overTheSpot or
	     * imDisplaysInClient input method styles).
             */
            if (win->win_use_im && win->xic) {
                XVaNestedList       list;
