#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)xv_win_lp.c 1.10 93/05/28";
#endif
#endif
...
static void		xv_block_sigs();
static void		xv_unblock_sigs();
...
Frame   frame;
{
    ...
    /*
     * Block signals when xv_window_loop() is active
     */
    xv_block_sigs(&oldmask);
...
/*
 * The functions xv_block_sigs() and xv_unblock_sigs() use code out of
 * ndet_signal_catcher()
 */
    ...
    sigset_t	*oldmask;
{
    ...
    /*
     * If not already in notifier's signal handler, block signals
     */
    ...
    sigset_t	*oldmask;
{
    ...
    /*
     * If not already in notifier's signal handler, unblock signals
     */
    if (!NTFY_IN_INTERRUPT)  {
