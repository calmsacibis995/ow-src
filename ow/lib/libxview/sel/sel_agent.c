#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)sel_agent.c 1.86 94/11/21";
#endif
#endif
    ...
    Atom                target;
{
	...
	/*
	 * This is in case of that the last paste failed. This usually happens
	 * when textsw failed to paste all selection data due to textsw's
	 * memory buffer is full. Then, resetting some variables is required
	 * so that textsw can paste after the insertion fail. (Bug 1104615)
	 *  Note: agent should be reset in the stage when the last
	 *	  paste failed, not now.
	 */
	agent->get_prop.offset = 0;
...
ReadProperty:
	...
	if (return_type == None) {
	    if (agent->get_prop.offset) {
		/*
	 	 * This is in case of that the last paste failed. This usually
		 * happens when textsw failed to paste all selection data due
		 * to textsw's memory buffer was full. Then, try to read the
		 * property newly after resetting the agent.
		 *  Note: the agent should be reset in the stage when the last
		 *	  paste failed, not now.
	 	 */
		agent->get_prop.offset = 0;
    ...
    XSelectionRequestEvent *req_event;
{
    ...
    ptr_for_agent = (Atom *)&agent->targets;
    for (i=0; i<sizeof(Seln_target_atoms)/sizeof(Atom); i++)
        if (*ptr_for_agent++) count++;

    targets=xv_malloc(count*sizeof(Atom));

    ptr_for_targets = targets;
    ptr_for_agent = (Atom *)&agent->targets;
    for (i=0; i<sizeof(Seln_target_atoms)/sizeof(Atom); i++) {
        if (*ptr_for_agent) {
         *ptr_for_targets = *ptr_for_agent;
          ptr_for_targets++;
        }
        ptr_for_agent++;
    }
    ...
    int             do_block;
{
	...
	/* there's some SunView 1 remnants, so there could be a 
	   SelectionNotify event for SUN_SELN_DO_FUNCTION and the targets
	   won't match. This will fail later so we make sure that the
	   targets match if a SelectionNotify event was found, otherwise
	   we block again. */
	
	if ((xevent.type & 0177)==SelectionNotify) {
	    XSelectionEvent *xselevent;
	    xselevent = (XSelectionEvent *)&xevent;
	    if (target != xselevent->target) {
		block(display, &xevent, agent->timeout);
	    }
	}
    }
...
#else SVR4
	result = select(ConnectionNumber(display) + 1, &select_ibits, NULL, NULL, timeout);
#endif SVR4
