#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)sel_clnt.c 20.42 94/03/11";
#endif
#endif
    ...
    Seln_rank       asked, *given;
{
    ...
    /* figure out if the holder is the same process. If so, then
       we need to call selection_send_yield_internal(),
       otherwise we only need to eventually call XSetSelectionOwner()
       which will send a SelectionClear event. */
    switch (asked) {
      case SELN_UNSPECIFIED:
	if (selection_function_pending(server)) {
	    asked = SELN_SECONDARY;
	} else {
	    asked = SELN_PRIMARY;
	}			/* FALL THROUGH	 */
      case SELN_CARET:
      ...
      case SELN_SECONDARY:
      case SELN_SHELF:
	if (agent->client_holder[ord(asked)].state==SELN_NONE) {
	    same_process = False;
	} else {
	    same_process = True;
	}
    ...
    *given = holder.rank;
    ...
    if (same_process) {
	result = selection_send_yield_internal(server,holder.rank,&holder);
    } else {
	result = Seln_seized_ok;
    }
    seln_give_up_selection_without_telling_server(server, holder.rank);
    ...
    Seln_file_info  input;
{
    ...
    switch (input.rank) {
      case SELN_UNSPECIFIED:
	if (selection_function_pending(server)) {
	    input.rank = SELN_SECONDARY;
	} else {
	    input.rank = SELN_PRIMARY;
	}			/* FALL THROUGH	 */
      case SELN_CARET:
      case SELN_PRIMARY:
      case SELN_SECONDARY:
      case SELN_SHELF:
	if (agent->client_holder[ord(input.rank)].state==SELN_NONE) {
	    same_process = False;
	} else {
	    same_process = True;
	}
	...
	if (same_process) {
	    (void)selection_send_yield_internal(server,input.rank,&holder);
	}
	seln_give_up_selection_without_telling_server(server, input.rank);

	/* need to set state to SELN_FILE since it was just lost above.
	   keeping it as SELN_EXISTS will cause selection_acquire() to 
	   return without calling selection_agent_acquire()*/
