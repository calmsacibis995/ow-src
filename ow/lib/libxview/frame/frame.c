#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)frame.c 20.72 93/06/15";
#endif
#endif
    ...
    Rect           *rect;
{
    ...
    win_getrect(frame, &new_rect);
    
    if ((int)xv_get(frame, XV_SHOW) || status_get(frame_info, iconic)) {
        int		x, y;
        XID		notused;
        XTranslateCoordinates(display, win_xid, 
        		      (XID) xv_get(xv_root(info), XV_XID), 
        		      0, 0, &x, &y, &notused);
	new_rect.r_left= x - left;
	new_rect.r_top = y - top;

    }
    			  
       rect->r_left = new_rect.r_left;
       rect->r_top = new_rect.r_top;
       rect->r_width = new_rect.r_width + left + right;
       rect->r_height = new_rect.r_height + top + bottom;
}
