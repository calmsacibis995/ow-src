...
static char     sccsid[] = "@(#)fm_cmdline.c 20.45 94/04/22";
#endif
...
extern char   **xv_app_name_argv;
extern int      xv_app_name_argc;
    ...
    Bool  	    from_init;
{
	...
	/* send a WM_COMMAND for the first frame */
	    ...
	    int i;
            ...
            DRAWABLE_INFO_MACRO(frame_public, info);
	    if(xv_app_name_argv) {
		XSetCommand(xv_display(info), xv_xid(info), xv_app_name_argv,
			    xv_app_name_argc);
		for(i=0; i < xv_app_name_argc; i++)
		    xv_free(xv_app_name_argv[i]);
		xv_free(xv_app_name_argv);
		xv_app_name_argv = NULL;
	    }
	    else
