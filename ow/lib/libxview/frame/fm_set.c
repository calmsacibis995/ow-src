#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)fm_set.c 20.111 94/11/21";
#endif
#endif
    ...
    Attr_attribute	 avlist[];
{
	...
	switch (attrs[0]) {
	  ...
	  case FRAME_NEXT_PANE: {
		...
		if ((pane == frame->focus_subwindow) || (first_count > 1))
		    break;
	  ...
	  case XV_RECT: {
		...
		adjust_height = (int)xv_get(frame->footer, XV_HEIGHT);
#ifdef OW_I18N
            if (status_get(frame, show_imstatus) && frame->imstatus)
                adjust_height += (int)xv_get(frame->imstatus, XV_HEIGHT);
#endif
	    if (adjust_height) {
	        Rect *rect = (Rect *)attrs[1];

	        rect->r_height += adjust_height;
	  ...
	  case XV_HEIGHT: {
		...
		rect.r_height = height + adjust_height;
	  ...
	  case XV_END_CREATE:{
		...
		if (status_get(frame, show_footer)) {
		    ...
		    /*
		     *  Further restrict the case of when the IM status 
		     *  region is created below the frame footer to
		     *  the IM styles that use it.  In other words, no
		     *  status region is required if the styles are
	 	     *  XIMStatusNone and XIMStatusNothing.
		     */
		    && SERVER_PRIVATE(server_public)->determined_im_style
		        & (XIMStatusArea | XIMStatusCallbacks)) {

                    /* Create an IMstatus region */
