#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)fmhlp_set.c 1.26 94/03/07";
#endif
#endif
    ...
    Attr_attribute  avlist[];
{
    ...
    int             result = XV_OK;
    int             add_decor, delete_decor, motif_decor = 0;
...
get_from_app_name:
                    if (xv_app_name_wcs) {
			char *help_string = XV_MSG ("Help");
			help = "%ws %s";
			i = (wslen(xv_app_name_wcs) + strlen (help_string)
				+ 2 /* space + \0 */ )
				* sizeof (wchar_t);
			wp = sizeof(wbuff) < i ? xv_malloc(i) : wbuff;
                        wsprintf(wp, help, xv_app_name_wcs, help_string);
                    } else {
                        wp = NULL;
                    }
	  ...
	  case XV_END_CREATE:
	    {
	        ...
	        motif_decor |= MWM_DECOR_RESIZE;
                XSetTransientForHint(xv_get(frame_public, XV_DISPLAY),
                     xv_get(frame_public, XV_XID),
                     xv_get(xv_get(frame_public, XV_OWNER), XV_XID));
	        break;
	    }
