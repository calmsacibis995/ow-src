/*#ident "@(#)ol_button.c	1.64 94/05/02 SMI" */
...
/* this returns a different base_off which depends on the label font 
   instead the olgx glyph font. */
int
     ...
     int state;
{
    ...
    /* only recalculate the offset for string labels */
    if ((state & OLGX_LABEL_IS_PIXMAP) || (state & OLGX_LABEL_IS_XIMAGE)) {
	return offset;
    }
    /* if the font is much bigger than the glyph font, then center it
       a little better.  This breaks the OpenLook spec and can not
       happen for XView (except for fonts > x-large scale), but it
       can happen for OLIT */
#ifdef OW_I18N    
    if (Olgx_Flags(info) & OLGX_FONTSET) {
	XFontSetExtents *fx;
	fx = XExtentsOfFontSet(info->textfontset);
	/* center base_offset if label font is a little larger than
	   button font. */
	if ((int)(offset - fx->max_logical_extent.y)>=(int)height) {
	    offset = height - 
		(int)(height-fx->max_logical_extent.height)/2 +
		fx->max_logical_extent.y;
	    /* we have to have some sort of offset */
	    if (offset<=1) {
		offset = 2;
	    }
	}
	   ...
	   /* need to remove the inactive flag because the button is
	    ...
	    olgx_draw_text(info, win, (char *) label,
			   ...
			   y + info->button_height 
			     - olgx_calc_font_offset(info,info->button_height,
						     state),
			   inside_width -
    ...
    int             state;
{
    ...
    /* no text can fit, so just return */
    if (max_width<0) {
	return;
    }
#ifdef OW_I18N
    ...
    int             x, y, width, height, state;
{
    ...
    XGetGeometry(info->dpy,pix,&root,&x_dummy,&y_dummy,&pixmap_width,
                 &pixmap_height,&bw_dummy,&depth);

    /* We don't want to pass any width or height greater than the
       pixmap width or height to XCopyArea() or XCopyPlane() because
       the background will get tiled if we do. */
					 ...
					 at texty */
{
	...
	if (state & OLGX_LABEL_IS_PIXMAP) {
		...
		if (label_pixel_pos < (m_pos + text_width)) {
		    ...
		    /* if underline is too low, then it means font is
		       larger than glyph font and so we need to fudge
		       the underline back into the area. */
			...
			/* if height was incorrectly passed, then need
			   to check if this is really below the texty.
			   If not, then just ignore all these calculations. */
			if (label_y_pos < texty) {
			    label_y_pos = old_y_pos;
			}
		    }
		    /* there could be some perverse case where the
		       label is really small or some combination of
		       font metrics is not accounted for,
		       so make sure label_y_pos is positive. */
		    if (label_y_pos <= y) {
			label_y_pos = y + 1;
		    }
    ...
    int             state;            /* state of the actual object */
{
    ...
    /* don't need to call olgx_calc_font_offset() for the offset because
       the text is placed at x,y anyway and the y is only used to center
       pixmaps or ximages which don't depend on the font. */
