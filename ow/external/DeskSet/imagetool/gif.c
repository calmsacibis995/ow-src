...
#ifndef lint
static char sccsid[] = "@(#)gif.c 1.24 97/02/24";
#endif
...
{                                                           \
  if (pstk_idx == PSTK_SIZE)                                \
    gifin_fatal( MGET("pixel stack overflow in PUSH_PIXEL()")) ;   \
  else                                                      \
    pstk[pstk_idx++] = (p) ;                                \
}
...
{
/*
  sprintf(gif_err_str, MGET ("Error reading GIF file: %s\n", msg)) ;
*/
  strcpy (gif_err_str, MGET ("Error reading GIF file: "));
              ...
              for (x = 0; x < gifin_img_width; x++)
                {
                  if (gifin_get_pixel(&pixel) != GIFIN_SUCCESS)
      			  ...
      			  closefile () ;
                        ...
                        {
                          sprintf(gif_err_str,
                                  MGET ("%s: Short read within image data\n"),
                                  image->file) ;
                          x = gifin_img_width ;
                          y = gifin_img_height ;
                        }
...
ImageInfo	*image;
{
    ...
    xil_get_info (gif_save_image, &width, &height, &nbands, &datatype);
