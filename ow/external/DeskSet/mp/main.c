...
{
  FILE *prolog_fp;
  char i18n_prologfile[MAXPATHLEN];
  char buf[MAXLINE];
  char *tmp = NULL;
  char *locale = NULL;
  int setuplocale = 0;
 ...
 */
  ...
  if ((prologue = getenv("MP_PROLOGUE")) == NULL)
    {
      if ((prologue = getenv("OPENWINHOME")) == NULL) prologue = PROLOGUE ;
      else
        {
          char buf[MAXPATHLEN] ;

          SPRINTF(buf, "%s/share/xnews/client/mp", prologue) ;
  ...
  /* shouldn't need OPENWINHOME set anymore at all... */
  if ( (tmp = getenv("OPENWINHOME")) == NULL )
      tmp = "/usr/openwin";

  SPRINTF(i18n_prologfile, "%s/lib/locale/", tmp);
  /* Figure out which locale document is in */
  if ( prolog_locale != NULL ) {     /* command line arg */
      strcat(i18n_prologfile, prolog_locale);	
      setuplocale = 1;
  } else if ( (tmp = getenv("MP_LANG")) != NULL ) {    /* environment var */
      strcat(i18n_prologfile, tmp);
      setuplocale = 1;
  } else {    /* query locale */
      if ( (locale = setlocale(LC_CTYPE, (char *)NULL)) != NULL ) {
	  if ( strcmp(locale, "C") == 0 ) { /* in C locale */
	      FPUTS("%!PS-Adobe-1.0\n", stdout);
	      FPUTS("/localeprolog 0 def\n\n", stdout);
	  } else {
	      strcat(i18n_prologfile, locale);
	      setuplocale = 1;
	  }
      }
  }
  ...
  if ( setuplocale ) {
	  strcat(i18n_prologfile, "/print/prolog.ps");
	  /* Check whether prolog.ps exist */
	  if ( (prolog_fp = fopen(i18n_prologfile, "r")) == NULL ) {
		/* DEBUG
		FPRINTF(stderr, "Cannot open %s prolog file.\n", i18n_prologfile);
		 */
