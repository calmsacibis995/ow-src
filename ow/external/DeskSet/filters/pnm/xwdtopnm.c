...
static int red_shift, green_shift, blue_shift;	/*	1196468	*/
...
main( argc, argv )
	...
	red_shift = green_shift = blue_shift = 0;	/* 1196468 */
    pnm_init( &argc, argv );
	...
	switch ( visualclass )
		...
		*xP = colors[getpixnum( ifp )];
		...
		switch ( bits_per_pixel )
		    ...
		    case 24:
		    case 32:
				/* Fix for 1196468 - colormap is wrong in truecolor mode */

				PPM_ASSIGN( *xP,
					( ( ul & red_mask ) >> red_shift ),
					( ( ul & green_mask ) >> green_shift ),
					( ( ul & blue_mask ) >> blue_shift ) );
...
"ARGH!  On this machine, X10 headers are larger than X11 headers!\n    You will have to re-write xwdtopnm." );
	...
	*padrightP =
	...
	if ( bits_per_pixel == sizeof(pixel_mask) * 8 )
		...
		if (red_mask > 0)
			for (red_shift = 0, i = red_mask; (i & 1) == 0; i >>= 1, red_shift++) {
			}
		green_mask = h11P->green_mask;
		if (green_mask > 0)
			for (green_shift = 0, i = green_mask; (i & 1) == 0; i >>= 1, green_shift++) {
			}
		blue_mask = h11P->blue_mask;
		if (blue_mask > 0)
			for (blue_shift = 0, i = blue_mask; (i & 1) == 0; i >>= 1, blue_shift++) {
			}
	}
    else
