#ident "@(#)winlist.cc	1.85 94/01/21 Copyright 1990-1992 Sun Microsystems, Inc."
	...
	batch_index		(0)
{
	STRING locale;
	STRING ja_locale = "ja";
	STRING japanese_locale = "japanese";
	...
	locale = (char *)xv_get(panel, XV_LC_BASIC_LOCALE);
	...
	if ((locale != ja_locale) && (locale != japanese_locale)) {
		italic_font = (Xv_Font) xv_find(panel, FONT,
		FONT_FAMILY,	xv_get(regular_font, FONT_FAMILY),
		FONT_SIZE,	xv_get(regular_font, FONT_SIZE),
		FONT_STYLE,	FONT_STYLE_ITALIC,
		NULL);

		if (italic_font == XV_NULL)
...
WINLIST::InsertEntry(int entry, const STRING &label, BOOL bold, GLYPH glyph)
{
	...
	InsertEntry(entry, label, font, glyph);
}

// Add entry to list.
//
void
WINLIST::InsertEntry(int entry, const STRING &label, Xv_font font, GLYPH glyph)
{
	int		n;
...
BOOL
WINLIST::IsHeader(int entry)
{
	if (((caddr_t) xv_get(win_list, PANEL_LIST_CLIENT_DATA, entry)) == NULL)
		return BOOL_TRUE;
	return BOOL_FALSE;
}
...
WINLIST::SetViewBottom(int entry)
{
	...
	SetViewTop(entry - (GetViewNumRows() - 1));
}
...
WINLIST::GetViewBottom()
{
	...
	return(GetViewTop() + GetViewNumRows() - 1);
}
