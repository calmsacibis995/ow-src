...
#ident "@(#)winlist.h	1.63 96/11/15 Copyright 1990-1992 Sun Microsystems, Inc."
...
class WINLIST {
	...
	// Default regular, italic, and bold fonts associated with this list.
	//
	Xv_font		bold_font;
	...
	// The actual InsertEntry function. Calls to the public interfaces
	// InsertEntry and AppendEntry use this function to do the work.
	//
	...
	void	AppendEntry(const STRING &label, BOOL bold)
			{ InsertEntry(NumEntries(), label, bold, NULL); }

	void	AppendEntry(const STRING &label)
			{ InsertEntry(NumEntries(), label, BOOL_FALSE, NULL); }

	void	AppendHeaderEntry(const STRING &label)
			{ InsertEntry(NumEntries(), label, italic_font, NULL); }

	BOOL	IsHeader(int entry);
