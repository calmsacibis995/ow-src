...
#ident	"@(#)ndbm.c	1.2	90/02/08 SMI"	/* SVr4.0 1.2	*/
	...
	for (i=0;;) {
		...
		if (item.dptr == NULL) {
			...
			if ((Sentry.dsize == key.dsize) &&
			    (memcmp(Sentry.dptr, key.dptr, key.dsize) == 0))
				return(-1);
			if (Oneflag == 2) {
				Sentry.dsize = key.dsize;
				Sentry.dptr  = malloc(key.dsize);
				memcpy(Sentry.dptr, key.dptr, key.dsize);
			}
#endif 0
			break;
		}
	...
	datum inkey;
{
	...
	for(i=0;; i+=2) {
		...
		if ((unsigned)i >= sp[0]) {
#else
		swab ((char *) sp, (char *) &swp, sizeof (short));
		if ((unsigned)i >= swp) {
#endif
			item.dptr = NULL;
			item.dsize = 0;
			break; /*from below*/
		}	/* } (to match the closing brace above) */
		else { 
#if !defined(i386)
...
#else
			swab ((char *) (sp+i), (char *) &swp, sizeof (short));
			swab ((char *) (sp+i+1),(char *) &swp1, sizeof (short));
			if (i > 0) item.dsize = swp - swp1;
			else item.dsize = PBLKSIZ - swp1;
			item.dptr = db->dbm_pagbuf+swp1;
#endif
		}
	...
	char buf[PBLKSIZ];
{
	...
	t = PBLKSIZ;
	if (n > 0) {
		swab ((char *) (sp+n), (char *) &swp, sizeof (short));
		t = swp;
	}
	swab ((char *) (sp+n+1), (char *) &swp, sizeof (short));
	item.dptr  = buf + swp;
	item.dsize = t - swp;
#endif
	return (item);
...
#else
	for (i=0, swab ((char *)sp, (char *)&j, sizeof (short)); i<j; i+=2, 
	        swab ((char *)(sp+i), (char *)&n, sizeof (short)) ) 
	{
	        swab ((char *)(sp+i+1), (char *)&swp, sizeof (short));
	        n -= swp;
		if (n != item.dsize)
			continue;
		if (n == 0 || memcmp(&buf[swp], item.dptr, n) == 0)
			return (i);
	}
#endif
	return (-1);
	...
	char buf[PBLKSIZ];
{
...
#else
		swp -= 2;
		swab ((char *)&swp, (char *) sp, sizeof (short));
#endif
		return (1);
	}
	i1 = PBLKSIZ;
	...
	swab ((char *)(sp+n+2), (char *)&swp, sizeof (short));
	i1 -= swp;
#endif
	if (i1 > 0) {
#if !defined(i386)
...
#else
		swab ((char *)(sp+i2), (char *)&swp, sizeof (short));
		i2 = swp;
		swab ((char *)(sp+n+2), (char *)&swp, sizeof (short));
		memmove(&buf[i2 + i1], &buf[i2], swp - i2);
#endif
	}
#if !defined(i386)
	sp[0] -= 2;
	...
	swp -= 2;
	swab ((char *)&swp, (char *) sp, sizeof (short));
	for (sp1 = sp + swp, sp += n+1; sp <= sp1; sp++) {
		swab ((char *) (sp+2), (char *)&swp, sizeof (short));
		swp += i1;
		swab ((char *)&swp, (char *) sp, sizeof (short));
	}
#endif
	...
	datum item, item1;
{
	...
	swab ((char *)&swp, (char *) sp, sizeof (short));
	swp = i1 + item1.dsize;
	swab ((char *)&swp, (char *) (sp + ++i2), sizeof (short));
#endif
	...
	char buf[PBLKSIZ];
{
	...
	if (t < (sp[0]+1)*sizeof(short))
		return (-1);
#else
	swab ((char *) sp, (char *)&swp1, sizeof (short));
	for (i=0; i<swp1; i++) {
		swab ((char *)(sp+i+1), (char *)&swp, sizeof (short));
		if (swp > t)
			return (-1);
	...
	if (t < (swp1+1)*sizeof(short))
		return (-1);
#endif
	return (0);
}

prnobj (char *buf, int num)
{
      int     i;
      char    *bp;
      
      for (i = 0, bp = buf; i < num; i++, bp++)
		printf ((isprint (*bp)) ? "%c  " : ".  ", *bp);
      printf("\n");
      for (i = 0, bp = buf; i < num; i++, bp++)
		printf ("%02X ", *bp);
