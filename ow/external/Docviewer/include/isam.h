/*
 * Copyright (c) 1988-92 by Sun Microsystems, Inc.
 */
...
/* The core functions of ISAM.  These are X/Open compliant;
 * the NetISAM extensions are later:
 */

#ifdef	__STDC__

int	isaddindex(int isfd, struct keydesc *desc);
int	isbuild(const char *path, int maxrec, struct keydesc *desc, int mode);
int	isclose(int isfd);
int	iscntl(int isfd, int func, ...);
int	isdelcurr(int isfd);
int	isdelete(int isfd, char *record);
int	isdelindex(int isfd, struct keydesc *desc);
int	isdelrec(int isfd, long recnum);
int	iserase(const char *path);
int	isindexinfo(int isfd, struct keydesc *desc, int number);
int	islock(int isfd);
int	isopen(const char *path, int mode);
int	isread(int isfd, char *buf, int mode);
int	isrelease(int isfd);
int	isrename(const char *oldname, const char *newname);
int	isrewcurr(int isfd, char *record);
int	isrewrec(int isfd, int recnum, char *record);
int	isrewrite(int isfd, char *record);
int	isstart(int isfd, struct keydesc *desc, int length, char *record, int mode);
int	isunlock(int isfd);
int	iswrcurr(int isfd, char *record);
int	iswrite(int isfd, char *buf);

/* Extensions, beyond the X/Open standard:
 */
int	isaddprimary(int isfd, struct keydesc *desc);
int	ischeckfile(int isfd, int diagmode, int *corrupt);
int	isdelprimary(int isfd);
int	isfsync(int isfd);
int	isgetcurpos(int isfd, int *len, char **buf);
int	isrepair(const char *path, int verbose);
int	issetcurpos(int isfd, char *buf);
int	issync();
int	iswrrec(int isfd, long recnum, char *buf);

/* NetISAM Programmer's toolkit, stores data in architecture
 * independant format and loads it back from that format.
 */
int	isldrec(struct istableinfo *p, char *s, char *r);
int	isstrec(struct istableinfo *p, char *s, char *r);

/* Data conversion routines:  load, store various kinds of 
 * data in a machine-independant format:
 */
