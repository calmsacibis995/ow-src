...
#ident	"@(#)convert.cc	1.23	93/02/17 SMI"
...
#include <multimedia/AudioTypeChannel.h>
#include <multimedia/AudioTypeMux.h>
#include <multimedia/AudioTypeSampleRate.h>
#ifdef TIMER
#include <multimedia/ProcessTimer.h>
#endif
	...
	AudioStream*	ofp)
{
	...
	char*		msg2;
#ifdef TIMER
	ProcessTimer	Convert_timer;
	ProcessTimer	Elapsed_timer;
#endif
...
#ifdef TIMER
	// If gathering statistics, get elapsed time
	if (Statistics) {
		Elapsed_timer.start();
	}
#endif
...
#ifdef TIMER
		// If gathering statistics, get inner loop conversion times
		if (Statistics) {
			Convert_timer.start();
		}
#endif
	...
	if (Statistics) {
		...
		Err(MGET(
		    "%s: length=%s  conversion time=%s  elapsed time=%s\n"),
		    ifp->GetName(),
		    Convert_timer.timestring(pos, tmps),
		    Convert_timer.report_total(),
		    Elapsed_timer.report_real());
	}
#endif
	delete obuf;
	free_conv_list(list);
