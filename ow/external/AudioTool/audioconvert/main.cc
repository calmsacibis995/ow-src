...
#ident	"@(#)main.cc	1.16	93/02/04 SMI"
...
char*		Stdin;
char*		Stdout;
char*		Suffix = ".AUDCVTMP";

char*		progname; // program name
char*		fake_argv[] = {"-", NULL}; // used for stdin when no args
	...
	char*		argv[])
{
	...
	if (strcmp(progname, "ulaw2pcm") == 0) {
		(void) parse_format("ulaw", ihdr, ifmt, i_offset);
		(void) parse_format("pcm", ohdr, ofmt, o_offset);
	} else if (strcmp(progname, "pcm2ulaw") == 0) {
		(void) parse_format("pcm", ihdr, ifmt, i_offset);
		(void) parse_format("ulaw", ohdr, ofmt, o_offset);
	} else if (strcmp(progname, "adpcm_enc") == 0) {
		(void) parse_format("ulaw", ihdr, ifmt, i_offset);
		(void) parse_format("g721", ohdr, ofmt, o_offset);
	} else if (strcmp(progname, "adpcm_dec") == 0) {
		(void) parse_format("g721", ihdr, ifmt, i_offset);
		(void) parse_format("ulaw", ohdr, ofmt, o_offset);
	...
	for ( ; optind < argc; optind++) {
		...
		if ((ofp == NULL) && !pflag) {
			...
			ofp = create_output_file(outfile,ohdr,ofmt,infoString);
...
usage()
{
	fprintf(stderr, MGET(
	    ...
	    progname);
#ifdef TIMER
	fprintf(stderr, MGET("\t-T\tReport timing statistics\n"));
#endif
	exit(1);
}
