...
#ident	"@(#)AudioUnixfile.cc	1.35	97/01/14 SMI"
...
#ifdef SUNOS41			// for getpagesize(), SunOS 4.x
#include <sysent.h>
#endif
...
decode_filehdr()
{
	...
	err = (AudioError) audio_decode_filehdr((unsigned char*)&fhdr,
	    &ohdr, &infosize);
	...
	hdr_local.endian = BIG_ENDIAN; //Files are always written in big endian.
	...
	if (S_ISREG(st.st_mode)) {
		setlength(GetHeader().Bytes_to_Time(
		    st.st_size - infosize - sizeof (Audio_filehdr)));
...
#ifdef notdef
	} else if (ohdr.data_size == AUDIO_UNKNOWN_SIZE) {
		setlength(AUDIO_UNKNOWN_TIME);
	} else {
		setlength(GetHeader().Bytes_to_Time(ohdr.data_size));
#else
...
encode_filehdr()
{
	...
	if (Undefined(GetLength()))
		...
		ohdr.data_size = (u_int)GetHeader().Time_to_Bytes(GetLength());
	...
	err = (AudioError) audio_write_filehdr(getfd(),
	    &ohdr, infostring, infolength);
	...
	Boolean		b)			// FALSE to set non-blocking
{
	...
	if (isfdset()) {
		...
		if ((flag < 0) && (errno == EOVERFLOW || errno == EINVAL)) {
		  RaiseError(AUDIO_UNIXERROR, Fatal, "Large File");
		} else if (b) {
