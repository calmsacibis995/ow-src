...
#ident	"@(#)AudioDetect.cc	1.15	97/02/24 SMI"
	...
	Double		mintime)		// minimum analysis buffer size
{
	...
	hdr = obj->GetHeader(from);
#ifdef notdef
	// Set up for the type of data we can process
	hdr.samples_per_unit = 1;
	hdr.bytes_per_unit = 1;
	hdr.encoding = ULAW;		// XXX - manual conversion for now
	hdr.channels = 1;
#endif
	...
	do {
		...
		if (obj->GetHeader(from).sample_rate != hdr.sample_rate) {
			hdr.sample_rate = obj->GetHeader(from).sample_rate;
			if (buf != NULL)
				buf->Dereference();
