...
#ident	"@(#)AudioFile.cc	1.29	97/01/14 SMI"
	...
	const char*	path)			// pathname or 0
{
	...
	if ((desc < 0) && (errno == EOVERFLOW)) {
		return (RaiseError(AUDIO_UNIXERROR, Fatal, "Large File"));
	} else if (desc < 0) {
	...
	int		openmode)
{
	...
	if (!GetAccess().Writeable()) {
		...
		{
			if ((mapaddr = mmap(0, (int)maplen, PROT_READ, 
...
Close()
{
	...
	if (GetAccess().Writeable() && (origlen != GetLength())) {
		...
		err = (AudioError) audio_rewrite_filesize(getfd(),
		    (u_int)GetHeader().Time_to_Bytes(GetLength()));
	}
