#pragma ident "@(#)util.c	1.9 95/02/22"
/*
...
static char *SysErrorMsg (int n)
{
    ...
    extern int sys_nerr;
    char *s = ((n >= 0 && n < sys_nerr) ? sys_errlist[n] :
					dgettext(OlmhDomain,"unknown error"));

    return (s ? s : dgettext(OlmhDomain,"no such error"));
}
...
void Punt(char *str)
{
	...
	(void)fprintf(stderr, dgettext(OlmhDomain,"forcing core dump.\n"));
	(void) fflush(stderr);
	abort();
    }
    else {
	(void)fprintf(stderr, dgettext(OlmhDomain,"exiting.\n"));
	(void)fflush(stderr);
	_exit(-1);
    }
}
...
int myclose(int fid)
{
    if (close(fid) < 0) Punt(dgettext(OlmhDomain,"Error in myclose!"));
    DEBUG1( "# %d : <Closed>\n", fid)
}
...
{
    int fid = fileno(file);
    if (fclose(file) < 0) Punt(dgettext(OlmhDomain,"Error in myfclose!"));
    DEBUG1("# %d : <Closed>\n", fid)
}
...
FILEPTR FOpenAndCheck(char *name, char *mode)
{
	...
	(void)sprintf(str, dgettext(OlmhDomain,
			"Error in FOpenAndCheck(%s, %s)"), name, mode);
	Punt(str);
    }
    return result;
}
...
void DeleteFileAndCheck(char *name)
{
	...
	perror(progName);
	(void)sprintf(str, dgettext(OlmhDomain,
			"DeleteFileAndCheck(%s) failed!"), name);
	Punt(str);
    }
}
...
void CopyFileAndCheck(char *from, char *to)
{
	...
	perror(progName);
	(void)sprintf(buf, dgettext(OlmhDomain,
			"CopyFileAndCheck(%s->%s) failed!"), from, to);
	Punt(buf);
    }
    do {
...
void RenameAndCheck(char *from, char *to)
{
	...
	if (errno != EXDEV) {
	    ...
	    perror(progName);
	    (void)sprintf(str, dgettext(OlmhDomain,
				"RenameAndCheck(%s->%s) failed!"), from, to);
	    Punt(str);
	}
...
{
    static Arg arglist[] = {XtNstring, NULL};
    arglist[0].value = (XtArgVal) str;
    XtSetValues(widget, arglist, XtNumber(arglist));
}


Widget CreateTextSW(Scrn scrn, char *name, ArgList args, Cardinal num_args)
{
    ...
    scrwin = XtCreateManagedWidget( "scrwin", scrolledWindowWidgetClass,
				    scrn->widget, args, num_args);

    return XtCreateManagedWidget( name, textEditWidgetClass, scrwin,
				  NULL, 0);
}
...
Widget CreateTitleBar(Scrn scrn, char *name)
{
    ...
    result = XtCreateManagedWidget( name, staticTextWidgetClass, scrn->widget,
...
Boolean
ConvertMailText(char *frompath , char *topath , Boolean ToEUC)
{
        FILE            *from , *to;
        enconv_t        cd;
        ...
        cp = &chead[0];
        if(!(cp != NULL && strcmp(cp->cn_locale,lang) == 0)){
        ...
        if(( from =  myfopen(frompath, "r")) == NULL) {
                char str[126];
        ...
        if(ToEUC == TRUE){
                if((cd = enconv_open( cp->cn_intcode, cp->cn_extcode))
                                                        == (enconv_t)-1){
                        fprintf(stderr,dgettext(OlmhDomain,
                ...
                if((cd = enconv_open( cp->cn_extcode, cp->cn_intcode))
                                                        == (enconv_t)-1){
                        fprintf(stderr,dgettext(OlmhDomain,
                                        "Conversion for mail text failed .\n"));                        if(from != NULL)
                                myfclose(from);
        ...
        rewind(from);
        rewind(to);
        while((ileft = fread(ibuf , 1 , BUFSIZ , from)) > 0){
                ip = ibuf;
                do{
                        op = obuf;
                        oleft = BUFSIZ;
