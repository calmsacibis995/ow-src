#pragma ident "@(#)pick.c	1.8 95/06/28"
/*
...
#define WTlabel		staticTextWidgetClass
#define WTbutton	oblongButtonWidgetClass
#define WTtextentry	textEditWidgetClass
...
static FormEntry CreateWidget(RowList row, WidgetClass class, ArgList args, Cardinal num_args);
static void DeleteWidget(FormEntry entry);
static void AddDetailGroup(FormBox form);
...
static PrepareToUpdate(FormBox form)
{
#ifdef NOTDEF
...
static ExecuteUpdate(FormBox form)
{
#ifdef NOTDEF
...
static void AddButton(RowList row, char *text, void (*func)())
{
    FormEntry entry;
    static Arg args[] = {
	{XtNlabel, NULL},
...
static void AddToggle(RowList row, char *text, int initial_state, Widget radio_group, XtPointer radio_data)
{
#ifdef NOTDEF
    FormEntry	entry;
    Arg		args[4];
...
/* ARGSUSED */
static void ExecRowOr(Widget w, XtPointer closure, XtPointer call_data)
{
...
/* ARGSUSED */
static void ExecGroupOr(Widget w, XtPointer closure, XtPointer call_data)
{
...
static int ParseRow(RowList row)
{
    int		result = FALSE;
    int		i;
    FormEntry	entry;
...
static int ParseGroup(Group group)
{
    int found = FALSE;
    int i;
...
/* ARGSUSED */
static void ExecOK(Widget w, XtPointer closure, XtPointer call_data)
{
	...
	*((short*)XawToggleGetCurrent(row2->wlist[1]->widget));
	...
	(void) sprintf(str, dgettext(OlmhDomain,
			"Sequence \"%s\" doesn't exist!"), fromseq);
	PopupError(str);
	return;
    }
...
/* ARGSUSED */
static void ExecCancel(Widget w, XtPointer closure, XtPointer call_data)
{
    Pick pick = ((FormEntry)closure)->row->group->form->pick;
    Scrn scrn = pick->scrn;
...
/*ARGSUSED*/
void XmhCancelPick(Widget w, XEvent *event, String *params, Cardinal *num_params)
{
    Scrn	scrn;
...
static AddGeneralGroup(FormBox form)
{
    Group group;
    RowList row;
    Widget widgetList[4];
    ...
    AddToggle(row, dgettext(OlmhDomain,"Yes"),
			 TRUE, (Widget) NULL, (XtPointer)(&true_data));
    AddToggle(row, dgettext(OlmhDomain,"No"), FALSE, 
	row->wlist[row->numwidgets - 1]->widget, (XtPointer)(&false_data));
    row =  AddRow(group, RTignore);
    widgetList[3] = row->widget;
    AddButton(row, dgettext(OlmhDomain,"OK"), ExecOK);
    AddButton(row, dgettext(OlmhDomain,"Cancel"), ExecCancel);
...
/* %%%	{XtNallowResize, (XtArgVal)TRUE}, */
    ...
    result->outer = XtCreateWidget( "pick", scrolledWindowWidgetClass,
				    pick->scrn->widget,
...
AddPick(Scrn scrn, Toc toc, char *fromseq, char *toseq)
{
    Pick pick;
    ...
    (void) sprintf(str, dgettext(OlmhDomain,"Pick: %s"), TocName(toc));
