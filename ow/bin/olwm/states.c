#ident	"@(#)states.c	26.70	96/10/31 SMI"
    ...
    int x, y;
{
    ...
    /*  ESC# 505714; BID# 1252764
     *  If client geometry can be off the screen, just return True.
     */  
    if (GRV.PositionOnScreen == False)
		return(True);
...
WinPane *ourWinInfo;
{
	...
	case NormalState:
	    ...
	    /* Check to see if we're in the busy state */
	    PropGetBusyState(dpy, winPane->core.self, &cli->isBusy);
	    if(cli->isBusy)
		winFrame->winBusy = MakeBusy(dpy, winFrame);
...
Time	timestamp;
{
	...
	if (cli == CurrentClient && !GRV.FocusFollowsMouse) {
		/* If need be, get a new timestamp */
...
Time	timestamp;
{
	...
	/*
	 * Update the WM_STATE property.  Note that this must follow the 
	 * reparenting of the client's window back to the root.
	 */
	ClientSetWMState(cli, WithdrawnState);
 ...
 *	For now just apply WA_PINSTATE and WA_MENUTYPE.
 */
void
	...
	XPropertyEvent	*event;
{
	...
	/*
	 * Next, update the menu type if necessary
	 */
        if (winAttr.flags & WA_MENUTYPE) {
                if (winAttr.menu_type == AtomMenuFull)
                        cli->framewin->core.client->wmDecors->menu_type =
				MENU_FULL;
                else if (winAttr.menu_type == AtomMenuLimited)
                        cli->framewin->core.client->wmDecors->menu_type = 
				MENU_LIMITED;
                else if (winAttr.menu_type == AtomNone)
                        cli->framewin->core.client->wmDecors->menu_type = 
				MENU_NONE;
        }
}
