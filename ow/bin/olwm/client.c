#ident	"@(#)client.c	26.56	93/07/29 SMI"
...
Client *cli;
{
	...
	/* 
	 * Next, if necessary, nuke the drag and drop property on both the
	 * pane frame and the icon frame. If window cacheing is turned off,
	 * this is a wasted step. 
	 */
 ...
 * ClientSetWMState
 *
 * Sets the wmState field of the client structure and writes the contents of
 * the WM_STATE property.  Since InvisibleState is private to olwm, makes sure
 * that it is coerced to IconicState.  Also, sets the icon to None if we're
 * going to Withdrawn state.
 */
void
