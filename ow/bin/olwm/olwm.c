#ident	"@(#)olwm.c	26.67	94/03/04 SMI"
...
Bool		FallbackMode;		/* Emergency fallback mode */
char		**ArgVec;		/* Argument Vector */
...
static void	DisplayFallbackMessage();
...
olwm: Warning: '%s' is invalid locale for the LC_CTYPE category,\n\
	...
	/* Cache the command line parameters */
	ArgVec = argv;
	...
	/* 
	 * As a final part of olwm, if we've been started in this special
	 * fallback mode, notify the user accordingly.
	 */
	if(FallbackMode) 
	    DisplayFallbackMessage(DefDpy);
...
#ifdef MAXPID
    /*
     * If the system is configured with MAXPID, ensure that the process
     * that is being signaled is sane.
     */
    if (pid <= 0 || pid > MAXPID)
	return;
#endif 
...
/*
 * DisplayFallbackMessage -- display a notice informing the user that 
 * olwm is in a "special" mode...
 */
#define FEEDBACK_MSG "Some fontsets could not be found -- olwm falling back\nto default fontsets and C locale. Please fix\nconfiguration and restart."

static void 
DisplayFallbackMessage(dpy)
Display *dpy;
{
	int screen;
        Text *buttons[1];
	...
	screen = DefaultScreen(DefDpy);
	buttons[0] = GetText("Ok");
	msg = GetText(FEEDBACK_MSG);
	...
	(void)UseNoticeBox(DefDpy,screen,&noticeBox);
