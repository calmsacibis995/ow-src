#ident	"@(#)kbdfuncs.c	1.18	97/01/03 SMI"
    ...
    XKeyEvent *ke;
{
	...
	if (child != None) {
		...
		/*  ESC# 504780; BID# 1241135
		*  
		*  It is not a good idea to send XConfigureWindow to
		*  windows that set override_redirect = True (breaks
		*  the X11 programming spec).  Window managers should
		*  not mess with these kinds of windows.
		*/
		XWindowAttributes winAttr;
		XGetWindowAttributes(dpy, child, &winAttr);
		if(winAttr.override_redirect == False) {
		   xwc.stack_mode = Opposite;
		   XConfigureWindow(dpy, child, CWStackMode, &xwc);
		}
...
{
    /*
     * Most of the these passive grab functions deal with the events
     * on the KeyPress event. Unfortunately, this creates some problems
     * in the event of the quit, as many applications (mostly xview)
     * have confirmers which appear ("Do you really want to quit?"). These
     * confirmers, at times, attempt to grab the keyboard, which could
     * fail if olwm hasn't released the keyboard (via keyrelease during
     * a passive grab). The real solution is for these apps to do less
     * restrictive prompting, but we'll alleviate a bunch of these problems
     * by dealing with the quit function only on key releases.
     */
