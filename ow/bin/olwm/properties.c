#ident	"@(#)properties.c	26.15	94/03/20 SMI"
...
#endif
		else if (atomList[i] == AtomWindowBusy)
			retFlags |= OLWinBusyAvail;
	}
...
/*
 * PropGetBusyState(dpy,win,state)
 */
Bool
PropGetBusyState(dpy,win,state)
	Display		*dpy;
	...
	Bool		*state;
{
	...
	if(!PropAvailable(win, OLWinBusyAvail))
	    return False;

	/* Grab the property, return if necessary */
	newBusyPtr = GetWindowProperty(dpy, win, AtomWindowBusy, 0L,
		LONG_LENGTH(*newBusyPtr), XA_INTEGER, 32, &nItems, &remain);
	if(newBusyPtr == NULL) 
	    return False;
	else {
	    if (nItems != LONG_LENGTH(Bool) || remain != 0) {
		/* Got property, but it is invalid */
		return False;
	    }
	}

	/* If we got here,  we're in pretty good shape */
	*state = *newBusyPtr;
