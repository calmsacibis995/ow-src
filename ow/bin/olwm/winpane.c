#ident	"@(#)winpane.c	26.22	94/03/20 SMI"
...
WinPane	*winInfo;
{
	...
	/*
	 * If we're dealing with an EnterNotify that occured during a 
	 * a grab, then don't worry about installing the colormap (this 
	 * could cause problems with TrackingSubwindows/WM_COLORMAP_WINDOWS).
	 */
...
WinPane	*winInfo;
{
	...
	{
            /* Mark current state */
	    StateWithdrawn(winInfo->core.client,NO_TIMESTAMP);
	}
}
