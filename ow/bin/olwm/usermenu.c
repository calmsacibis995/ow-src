#ident	"@(#)usermenu.c	26.62	93/08/16 SMI"
    ...
    menudata   *menu;
{
    ...
    int         rval = MENU_NOTFOUND;
    static int  menuRecursionCount = 0;	/* Needed to detect recursion */

    /*
     * We can't deal with displaying anymore than MAX_ACTIVE_MENUS, so
     * use this as a means of detecting recursion problems. If the
     * recursion count goes higher than this, bail with an error.
     */
    if (++menuRecursionCount > MAX_ACTIVE_MENUS) {
    ...
    /*
     * Next, try to figure out where the menu file is located. First, expand
     * all environment variables currently in the path.
     */
    if ((new = expandPath(file)) != NULL)
	file = new;

    /*
     * Next, if the file is not given as a path, try to find out where 
     * precisely the file is...
     */
    if (strchr(file, '/') == NULL) {

	/* initialize the menuSearchPath the first time through */
	if(menuSearchPath == NULL)
	    makeMenuSearchPath();

	/* Walk down the list of paths, find the appropriate one */
	for (pFmt = menuSearchPath; *pFmt; pFmt++) {
	    (void)sprintf(fullPath, *pFmt, file, locale);

#ifdef DEBUG
fprintf(stderr,"menuFromFile: trying '%s'\n",fullPath);
#endif

	    if(access(fullPath, R_OK) == 0) {
		/* 
		 * Bingo -- try to load up the menu. If it fails, continue
		 * the search...
		 */
		if ((stream = fopen(fullPath, "r")) != NULL) {
		    rval = parseMenu(fullPath, stream, menu, &lineno);
		    (void)fclose(stream);

		    /* If things are looking good, then do exit loop */
		    if(rval >= MENU_OK)
		        break;
		    else 
			fprintf(stderr, 
			    GetString("olwm: Error trying to load menu file %s, attempting to fallback to other menu files\n"),fullPath);
	        }
	    }
	} 

	/*
	 * If there's a problem, note it and bail
	 */
	if(rval < MENU_OK) {
	...
	/* 
	 * Don't worry about the leading path stuff -- just try opening
	 * the file as is.
	 */
        ...
        /* Made it this far -- go ahead and parse the menu */
    ...
    char       *file;
{
    ...
    if (menuFromFile(file, userroot) >= MENU_OK) {
    ...
    int        *lineno;
{
	...
	if (strcmp(prog, "INCLUDE") == 0) {
	    ...
	    if (args != NULL) {
		rval = menuFromFile(args, currentMenu);
		switch (rval) {
	        case MENU_PINNABLE:
	    	    currentMenu->pinnable = True;
		    /* FALL THRU */
	...
	if (strcmp(prog, "MENU") == 0) {
		...
		if ((rval = parseMenu(filename, stream,
		    ...
		    /*
		     * If we've got a fatal error, we've really got some
		     * problems. With INCLUDE syntaxt it is a little easier
		     * to handle error conditions, as they are all contained
		     * in the included file. But here, where should the 
		     * stream pointer be? Best thing to do is to bail. 
		     */
		    return (MENU_FATAL);
		} else
    ...
    char       *deftitle;
{
...
#else
	b->label[0] = MemNewString(bdata->name);
#endif
	b->label[1] = NULL;
	b->which = 0;
	...
	bsave = bdata;
	bdata = bdata->next;
	MemFree(bsave->name);		/* Free original string */
	MemFree(bsave);
	AppendMenuItem(m, b);
    }
 ...
 *	
 * 	Note: centerline has a problem in the way it interprets union 
 * 	initializations, so deal define something special for the 
 * 	Button structures to initialize the union elements.
 */
#ifdef OBJECTCENTER
#define ENUM_NULL	NULL
#else
#define ENUM_NULL	{ NULL }
#endif
