#ident	"@(#)menu.c	26.80	94/03/10 SMI"
    ...
    Bool	 flbutton;
{
    ...
    if (menu->menudirty) {
	/*
	 * A piece of one of the menus has changed. Change all corresponding
	 * menus on all of the screens to ensure consistency with what's
	 * cached in the global menu list.
	 */
	l = ScreenInfoList;
	for(scr = ListEnum(&l); scr != NULL; scr = ListEnum(&l)) {
	    menuCache = scr->menuCache;
	    for(i = 0; i < menuCache->nextSlot; i++) {
		if(menu == menuCache->menuInfoList[i]->menu) {
		    menuCache->menuInfoList[i]->cachedMenuIsDirty = True;
		    break;
		}
	    }
	}
    ...
    if(menuInfo->cachedMenuIsDirty) {
	recalcCachedMenu(winInfo, menu);
	menuInfo->cachedMenuIsDirty = False;
    }
    ...
    KeyBinding *binding;
{
    ...
    Text *modsep;
    Text *temp;
    unsigned int altctlmask, modmask;
    ...
    /*
     * Here's a bit of ugliness. In certain circumstances, we want to
     * map the ctl+alt sequence to the meta sequence. Hence, do so.
     */
    if(GRV.CtrlAltMetaKey) {
        altctlmask =  stringToModifier(DefDpy, "Alt");
	altctlmask |= stringToModifier(DefDpy, "Ctl");
	if((altctlmask & binding->modstate) == altctlmask) {
	    bInfo->has_meta_mark = True;
	    modmask = (binding->modstate & (~altctlmask));
	} else {
	    modmask = binding->modstate;
	}
    } else {
	modmask = binding->modstate; 
    }
	...
	if (ks == XK_Meta_L || ks == XK_Meta_R) {
	    ...
	    case XK_Shift_L:
	    case XK_Shift_R:
/* STRING_EXTRACTION - X defines a number of shift modifiers;
 *	shift, ctrl, alt, super and hyper.  Only shift, ctrl
 *	and alt are used with existing Sun keyboards; super
 *	and hyper are for future use.  The following string
 *	messages are these shift modifiers.
 */
		temp = GetText("shift");
		break;
	    case XK_Control_L:
    ...
    ScreenInfo *scrInfo;
{
    ...
    /* 
     * Loop for all of special non-root menus. Don't forget the last submenu
     * hanging off of the limited menu (MENU_LIMITED + 1 slot).
     */
    for (i = 0; i <= (int) MENU_LIMITED + 1; i++) {
    ...
    int        *bindex;
{
	...
	if (hitwindow != None) {
	    /*
	     * Translate where the event occurred relative to the 
	     * menuwin.
	     */
	    WinRootPos(menuInfo->menuWin,&RootX,&RootY);
	    ex = rx - RootX;
	    ey = ry - RootY;
    ...
    Bool	fInverse;
{
    ...
    /* If this is a SEPARATOR, return */
    if ((*pb->label == NULL) && !pb->enabled)
	return;
