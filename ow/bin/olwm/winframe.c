#ident	"@(#)winframe.c	26.77	93/08/16 SMI"
    ...
    WinPaneFrame *winInfo;
{
	...
	/* Intersect with the pane's rectangle */
	paneRect.x = widthLeftFrame(winInfo);
	paneRect.y = heightTopFrame(winInfo);
	paneRect.width = winInfo->fcore.panewin->core.width;
	paneRect.height = winInfo->fcore.panewin->core.height;
	XShapeCombineRectangles(cli->dpy, winInfo->core.self, ShapeBounding,
			        0, 0, &paneRect, 1, ShapeIntersect, Unsorted);
