#ident	"@(#)wingframe.c	26.33	94/06/23 SMI"
...
static	Window		windowOfLastButPress = None;
	...
	switch (a) {
		 ...
		 * Save the location where the button went down so we
		    ...
		    (WinFunc(frameInfo,fcore.selectPress) != NULL))
                 ...
                 * twice.  So, we check that the time stamp and window
		 * of this button press is different than the last.
                 */
                        if (event->xbutton.time == timeOfLastButPress &&
			    event->xbutton.window == windowOfLastButPress)
                                /* We already dealt with this event. */
                                break;
                        timeOfLastButPress = event->xbutton.time;
                        windowOfLastButPress = event->xbutton.window;
	    ...
	    currentAction = a;
	    if (event->xbutton.subwindow != panewindow
	    	&& WinFunc(frameInfo,fcore.menuPress) != NULL) {
