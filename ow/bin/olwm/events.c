#ident	"@(#)events.c	26.49	94/04/11 SMI"
    ...
    unsigned int	mod;
{
    ...
    /*
     * For each modifier number, there's a possible of max_keypermod
     * being attached to the key. Therefore, search through the list.
     */
    for(i=mod*ModMap->max_keypermod; i<(mod + 1)*ModMap->max_keypermod; ++i) {
	kc = ModMap->modifiermap[i];
	if(kc != 0)
	    break;
    }

    /* If none was found, return NoSymbol */
    if(kc == 0)
	return NoSymbol;
	
    /* Otherwise, return the keysym */
